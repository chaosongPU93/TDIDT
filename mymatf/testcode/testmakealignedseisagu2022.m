% testmakealignedseisagu2022.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% try to make an aligned seismogram of a specific tremor
% burts as the logo of the poster for AGU 2022
% 
% 
%
...
tstbuf = min(tcnti(indtmaxi)-2)+100;
tedbuf = tstbuf+50;
...
for ista = 1: nsta
  optdat(:, ista+1) = optseg(1+msftaddm-off1ic(ista): end-msftaddm-off1ic(ista), ista+1);
end
...
plot(ax, relatime, optdat(:,2)*0.8, 'r-', 'linew',1);
plot(ax, relatime, optdat(:,3), 'b-', 'linew',1);
plot(ax, relatime, optdat(:,4), 'k-', 'linew',1);
xlim(ax,[(i-1)*50 i*50]);
% ylim(ax,[-1 1]);
% axis(ax,'off');
...
%%
clc
f = initfig(11,1.5,1,1); %initialize fig
nrow = 1;
ncol = 1;
xran = [0.06 0.96]; yran = [0.05 0.95];
xsep = 0.03; ysep = 0.03;
optaxpos(f,nrow,ncol,xran,yran,xsep,ysep);

ax = f.ax(1);
hold(ax,'on'); grid(ax,'off'); ax.Box = 'off';
relatime = (1:size(optdat,1))/sps;
plot(ax, relatime, optdat(:,2), 'r-', 'linew',0.8);
plot(ax, relatime, optdat(:,3), 'b-', 'linew',0.8);
plot(ax, relatime, optdat(:,4), 'k-', 'linew',0.8);
xlim(ax,[0 40]);
ylim(ax,[-0.25 0.25]);
axis(ax,'off');

set(f.fig,'Color','none');

orient(f.fig,'landscape');
% print(f.fig,'-dpdf','/home/chaosong/Pictures/seislogo.pdf');
export_fig(f.fig,'-pdf','-transparent','/home/chaosong/Pictures/seislogo.pdf');
