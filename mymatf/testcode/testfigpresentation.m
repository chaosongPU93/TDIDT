% function get_filtering_effect_PGCv4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
%   differen with v3, stack first, then filter with different bands. . It turns
%   out that order of filtering/stack doesn't really affect the result too much
%    under 40 or 80 sps base
%
%
% By Chao Song, chaosong@princeton.edu
% First created date:   2019/09/23
% Last modified date:   2019/09/23
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
FLAG = 'PGC';
CATA = 'new';
[PERMROTS, POLROTS] = GetRotsCommon(FLAG,fam,datapath,0,0);
    ...
    for ista = 1: nsta
        ...
        if LIA
            ...
            if isfile(fname)    % if have the data file
                ...
                % 2. this is for data with no response
                [opt, ort, timepola] = readpola_nofilterv2(prename, ...
...
for ista = 1: nsta
    ...
    ampmax = max(stack(ista, mid-sps*5+1: mid+sps*5));
    ampmin = min(stack(ista, mid-sps*5+1: mid+sps*5));
    if ampmax >= -ampmin
...
%%% figure 2
figure('Position',[0.1*wid 0.2*hite 0.3*wid 0.3*hite]); %(1:10*sps+1)/sps,
color=['r','b','k'];
for ista = 1: nsta
...
color=['r','b','k'];
ista = 1;
[psdxx, fp] = pwelch(stackno(ista, mid-5*sps: mid+5*sps),[],[],[],sps); %40 is sps
p1 = plot(fp, 10*log10(psdxx),'linewidth', 1,'color',...
    color(ista)); hold on
ista = 2;
[psdxx, fp] = pwelch(stackno(ista, mid-5*sps: mid+5*sps),[],[],[],sps); %40 is sps
p2 = plot(fp, 10*log10(psdxx),'linewidth', 1,'color',...
    color(ista)); hold on
ista = 3;
[psdxx, fp] = pwelch(stackno(ista, mid-5*sps: mid+5*sps),[],[],[],sps); %40 is sps
p3 = plot(fp, 10*log10(psdxx),'linewidth', 1,'color',...
    color(ista)); hold on
plot([2 2],[-100 100],'k--','linewidth',1.5);
legend([p1 p2 p3],{stas(:,:)},'location','Northeast','fontsize',12);
text(4,-20,'f_{peak} = ~2 Hz','fontsize',14);
xlabel('Frequency (Hz)');
ylabel('PSD (dB)');
xlim([0 20]);
ylim([-80 -10]);
...
%%% cc PGC and SSIB
[coef, lag] = xcorr(stackno(2,mid-5*sps: mid+5*sps), ...
                        stackno(1,mid-5*sps: mid+5*sps), 'coeff'); % cc relative to PGC
[maxcoef, idx] = max(coef);
lagsamp = lag(idx);
figure
plot(lag,coef, 'linewidth', 1); hold on
% text(-300,0.8,'Normalized cross-correlation function','fontsize',14);
vertical_cursors

figure
plot(stackno(1, mid-5*sps: mid+5*sps) + 1* 1, 'linewidth', 1); hold on
plot(stackno(2, mid+lagsamp-5*sps: mid+lagsamp+5*sps) + 1* 2, 'linewidth', 1);
text(50,1*1,stas(1,:),'fontsize',12);
text(50,1*2,stas(2,:),'fontsize',12);
...
figure
hold on
plot(-mshift:mshift,sumstack12n,'b-','linewidth', 1);
plot(-mshift:mshift,sumstack13n,'r-','linewidth', 1);
plot(-mshift:mshift,sumstack32n,'k-','linewidth', 1);

plot(imaxstack12cent,sumstack12n(imaxstack12),'bs','markersize',6);
plot(imaxstack13cent,sumstack13n(imaxstack13),'ro','markersize',6);
plot(imaxstack32cent,sumstack32n(imaxstack32),'kd','markersize',6);
...
plot(xmaxstack12ntmphf,sumstack12n(xmaxstack12ntmphf+mshift+1),'bs','markersize',10);
plot(xmaxstack13ntmphf,sumstack13n(xmaxstack13ntmphf+mshift+1),'ro','markersize',10);
plot(xmaxstack32ntmphf,sumstack32n(xmaxstack32ntmphf+mshift+1),'kd','markersize',10);
% vertical_cursors
legend('CC PGC-SSIB(12)','CC PGC-SILB(13)','CC SILB-SSIB(32)','individual \Delta{t}_{12}=-1',...
       'individual \Delta{t}_{13}=0', 'individual \Delta{t}_{32}=-1',...
       'constrained \Delta{t}_{12}=-1','constrained \Delta{t}_{13}=0',...
       'constrained \Delta{t}_{32}=-1', 'interpretor','latex');
xlabel('Samples (on 80 sps)','fontsize',12);
...
% plot(xmaxstack13n,ymaxstack13n,'ro','markersize',10);
% plot(xmaxstack32n,ymaxstack32n,'kd','markersize',10);
plot(xmaxstack12ntmplf,sumstack12n(xmaxstack12ntmplf+mshift+1),'bs','markersize',10);
plot(xmaxstack13ntmplf,sumstack13n(xmaxstack13ntmplf+mshift+1),'ro','markersize',10);
plot(xmaxstack32ntmplf,sumstack32n(xmaxstack32ntmplf+mshift+1),'kd','markersize',10);
% vertical_cursors
legend('CC PGC-SSIB(12)','CC PGC-SILB(13)','CC SILB-SSIB(32)','individual \Delta{t}_{12}=-3',...
       'individual \Delta{t}_{13}=-3', 'individual \Delta{t}_{32}=2',...
       'constrained \Delta{t}_{12}=-2','constrained \Delta{t}_{13}=-3',...
       'constrained \Delta{t}_{32}=1', 'interpretor','latex');
xlabel('Samples (on 80 sps)','fontsize',12);
ylabel('Normalized CC Coef.','fontsize',12);
