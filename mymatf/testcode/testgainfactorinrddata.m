% testgainfactorinrddata.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
%
% --This function is to test why the same gain factor to scale the data at KLNB
% result in a much different amplitude level from other POLARIS stations before
% and after 2003. In particular, before and after 2003, the deconvolved impulse
% amp ratio, or the waveform amp ratio between the trio stations remains pretty
% stable (see slide 11 in 'pre20221118'). That being said, the gain factor used
% for PGC for all times, and gain factor used for SSIB, SILB before and after 
% 2003 is fine, at least the relative amp relation is not altered. However,
% as another POLARIS station, amp ratio between PGC and KLNB clearly changed 
% from <1 to ~1. In other words, KLNB became smaller.
%
% --Given gain factor at PGC is likely to be fine, maybe the gain
% factors before and after 2003 that have a difference of 5 does not apply to 
% KLNB. According to the amp ratio at other sta pairs and the distance between
% them, amp ratio between PGC and KLNB is more likely to be ~1. Thus, the factor
% before 2003 CAN be improper for KLNB.
% 
% 
%
...
%%
f1 = plt_traceindetail(optseg(:,2:end),stas,50*sps);
title(f1.ax(1),sprintf('Burst #%d, %s %s %s',idxbst,dy,mo,yr));

%%
figure
plot(optseg(:,2),'k-'); hold on
plot(optseg(:,end),'r-');
