...
figpath = strcat(workpath, '/project2021/testfigs');
...
%%%%%% Uncomment above WHEN using merge_fam_befhypo.m, means removing duplicates AFTER hypo %%%%%%%
    ...
    allrst_new = [datemat allrst(:,1:9) eratprior eratpost eratprior4 eratpost4 ...
                  eratprior8 eratpost8 eratprior16 eratpost16 famarr];

    % sort by the energy ratio against prior/posterior 2.5*length_of_strongest_window          
    [allrst_sort, indori] = sortrows(allrst_new, [12, 11],'descend');
    
%     % get the 95th percentile of col 12 and col 11
%     col11thr = prctile(allrst_new(:,11), 80);
%     col12thr = prctile(allrst_new(:,12), 80);
%     indobj = indori(allrst_sort(:,11)>=col11thr & ...
%                     allrst_sort(:,12)>=col12thr);
%     rstobj = allrst_new(indobj,:);

%     %way 1, choose some thresholds
%     const = 0.2;
...
%     rstobj = allrst_new(indobj,:);
    ...
    %way 2, just blindly choose the first 5%
    nuse = ceil(size(allrst_sort,1)* 1);
...
%         plot(ax,[is+1/hi is+1/lo],[-0.8*yma -0.8*yma],'k','linew',3)
        ...
        off12 = allrst_new(ndet, 4);
        off13 = allrst_new(ndet, 3);
...
%         minampneg = min(tracehf(max(idiff-cncntr,1):min(idiff+cncntr,winlen), 2:4), [],1);    
        ...
        text(ax,0.05,0.9,strcat(num2str(lohf),'-',num2str(hihf),{' Hz'}),'fontsize',9,'unit',...
            'normalized','Horizontalalignment','left');
        text(ax,0.95,0.9,sprintf('%.4f',ymami(1)),'fontsize',8,'unit',...
             'normalized','Horizontalalignment','right','color','r');
        text(ax,0.95,0.8,sprintf('%.4f',ymami(2)),'fontsize',8,'unit',...
...
%         TAPER = 'none';
        ...
        elseif strcmp(WAVE, 'day')
            ...
            nwinlen = 64*sps;
            trace1d = [];
            trace1d(:,1:2) = trace1df(ind-nwinlen/2+1: ind+nwinlen/2, 1:2);
            % although no pre-alignment, but aligned here according to the results
            trace1d(:,3) = trace1df(ind-nwinlen/2+1 - off12: ind+nwinlen/2 - off12, 3);
            trace1d(:,4) = trace1df(ind-nwinlen/2+1 - off13: ind+nwinlen/2 - off13, 4);
            trace = trace1d;
            
            [~,ind] = min(abs(rstobj(i, 8)-trace1dbbf(:,1)));
            trace1dbb = [];
            trace1dbb(:,1:2) = trace1dbbf(ind-nwinlen/2+1: ind+nwinlen/2, 1:2);
            trace1dbb(:,3) = trace1dbbf(ind-nwinlen/2+1 - off12: ind+nwinlen/2 - off12, 3);
            trace1dbb(:,4) = trace1dbbf(ind-nwinlen/2+1 - off13: ind+nwinlen/2 - off13, 4);
            tracebb = trace1dbb;
...
%         %%%%%
        ...
        maxamppos = max(trace(max(idiff-cncntr,1):min(idiff+cncntr,nwinlen), 2:4), [],1);
        minampneg = min(trace(max(idiff-cncntr,1):min(idiff+cncntr,nwinlen), 2:4), [],1);    
        ymami = max([maxamppos; -minampneg], [], 1);
        ym = 1.1*max(ymami);
...
%         plot(ax,[is+1/hi is+1/lo],[-0.8*yma -0.8*yma],'k','linew',3)
        ...
        text(ax,0.05,0.1,FLAG,'fontsize',10,'unit','normalized','Horizontalalignment','left');
        text(ax,0.15,0.1,fam,'fontsize',10,'unit','normalized','Horizontalalignment','left');
        text(ax,0.25, 0.9, num2str(off13),'fontsize',10,'color','k','unit','normalized',...
             'Horizontalalignment','left');
        text(ax,0.35, 0.9, num2str(off12),'fontsize',10,'color','b','unit','normalized',...
             'Horizontalalignment','left');
...
%         xlabel(ax,'Time (s)','fontsize',9); 
        ...
        tracebp = tracebbtap;
        for ista = 1: nsta
            tracebp(:,ista+1) = Bandpass(tracebbtap(:,ista+1), sps, lohf, hihf, npo, npa, 'butter');
        end
        
%         idiff = round((allrst_new(ndet, 8)-allrst_new(ndet, 2)+winlensec/2)*sps);
        fixlen = max(4*sps, 1/lohf*sps);
        maxamppos = max(tracebp(max(idiff-0.5*fixlen,1): min(idiff+0.5*fixlen,nwinlen), 2:4), [],1);
        minampneg = min(tracebp(max(idiff-0.5*fixlen,1): min(idiff+0.5*fixlen,nwinlen), 2:4), [],1);
%         maxamppos = max(tracebp(max(idiff-cncntr,1):min(idiff+cncntr,nwinlen), 2:4), [],1);
%         minampneg = min(tracebp(max(idiff-cncntr,1):min(idiff+cncntr,nwinlen), 2:4), [],1);    
        ymami = max([maxamppos; -minampneg], [], 1);
        ym = 1.1*max(ymami);
...
%         patch(ax,xarea,yarea,'k','Facealpha',0.15,'edgecolor','none');
        ...
        plot(ax,[is+(idiff-0.5*fixlen)/sps is+(idiff-0.5*fixlen)/sps],ax.YLim,'k--','linew',1.5);
        plot(ax,[is+(idiff+0.5*fixlen)/sps is+(idiff+0.5*fixlen)/sps],ax.YLim,'k--','linew',1.5);
        ...
        for ifreq = 1: length(octavef)-1
            ...
            if diff(2*[lohf hihf]/sps)>=0.01  % the bandwidth is moderate
                ...
                for ista = 1: nsta
                    ...
                    %interpolate finally
                    tracebp(:,ista+1) = interp(tracedsbp(:,ista),denom/num);
                end
            ...
            fixlen = max(4*sps, 1/lohf*sps);
            maxamppos = max(tracebp(max(idiff-0.5*fixlen,1): min(idiff+0.5*fixlen,nwinlen), 2:4),...
                            [],1);
            minampneg = min(tracebp(max(idiff-0.5*fixlen,1): min(idiff+0.5*fixlen,nwinlen), 2:4),...
                            [],1);
%             maxamppos = max(tracebp(:, 2:4), [],1);
%             minampneg = min(tracebp(:, 2:4), [],1);
            ymami = max([maxamppos; -minampneg], [], 1);
...
%             patch(ax,xarea,yarea,'k','Facealpha',0.15,'edgecolor','none');
            ...
            if lohf<1 || hihf <1
                loperd = round(1/lohf);
                hiperd = round(1/hihf);
                text(ax,0.05,0.9,strcat(num2str(hiperd),'-',num2str(loperd),{' s'}),'fontsize',...
                     9,'unit',9,'unit','normalized','Horizontalalignment','left');
            else
                text(ax,0.05,0.9,strcat(num2str(lohf),'-',num2str(hihf),{' Hz'}),'fontsize',9,...
...
%             ax.YTick = [];
        ...
        %%%%%%%%%%%%%%%%% PLOT 3, amp variation with octave passband %%%%%%%%%%%%%%%%%%%%
        f.fig = figure;
        f.fig.Renderer = 'painters';
        ax = gca;
        hold(ax,'on');
...
%         ymmat = ymmat/ max(max(ymmat));   % normalized by the global max
        ...
        text(ax,0.05,0.95,FLAG,'fontsize',12,'unit','normalized','Horizontalalignment','left');
        text(ax,0.2,0.95,fam,'fontsize',12,'unit','normalized','Horizontalalignment','left');
        text(ax,0.95,0.95,strcat(num2str(round(is+idiff/sps)),{' s, on '},num2str(date)),...
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         
%         %%%%%%%%%%%%%%%%% PLOT 6, phase of cross spectrum %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         % NOT very helpful
%         %use the broadband trace of the detecting window, so the length is fixed
%         %pre-aligned 
%         x = tracebbf(winlen*(ndet-1)+1:winlen*ndet, 1:4);
