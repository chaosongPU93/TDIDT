...
% lenpool = ['144';
%           ];
% if ismember(fam,lenpool,'rows')
%     before = 0.5*templen;
%     after = 1.5*templen-1;
% else
%     before = 0.25*templen;
%     after = 1.75*templen-1;
% end
before = 1*templen;
after = 1*templen-1;
switch fam
...
size(stacktmp, 2)/2
(indzero - size(stacktmp, 2)/2)/sps
indst = indzero - templen/2 -extra
inded = indzero + templen/2 +extra-1
if indst >=1 &&  inded <= 2*templen+2*extra
    disp('enough');
    windata = [];
...
else    % need to redo the stacking
    disp(indst -1);
    disp(2*templen+2*extra - inded);
    disp('Adjusting length now');
    if indst < 1
        before = before + (1- indst);
        after = after - (1- indst);
    elseif inded > 2*templen+2*extra
        before = before + (2*templen+2*extra-inded);
        after = after - (2*templen+2*extra-inded);
    end
    ...
    nstack = 0;
    nLFE = 0; 
    fprintf('Redoing Part 1 due to a bad window segmentation \n');
