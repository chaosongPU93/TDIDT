...
hftime = sortrows(hftime, [13, 14]);
lftime = sortrows(lftime, [13, 14]);
...
%     i=46;
    disp(trange(i,:));
    indhf = find(hftime(:,13)==trange(i,1) & hftime(:,14)>=trange(i,2) & ...
                 hftime(:,14)<=trange(i,3));
    mighf = hftime(indhf,:);
    
    indlf = find(lftime(:,13)==trange(i,1) & lftime(:,14)>=trange(i,2) & ...
    ...
    for iang = 1: length(angle)
        ...
        [fitobj,gof,~] = fit(mighfdum(:,14)/3600, mighfdum(:,1),fttpfree,'Robust','Bisquare',...
                                  'StartPoint',[1 1]);
...
%     set(f.ax(6), 'position', [ 0.5, 0.078, 0.32, 0.22]);
    ...
    scatter(f.ax(1),mighf(:,1),mighf(:,2), 20, mighf(:,14)/3600, 'filled','o');  %, 'MarkerEdgeColor', 'w')
    colormap(f.ax(1),'jet');
...
%     c.Label.String = strcat(num2str(trange(i,1)),' of HF',' (hr)');
    ...
    scatter(f.ax(2),miglf(:,1),miglf(:,2), 20, miglf(:,14)/3600, 'filled','o');  %, 'MarkerEdgeColor', 'w')
    colormap(f.ax(2),'jet');
...
%     c.Label.String = strcat(num2str(trange(i,1)),' of LF',' (hr)');
    ...
    hold(f.ax(3),'on');
    f.ax(3).FontSize = 9;
    scatter(f.ax(3),mighfdum(:,14)/3600,mighfdum(:,1),20,[0.6 0.6 0.6],'filled','o',...
            'MarkerEdgeColor','k');    
    scatter(f.ax(3),miglfdum(:,14)/3600,miglfdum(:,1),20,[0.6 1 1],'filled','o',...
            'MarkerEdgeColor','k');
    % create fit object
        
    [fitobjhfprop,gofhfprop,outphfprop] = fit(mighfdum2(:,14)/3600, mighfdum2(:,1),fttpfree,'Robust',...
...
%     fitproplf = slopeprophf*miglfdum(:,end)/3600+intcptproplf;
    ...
    [fitobjlfprop,goflfprop,outplfprop] = fit(miglfdum2(:,14)/3600, miglfdum2(:,1),fttpfix,'Robust',...
