% testwletccsharpness.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is to visually check the sharpness of the CC function between the 
% LFE templates at different stations.
%
%
% Chao Song, chaosong@princeton.edu
% First created date:   2024/02/19
...
%%
[ccwlet(:,1),lags]=xcorr(greenf(:,1),greenf(:,2),mshiftadd,'coeff'); %shift STAtmp(2,:) to right for positive values
ccwlet(:,2)=xcorr(greenf(:,1),greenf(:,3),mshiftadd,'coeff'); %shift STAtmp(3,:) to right for positive values
ccwlet(:,3)=xcorr(greenf(:,2),greenf(:,3),mshiftadd,'coeff');
ccwletn(:,1)=ccwlet(:,1)+1-max(ccwlet(:,1));
ccwletn(:,2)=ccwlet(:,2)+1-max(ccwlet(:,2));
ccwletn(:,3)=ccwlet(:,3)+1-max(ccwlet(:,3));

%%
f=initfig(6,7,2,1);
ax=f.ax(1); hold(ax,'on'); ax.Box='on'; grid(ax,'on');
plot(ax,lags,ccwlet(:,1),'r-','linew',1.5);
plot(ax,lags,ccwlet(:,2),'b-','linew',1.5);
plot(ax,lags,ccwlet(:,3),'k-','linew',1.5);
legend(ax,'sta 1--2','sta 1--3','sta 2--3');
xlabel(ax,'lag (samples)');
ylabel(ax,'CC function');
ax.XAxis.MinorTick='on';
ax=f.ax(2); hold(ax,'on'); ax.Box='on'; grid(ax,'on');
plot(ax,lags,ccwletn(:,1),'r-','linew',1.5);
plot(ax,lags,ccwletn(:,2),'b-','linew',1.5);
plot(ax,lags,ccwletn(:,3),'k-','linew',1.5);
ylabel(ax,'Normalized CC function');
ax.XAxis.MinorTick='on';
