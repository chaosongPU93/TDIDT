...
pltxran = [0.05 0.95]; pltyran = [0.1 0.9];
...
longticks(ax,4);  % make ticks outward
nolabels(ax,1); % do not shown tick labels
...
ylabel(ax,'Amplitude','FontSize',12);
yyaxis(ax,'right');
plot(ax,ircc,rccort,'o','color',[.5 .5 .5],'markersize',2);
ylabel(ax,'Running CC','FontSize',12);
ylim(ax,[-1 1]);
...
ylabel(ax,'Amplitude','FontSize',12);
yyaxis(ax,'right');
plot(ax,ircc,rccvert,'o','color',[.5 .5 .5],'markersize',2);
ylabel(ax,'Running CC','FontSize',12);
ylim(ax,[-1 1]);
% shrink(ax,2,1); % shrink the width of subpanel by a factor of 2, height not changed
% tmp = getpos(f.ax(1));  % get the position of panel 1, same as f.ax(1).Position
% ax.Position(1) = tmp(1);  % align the x position
% serre(f.ax,1/2,'down'); %Brings subplot panels closer together, down, by a fraction of 1/2
...
% if sps==40
%   zerocs = [1198 1199 1199];
% elseif sps==80
%   zerocs = [2393 2395 2394];
% elseif sps==100
%   zerocs = [2990 2993 2992];
% end
...
for ista = 1:nsta
  ...
  text(size(stawlet,1)*0.45, 0.3*ista, stas(ista,:));
end
...
%   [f] = plt_wletsig_timefreq(wletbb,sigbb,wlet,sig,[],[],sps,[lowlet hiwlet],[losig hisig]);
  ...
  %carefully tweak the axis postion
  axall = [];
  axall = [f.ax(1) f.ax(2) f.ax(3) f.ax(4) f.ax(5) f.ax(6)];
  serre(axall,1/3,'down'); clear axall
  axall = [f.ax(1) f.ax(2)];
  serre(axall,1/2,'down'); clear axall
  axall = [f.ax(3) f.ax(4)];
  serre(axall,1/2,'down'); clear axall
  axall = [f.ax(5) f.ax(6)];
  serre(axall,1/2,'down'); clear axall
  
  %if you want to shrink the time axis to bring all axes to the true ratio
  shrkflg = 0;
  if shrkflg
    axpos = getpos(f.ax(1),1);
    shrink(f.ax(1),lmax/lwlet,1);
    f.ax(1).Position(1)=axpos;
    shrink(f.ax(2),lmax/lwlet,1);
    f.ax(2).Position(1)=axpos;
    shrink(f.ax(3),lmax/lsig,1);
    f.ax(3).Position(1)=axpos;
    shrink(f.ax(4),lmax/lsig,1);
    f.ax(4).Position(1)=axpos;
    shrink(f.ax(5),lmax/lnoi,1);
    f.ax(5).Position(1)=axpos;
    shrink(f.ax(6),lmax/lnoi,1);
    f.ax(6).Position(1)=axpos;
  end
...
% figure
% plot(ircc,rcc,'-','color',[.5 .5 .5],'LineWidth',1); hold on
% plot(ircccon,rcccon,'k-','LineWidth',1);
% plot(irccc123,rccc123,'b-','LineWidth',1);
% legend('Original rcc','2-sta convolution rcc','3-sta convolution rcc','Location','south');

%% Convolution between templates
% %%%Unlike the convolution between signal and template, this givesonly 3 traces, because temp1*temp2
% %%%is the same as temp2*temp1.
% w1w2 = conv(stawlet(:,1),stawlet(:,2),'full'); % the order doesn't matter
% w1w2 = w1w2(1+round(statwlet(2)/dt):lwlet+round(statwlet(2)/dt));  % cut accordingly
% w1w3 = conv(stawlet(:,1),stawlet(:,3),'full'); % the order doesn't matter
% w1w3 = w1w3(1+round(statwlet(3)/dt):lwlet+round(statwlet(3)/dt));  % cut accordingly
% w2w3 = conv(stawlet(:,2),stawlet(:,3),'full'); % the order doesn't matter
% w2w3 = w2w3(1+round(statwlet(3)/dt):lwlet+round(statwlet(3)/dt));  % cut accordingly
% 
% figure
% subplot(311)
...
% plot(stawlet(:,2),'b-'); 
% plot(w1w2,'k-');
% text(0.95,0.85,'Temp1 conv Temp2','Units','normalized','HorizontalAlignment',...
%   'right','FontSize',12);
% xlim([0 lwlet]);
% yran = 1.5*[-max(max(abs(stawlet))) max(max(abs(stawlet)))];
% ylim(yran);
% 
% subplot(312)
% plot(stawlet(:,1),'r-'); hold on
% plot(stawlet(:,3),'b-');
% plot(w1w3,'k-');
% text(0.95,0.85,'Temp1 conv Temp3','Units','normalized','HorizontalAlignment',...
%   'right','FontSize',12);
% xlim([0 lwlet]);
% ylim(yran);
% 
...
% plot(stawlet(:,3),'b-');
% plot(w2w3,'k-');
% text(0.95,0.85,'Temp2 conv Temp3','Units','normalized','HorizontalAlignment',...
%   'right','FontSize',12);
% xlim([0 lwlet]);
% ylim(yran);
...
% %%%scatter of offsets, normalized to 40 Hz, and account for prealignment offset 
% impindept = impindep;
...
% scatter(gca,off1i(2),off1i(3),20,'MarkerEdgeColor','k');
...
impindept = impindep;
impindept(:,7:8) = impindept(:,7:8)+repmat([off1i(2) off1i(3)],size(impindept,1),1);
[f] = plt_decon_imp_scatter_space(impindept,xran,yran,cran,sps,'interpchao',0);
...
% xran = [(-loff_max+off1i(2))/spsscale (loff_max+off1i(2))/spsscale]; 
% yran = [(-loff_max+off1i(3))/spsscale (loff_max+off1i(3))/spsscale];
% cran = [0 lsig/spsscale];
% [f] = plt_decon_imp_scatter(ampitt,xran,yran,cran,sps,spsscale);
% scatter(gca,off1i(2),off1i(3),20,'MarkerEdgeColor','k');
% title(gca,sprintf('Joint, using data of %d Hz, normalized to 40 Hz',sps));
...
ampitt = ampit;
ampitt(:,7:8) = ampitt(:,7:8)+repmat([off1i(2) off1i(3)],size(ampitt,1),1);
[f] = plt_decon_imp_scatter_space(ampitt,xran,yran,cran,sps,'interpchao',0);
