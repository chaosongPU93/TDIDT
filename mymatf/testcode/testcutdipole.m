...
%
% -- This part of the code is to obtain the zero-crossing time, min-to-max time lag of the main 
%   dipole and the full duration of the dipole, of our best example detections and the templates.
%   This part would save the segment of the main dipole to file.
% -- Another following code 'testsuperposition' will read in the seismogram, do stretching or
%   shrinking if necessary, and then try to superimpose several cycles of templates that are
%   separated in time in some manners, to see if the example detection could be simulated by this
...
%%% choose the best example at ~900s in 2004199 from fam 243
nd = 7;
...
if iup == 1
    ...
    fname1 = strcat(rstpath,'/MAPS/pj21mapallup_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
        num2str(winlen/sps),'s',num2str(sps),'sps', '4add');
    fname2 = strcat(rstpath,'/MAPS/pj21mapallup_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
        num2str(winlen/sps),'s',num2str(sps),'sps', '3add');
end
...
% set(f.ax(9), 'position', [ 0.5, 0.05, 0.45, 0.21]);     % for spectrum
...
%         'fontsize',8,'unit','normalized','Horizontalalignment','right');
    text(ax,0.75,0.75,sprintf('%.2f/%.2f=%.2f',tlag(1),tdura(1),trat(1)),...
        'fontsize',8,'unit','normalized','Horizontalalignment','right','color','r');
    text(ax,0.75,0.3,sprintf('%.2f/%.2f=%.2f',tlag(2),tdura(2),trat(2)),...
...
iend = nwinlen+sps;
tracebbali = [];
tracebbali(:, 1:2) = tracebb(istart: iend, 1:2);
tracebbali(:, 3) = tracebb(istart-round(off12add): iend-round(off12add), 3);
tracebbali(:, 4) = tracebb(istart-round(off13add): iend-round(off13add), 4);
...
rstbest = [isolfsort rstbest];


%% get the duration and time lag of the template
...
%% cut out the seismogram of the main arrival from start to end for the example and template
%cut out the desired seismogram

%save to file
fid = fopen(strcat(rstpath, '/MAPS/bestexample_',fam,'_',num2str(date),'_',num2str(nwinlen/20),...
    's',num2str(20),'sps'),'w+');
fprintf(fid,'%.6f %.6f %.6f \n',tracebest');    % 3 col

fid = fopen(strcat(rstpath, '/MAPS/templateseg_',fam,'_',num2str(wlen/sps),...
    's',num2str(sps),'sps'),'w+');
fprintf(fid,'%.6f %.6f %.6f \n',tracetemp');    % 3 col
