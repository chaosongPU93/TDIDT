function mergeday_lf_PGC(fam,fcflag,convflag,iup)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     nd=1;
    ...
    if size(filtlf,1) > 2   % which means containing the effects of additional stations
        rstday(:, 2) = rstday(:, 2) - filtlf(2);  % the original correction contains all seven with diff algorithm
        rstday(:, 3) = rstday(:, 3) - filtlf(3);  % ranges with order
    else
        rstday(:, 2) = rstday(:, 2) - filtlf(1);  % since the total offset 12 contains filtering effect, so substract to correct
        rstday(:, 3) = rstday(:, 3) - filtlf(2);
    end
