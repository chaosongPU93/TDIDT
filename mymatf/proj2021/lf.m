...
%
%   2021/03/18
%   -- a mutant version of 'lf_LZBtrio_detection_resp_check_v2.m', in order 
...
%         n=1;
        ...
        if xcmaxAVEn(n)<xcmaxAVEnmin || abs(loopoff(n))>loopoffmax ...
            ...
            if abs(iSTA12bang-imaxSTA12n) <= loopoffmax && ...  %not sure if these 3 lines are satisfied automatically ...
               ...
               3*xcmaxAVEnmin && abs(tmp12) <= mshift-cyclskip && ... 
               abs(tmp13) <= mshift-cyclskip && abs(tmp32) <= mshift-cyclskip
...
%                     %%%%%%%For comparison, end
                      ...
                      match(nin+1,2)=0;
                        ...
                        STAamp(nin+1,2)=prctile(abs(STAopt(2,istart-imaxSTA12wr:...
                                                             iend-imaxSTA12wr)),80);
                        STAamp(nin+1,3)=prctile(abs(STAopt(3,istart-imaxSTA13wr:...
                                                             iend-imaxSTA13wr)),80);
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
                        ...
                        xmaxSTA12ntmp(n)=mshift+1; xmaxSTA13ntmp(n)=mshift+1; 
                        xmaxSTA32ntmp(n)=mshift+1;
                    end                
                else
                    % 20 == mshift+1
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21trace1d_',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21trace1dbb_',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
                '_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
...
%     colnum = [7 1 4];    % col numbers that are timing and cc coefs
    if nstanew == 3
