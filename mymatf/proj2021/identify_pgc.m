...
%
% this script reads in the raw detection file from each fam of interest, so
% the target may be imaged by different fams, i.e. there will be duplicates
%
%
% Note 2021/06/10:
...
candidate = goodlfdetections(FLAG,fam,nday);


%% the sequential number of qualified detections from 'way 3'
...
%         plot(ax,[is+1/hi is+1/lo],[-0.8*yma -0.8*yma],'k','linew',3)
        ...
        text(ax,0.02,0.8,strcat({'Aligned'}),'fontsize',9,'unit',...
             'normalized','Horizontalalignment','left'); 
...
%%
rstsave_sort = sortrows(rstsave, [1 8 2]);    
[rstsave_nodou,~,~] = RemoveDoubleCounting(rstsave_sort,1.1,nfam,[8 5]);
for ifam = 1: nfam
    fam = fampool(ifam, :);
    num(ifam,1) = sum(rstsave_nodou(:,end)==str2double(fam));
    
end

%index of high-quality detections out of the rstsave_nodou checked by eye [81 in total]
indgood = [27, 31, 36, 38, 41, 43, 58, 61, 64, 67, 78]; %, 77
rstgood = rstsave_nodou(indgood, :);    % remember to make a hard copy of the information!
