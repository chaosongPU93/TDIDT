% function identify_counter_pgc_hf(fam,iup)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% THIS is to find the counter example of 11 (12) target detections where the
% full duration seems longer than the templates, which is identified
% in 'identify_pgc.m', i.e., we want to find the cases in which the duration
% is relatively shorter, from the merged catalog of all fams of interest (HF).
%
%   different from 'identify_counter_pgc.m', this code is to go through the HF
%   catalog to find the detections that are at the roughly the same region of 
%   the target detections. (We might be able to loose the constraint of being
%   in the same date, as many people might agree that the attenuation might have
%   a sharp boundary in space, but is invariant within the same ETS episode)
%
...
% First created date:   2021/07/19
% Last modified date:   2021/07/19
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
SUFFIXhf = strcat('up.hf.time.',num2str(winlensechf),'_',num2str(winlenseclf),'.',num2str(loopoffmax),...
                  '.',num2str(xcmaxAVEnmin));
SUFFIXlf = strcat('lf.time.',num2str(winlensechf),'_',num2str(winlenseclf),'.',num2str(loopoffmax),...
                  '.',num2str(xcmaxAVEnmin));
...
for i = 6: size(rstgood, 1)
    ...
    if i == 1 || i == 4
        ...
        y0 = rstgood(i, 2);
        rad = 2.5;
    elseif i == 2 || i == 6 || i == 7 || i == 9
        % boundary of a circular region around the (0,0)
        x0 = 0;
        y0 = 0;
        rad = 5;
    elseif i == 3 || i == 5 || i == 8 || i == 10 || i == 11
        % boundary of a circular region around the target 5, the best one
        x0 = rstgood(5, 1);
        y0 = rstgood(5, 2);
        rad = 2.5;
    end
    ...
    % events inside bound
    [iin,ion] = inpolygon(hftime(:,1),hftime(:,2),bnd(:,1),bnd(:,2));
    isinbnd = iin | ion;
    ...
    % events inside bound and also occur in the same ETS period, i.e. same year
    hfbndtime = hfbnd(floor(hfbnd(:,14)/1000) == year, :);
%     hfbndtime = hfbnd;

    %%%%%%%%%%%%%%%%
    % here we may have lots of detections, but we still need cases to be relatively isolated, so
    % just we did in 'identify_pgc.m', we obtain the energy ratio, and choose the first few percent
    erat(:, 1) = hfbndtime(:,21)./hfbndtime(:,22);
    erat(:, 2) = hfbndtime(:,21)./hfbndtime(:,23);
...
%     perc = 0.1;
%     nuse = round(size(erat_sort,1)* perc);
%     indobj = indori(1: nuse);
%     hfobj = hfbndtime(indobj,:);
    ...
    if isempty(hfobj)
        ...
        for j = 1: size(hfobj,1)
            ...
            fname2 = strcat(rstpath, '/MAPS/pj21trace1dup_',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
            ...
            fname = strcat(rstpath, '/MAPS/pj21trace1dbbup_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
            ...
            PREFIX = strcat(fam,'.lf',num2str(lolf),'-',num2str(hilf),'.hf',num2str(lohf),'-',num2str(hihf),...
                '.hw',num2str(winlensechf),'.lw',num2str(winlenseclf),'.lo',num2str(lofflf),'.ccm', ...
            ...
            if strcmp(WAVE, 'day')
                ...
                %             nwinlen = 64*sps;
                [~,ind] = min(abs(hfobj(j, 16)-trace1df(:,1)));
                ...
                trace1d(:,3) = trace1df(ind-nwinlen/2 - round(off12): ...
                    ind+nwinlen/2-1 - round(off12), 3);
                trace1d(:,4) = trace1df(ind-nwinlen/2 - round(off13): ...
                    ind+nwinlen/2-1 - round(off13), 4);
                trace = trace1d;
                
                [~,ind] = min(abs(hfobj(j, 16)-trace1dbbf(:,1)));
                % here, instead of using 'idiff', we use idiff+ cncntr/2 as a rough estimate to 0-crossing
                ind = ind+cncntr/2;
                trace1dbb = [];
                % +/-1 sec for alignment in different passbands
                trace1dbb(:,1:2) = trace1dbbf(ind-nwinlen/2-sps: ind+nwinlen/2+sps-1, 1:2);
                trace1dbb(:,3) = trace1dbbf(ind-nwinlen/2-sps - round(off12): ...
                    ind+nwinlen/2+sps-1 - round(off12), 3);
                trace1dbb(:,4) = trace1dbbf(ind-nwinlen/2-sps - round(off13): ...
                    ind+nwinlen/2+sps-1 - round(off13), 4);
                tracebb = trace1dbb;
...
%                 2:4), [],1);
            ...
            [maxamppos,indmax] = max(trace(max(nwinlen/2-0.5*fixlen,1): ...
                min(nwinlen/2+0.5*fixlen-1,nwinlen),2:4), [],1);
            [minampneg,indmin] = min(trace(max(nwinlen/2-0.5*fixlen,1): ...
                min(nwinlen/2+0.5*fixlen-1,nwinlen),2:4), [],1);            
            ...
            text(ax,0.25, 0.9, num2str(off12),'fontsize',8,'color','b','unit','normalized',...
                'Horizontalalignment','left');
            text(ax,0.75,0.9,sprintf('%.4f',hfobj(j, 17)),'fontsize',8,'unit','normalized',...
                'Horizontalalignment','right');
            % if it is also confirmed to be real by the 4th station
            %         flagconf = [];
            for jj = 1: nstasnew
                if hfobj(j, 43+jj-1) == 1
            ...
            % NOTE: here the resolution of off12add is subject to iup, and may be diff from off12!
            ...
            for ifreq = 1: length(octavef)-1
                ...
                plot(ax,tracebp(:, 1),tracebp(:, 3),'b','linew',0.5);
                plot(ax,tracebp(:, 1),tracebp(:, 4),'k','linew',0.5);
%                 plot(ax,[is ien],[medfull medfull],'--','color',[0.6 0.6 0.6],'linew', 1.5);
