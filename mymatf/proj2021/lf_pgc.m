...
%
%   2021/03/18
%   -- a mutant version of 'lf_PGCtrio_detection_resp_check_v2.m', in order 
%       to find isolated LF arrivals like that in Figure 1 in our first paper
%       found by eyes using the PGC trio
%   -- basically the bulk of the code is the same
%   -- previously i was using my eyes to find them and plot them with the code
%       'lf_PGCtrio_detection_resample_forplot_v2.m' by noting their dates and
%       sequential numbers
%
%   2021/06/30
%   -- use 'GetRotsPGC' for fams using PGC trio other than 002; use 'GetRots'
%
%        
% 
%
% Modified by Chao Song, chaosong@princeton.edu
% First created date:   2021/03/20
...
% if isequal(fam,'243')
%     tempoffs = round([1241 1300 1270]/2);
% else
%     tempoffs = round([1241 1300 1270]/2);
% end
...
%         n=1;
        ...
        if xcmaxAVEn(n)<xcmaxAVEnmin || abs(loopoff(n))>loopoffmax ...
            ...
            for iSTA12 = max(1,imaxSTA12n-floor(loopoffmax+1)): ...
                         min(imaxSTA12n+floor(loopoffmax+1),2*mshift+1)
                for iSTA13 = max(1,imaxSTA13n-floor(loopoffmax+1)): ...
                             min(imaxSTA13n+floor(loopoffmax+1),2*mshift+1)
            ...
            if abs(iSTA12bang-imaxSTA12n) <= loopoffmax && ...  %not sure if these 3 lines are satisfied automatically ...
               ...
               sumsSTA12n(n,iSTA12bang)+sumsSTA13n(n,iSTA13bang)+...
                    sumsSTA32n(n,iSTA32bang) >= 3*xcmaxAVEnmin && ...  % xcmaxAVEnmin, predetermined
               abs(tmp12) <= mshift-cyclskip && abs(tmp13) <= mshift-cyclskip && ...
               abs(tmp32) <= mshift-cyclskip
               ...
               %%% == xcmaxconprev according to codes
                ...
                xcmaxAVEnbang(nin+1)=(sumsSTA12n(n,iSTA12bang)+sumsSTA13n(n,iSTA13bang)+...
...
% is = STA1file(winlen*(nin-1)+1,1);
% ien= STA1file(winlen*nin,1);
% issamp=round(is*sps);
...
% [maxhil loc]=max(medhil(issamp+STA32file(nin,3):issamp+STA32file(nin,3)+cncntr-1));
% maxloc=issamp+STA32file(nt,3)+loc-1;
% icheck=maxloc;
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
    ...
    fid = fopen([rstpath,'/MAPS/pj21map',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21trace_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21trace1d_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21trace1dbb_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
...
%     end
    ...
    fid = fopen([rstpath,'/MAPS/pj21mapadd',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21traceadd_',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 3 start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%% Write results into files
    fid = fopen([rstpath,'/MAPS/pj21mapall',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                 num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'add'],'w+');
    if nstanew == 3
...
%         fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %7.2f %10.3e %10.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %d %d %d %d %.4f %.4f %.4f %.4f %d %d %d %d %.3f %.3f %.3f %.3f %6.2f %6.2f %6.2f \n',allrst_new(1:nin_new,:)');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21traceall_',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
                '_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'add'),'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/pj21traceallbb_',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
                '_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'add'),'w+');
