function pltaddcheck_lf_PGC(fam,hitallow,fcflag,convflag,iup)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 3-sta trio PGC, especially for fam 243
%
% reference to plotaddcheck_hf_PGC.m and plotaddcheck_hf_LZB.m, ONLY for fam 243
%
%
%
% Modified by Chao Song, chaosong@princeton.edu
% First created date:   2020/02/16
...
% of templates
if isequal(fam,'002')
    [timoffrot,~,~,~] = GetDays(fam,freqflag);
...
% hitallow = 150;     % max number of hit allowed to iniate the matrix
...
if size(filtlf,1) > 2   % which means containing the effects of additional stations
    STA12off = STA12off - filtlf(2);  % the original correction contains all seven with diff algorithm
    STA13off = STA13off - filtlf(3);  % ranges with order
else
    STA12off = STA12off - filtlf(1);  % since the total offset 12 contains filtering effect, so substract to correct
...
if isequal(fam,'002')
    ...
    CATA = 'new';
    [PERMROTS, POLROTS] = GetRotsCommon(FLAG,fam,CATA,datapath,0,0);
    reftfam = PERMROTS(1,4);     % reftime is the reference time at the 1st station,depends on the choice of 1st station
...
reftref = refperm(1,4);     % reftime is the reference time at the 1st station,depends on the choice of 1st station
refperm(:,4) = refperm(:,4)-reftref;    % to make sure that 1st station, PGC is 0
...
conv(1) = POLROTS(1,4) - refpol(1,4);   % station 2, SSIB
conv(2) = POLROTS(2,4) - refpol(2,4);    % station 3, SILB
if convflag
...
stdtol = 0.2;   % tolerence of std
countmin = 2;   % min counts allowed in the vicinity
[STA14meansec,STA14stdsec,STA134offsec,STA124offsec,STA134off,STA124off,STA14count,STA14tbg,...
    STA14tct,STA14tdate,STA14cc] = Addcheck_meanstd(1,filtlf,countmat14,offmat14,tbgmat14,...
...
[STA15meansec,STA15stdsec,STA135offsec,STA125offsec,STA135off,STA125off,STA15count,STA15tbg,...
    STA15tct,STA15tdate,STA15cc] = Addcheck_meanstd(2,filtlf,countmat15,offmat15,tbgmat15,...
...
[STA16meansec,STA16stdsec,STA136offsec,STA126offsec,STA136off,STA126off,STA16count,STA16tbg,...
    STA16tct,STA16tdate,STA16cc] = Addcheck_meanstd(3,filtlf,countmat16,offmat16,tbgmat16,...
...
[STA17meansec,STA17stdsec,STA137offsec,STA127offsec,STA137off,STA127off,STA17count,STA17tbg,...
    STA17tct,STA17tdate,STA17cc] = Addcheck_meanstd(4,filtlf,countmat17,offmat17,tbgmat17,...
...
if convflag == 0 
    ...
    print('-depsc',strcat(rstpath,'/FIGS/pj21',PREFIX,'_','AddOffCheck', '_',num2str(lo),'-',...
...
%     fid = fopen(strcat(rstpath, '/MAPS/pj21countori_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',...
...
if ~isempty(famarr)
    savefile14 = [STA124off STA134off STA124offsec STA134offsec STA14meansec STA14stdsec STA14count ...
        famarr];
    fid = fopen(strcat(rstpath, '/MAPS/pj21countadd_', stasnew(1,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d %d \n',savefile14');  % 8 cols
    fclose(fid);
    fid = fopen(strcat(rstpath, '/MAPS/pj21timeadd_', stasnew(1,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f %d \n',STA14time');  % 9 cols
    fclose(fid);
...
if ~isempty(famarr)
    savefile15 = [STA125off STA135off STA125offsec STA135offsec STA15meansec STA15stdsec STA15count ...
        famarr];
    fid = fopen(strcat(rstpath, '/MAPS/pj21countadd_', stasnew(2,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d %d \n',savefile15');
    fclose(fid);
    fid = fopen(strcat(rstpath, '/MAPS/pj21timeadd_', stasnew(2,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f %d \n',STA15time');
    fclose(fid);
...
if ~isempty(famarr)
    savefile16 = [STA126off STA136off STA126offsec STA136offsec STA16meansec STA16stdsec STA16count ...
        famarr];
    fid = fopen(strcat(rstpath, '/MAPS/pj21countadd_', stasnew(3,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d %d \n',savefile16');
    fclose(fid);
    fid = fopen(strcat(rstpath, '/MAPS/pj21timeadd_', stasnew(3,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f %d \n',STA16time');
    fclose(fid);
...
if ~isempty(famarr)
    savefile17 = [STA127off STA137off STA127offsec STA137offsec STA17meansec STA17stdsec STA17count ...
        famarr];
    fid = fopen(strcat(rstpath, '/MAPS/pj21countadd_', stasnew(4,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d %d \n',savefile17');
    fclose(fid);
    fid = fopen(strcat(rstpath, '/MAPS/pj21timeadd_', stasnew(4,:), '_',PREFIX,'_',num2str(lo),'-',...
        num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),...
        'w+');
    fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f %d \n',STA17time');
    fclose(fid);
