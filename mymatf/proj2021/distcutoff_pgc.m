...
%
%   version for fams using PGC trio, currently for our purpose, it is not
...
SUFFIXhf = strcat('up.hf.time.',num2str(winlenhf),'_',num2str(winlenlf),'.',num2str(lofflf),...
                  '.',num2str(ccminlf));
fname = strcat(rstpath, '/evtloc.pgcfam.pj21.',SUFFIXhf);
...
SUFFIXlf = strcat('lf.time.',num2str(winlenhf),'_',num2str(winlenlf),'.',num2str(lofflf),...
                  '.',num2str(ccminlf));
fname = strcat(rstpath, '/evtloc.pgcfam.pj21.',SUFFIXlf);
...
loccont = [
           ...
           -123.432833 48.368167 37.9200;   % 253
           -123.378333 48.282000 37.7200;   % 036
           -123.081500 48.397667 42.9300;   % 251
           ];

relacont = loccont;
% [dx, dy] = absloc2relaloc(loccont(:,1),loccont(:,2),loccont(2,1),loccont(2,2));
[dx, dy] = absloc2relaloc(loccont(:,1),loccont(:,2),-123.585000, 48.436667);
relacont(:,1) = dx;
relacont(:,2) = dy;       
...
% relative coordinates to fam 002
[dx, dy] = absloc2relaloc(hfrelatimecut(:,4),hfrelatimecut(:,5),-123.585000, 48.436667);
hfrelatimecut = [dx dy hfrelatimecut];     % now increases to 58 cols

[dx, dy] = absloc2relaloc(lfrelatimecut(:,4),lfrelatimecut(:,5),-123.585000, 48.436667);
...
fid = fopen(strcat(rstpath, '/evtloc.pgcfam.pj21.',num2str(distmaxhf),'kmdcut.',SUFFIXhf),'w+');
...
fid = fopen(strcat(rstpath, '/evtloc.pgcfam.pj21.',num2str(distmaxlf),'kmdcut.',SUFFIXlf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %6.2f %6.2f %.4f %.4f %d %d %9.1f %10.3f %8.3f %7.2f %10.3e %7.3f %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %7.3f %7.3f %7.3f %7.3f %7.3f %5.2f %5.2f %7.3f %7.3f %7.3f %6.2f %6.2f %6.2f %d %d %d %d %.4f %.4f %.4f %.4f %d %d %d %d %.3f %.3f %.3f %.3f \n',...
        lfrelatimecut');
fclose(fid);
...
[dx, dy] = absloc2relaloc(hfrelatime(:,4),hfrelatime(:,5),-123.585000, 48.436667);
hfrelatimenocut = [dx dy hfrelatime];

[dx, dy] = absloc2relaloc(lfrelatime(:,4),lfrelatime(:,5),-123.585000, 48.436667);
lfrelatimenocut = [dx dy lfrelatime];


% format is the same as above
fid = fopen(strcat(rstpath, '/evtloc.pgcfam.pj21.nodcut.',SUFFIXhf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %6.2f %6.2f %.4f %.4f %d %d %9.1f %10.3f %8.3f %7.2f %10.3e %7.3f %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %7.3f %7.3f %7.3f %7.3f %7.3f %5.2f %5.2f %7.3f %7.3f %7.3f %6.2f %6.2f %6.2f %d %d %d %d %.4f %.4f %.4f %.4f %d %d %d %d %.3f %.3f %.3f %.3f \n',...
        hfrelatimenocut');
fclose(fid);

fid = fopen(strcat(rstpath, '/evtloc.pgcfam.pj21.nodcut.',SUFFIXlf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %6.2f %6.2f %.4f %.4f %d %d %9.1f %10.3f %8.3f %7.2f %10.3e %7.3f %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %7.3f %7.3f %7.3f %7.3f %7.3f %5.2f %5.2f %7.3f %7.3f %7.3f %6.2f %6.2f %6.2f %d %d %d %d %.4f %.4f %.4f %.4f %d %d %d %d %.3f %.3f %.3f %.3f \n',...
        lfrelatimenocut');
fclose(fid);
