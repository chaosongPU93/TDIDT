...
% an appropriate 
%
%   Different from 'identify.m' which check the raw catalogs that are separated
%   by each day and each fam containing duplicates from different fams, this
%   code tries to check the merged catalog of all fams that could be separated
%   each day of merged fams. Ideally should be more efficient than 'identify.m'
%
% Note 2021/06/10:
...
%         % by col 1-6; we also expect the side lobe amplitude to be small, j.e. long-term average to be
...
%         if sum(hilwid(j,1:6) <= tolhilwid.* refhilwid(end, 1:6)) >= 4 && ...
%            sum(hilwid(j,7:end) <= tolhilwid.* refhilwid(end, 7:end)) >= 4 && ...
%            sum(medlmaxarat <= tolarat) == 4 && ...
%            sum(medlminarat <= tolarat) == 4
%        
%             rstsave = [rstsave; rstobj(j, :)];
%             hold(f.ax(1),'on');
