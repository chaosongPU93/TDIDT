% function [ROTS]=calc_rots_noresp_pgc(fam,lo,hi,sps,bef,aft,splitoff,staoff,mainsta)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This is the script to get the shear-splitting parameter and rotation
% angles from scratch, following the method illustrated in NOTES. 
% Deal with fam 002 in PGC trio ONLY !!! As the old version of LFE 
% catalogs is separated for each fam, so make sure you link the correct
% fam
%
%   mutant version from 'calc_rots_noresp.m' for any fam using PGC trio. 
%   This is necessary if the LFE catalogs used are the older version. 
%   Use the original seperated Bostock's
...
freqflag='hf';
[timoffrot,~,bostname,tempoffs] = GetDays(fam,freqflag);
...
subplot(2,1,1)  % plot E component 
for ista = 1:nsta
    plot(stackE(ista,:)*10 + 1* ista); hold on
    text(50,1*ista,stas(ista,:));
end

subplot(2,1,2)  % plot N component
for ista = 1:nsta
    plot(stackN(ista,:)*10 + 1* ista); hold on
    text(50,1*ista,stas(ista,:));
end
...
%%
figure
plot(stackE(2,:)*10 + 1, 'b-'); hold on
plot(stackN(2,:)*10 + 2, 'r-');
text(1050,0.9,'SSIB E','FontSize',10);
text(1050,1.9,'SSIB N','FontSize',10);
text(0.05,0.95,'Velocity','Units','normalized','FontSize',12);
text(0.95,0.95,sprintf('Bandpassed by %.1f-%.1f Hz',lo,hi),'Units','normalized','FontSize',12,...
  'HorizontalAlignment','right');
ax = gca;
plot([1268 1268], ax.YLim, 'b--');
plot([1278 1278], ax.YLim, 'r--');

xlabel('Samples at 40 Hz');
ylabel('Amplitude');
xlim([1000 1800]);    
...
figure  % fig 9, plot the longer normalized split corrected seismogram before shifting between different stations
for ista = 1:nsta
...
figure  % fig 10, plot the shorter normalized split corrected seismogram before shifting between different stations
for ista = 1:nsta
...
figure      % fig11, plot normalized final optimal results, fig 9
for ista = 1: nsta
...
%%% check if the orthogonal component is indeed very small, fig 12
...
% fig 13, plot the cc coef between fast and slow comp.
figure
for ista = 1: nsta
...
% fig 14, plot the enengy ratio between opt and ort comp.
figure
for ista = 1: nsta
...
fid = fopen(strcat(rstpath,PREFIX,'_',num2str(sps),'sps',num2str(nsta),'stas_oldcat'),'w+');
fprintf(fid,'%d %d %d %d \n',ROTS');
fclose(fid);
