...
% station 4/5, ... is to check the low freq (0.5-1.25hz) detections by the
% 3-sta trio PGC
%
...
%   check script 'lf_PGCtrio_detection_resp_check.m'
%
%   3. save results
%
...
winlensec=8;       % CHECK in detection script for what were used in first-year report
...
xcmaxAVEnmin=0.15; %0.44; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
...
hitallow = 40;     % max number of hit allowed to iniate the matrix
...
% filtcor = load(fname);
% spsratio = sps/40;
% filtlf = -filtcor(:,2)*spsratio;   % lf template shift due to filterig effect, but the sign convention is opposite to detection here  
...
%         ndSTA13off(i);
    ...
    ccmat23(ndSTA13off(i)+mshift+1, ndSTA12off(i)+mshift+1, temp) = ...
        ndcc23(i);
    ...
    [temp_count14,temp_off14,temp_tct14,temp_tbg14,temp_tdate14,temp_cc14] = ...
        Addcheck_offcount(1,nstanew,date,mshift,allrst_new,countmat14,offmat14,tctmat14,tbgmat14,tdatemat14,ccmat14);
    ...
    [temp_count15,temp_off15,temp_tct15,temp_tbg15,temp_tdate15,temp_cc15] = ...
        Addcheck_offcount(2,nstanew,date,mshift,allrst_new,countmat15,offmat15,tctmat15,tbgmat15,tdatemat15,ccmat15);
    ...
    if nstanew > 2
        ...
        [temp_count16,temp_off16,temp_tct16,temp_tbg16,temp_tdate16,temp_cc16] = ...
            Addcheck_offcount(3,nstanew,date,mshift,allrst_new,countmat16,offmat16,tctmat16,tbgmat16,tdatemat16,ccmat16);
        ...
        if nstanew > 3
            ...
            [temp_count17,temp_off17,temp_tct17,temp_tbg17,temp_tdate17,temp_cc17] = ...
                Addcheck_offcount(4,nstanew,date,mshift,allrst_new,countmat17,offmat17,tctmat17,tbgmat17,tdatemat17,ccmat17);
...
countmin = 6;   % min counts allowed in the vicinity
countmin = 1;
...
STA12off = ioff12 -(mshift+1);
STA13off = ioff13 -(mshift+1);

%%% filtering correction
if size(filtlf,1) > 2   % which means containing the effects of additional stations
    STA12off = STA12off - filtlf(2);  % since the total offset 12 contains filtering effect, so substract to correct
    STA13off = STA13off - filtlf(3);
else
    STA12off = STA12off - filtlf(1);
    STA13off = STA13off - filtlf(2);
end
...
STA23tdate(:,1) = STA23tdate(:,1)-(mshift+1);    
STA23tdate(:,2) = STA23tdate(:,2)-(mshift+1);
if size(filtlf,1) > 2
    STA23tdate(:,1) = STA23tdate(:,1)-filtlf(2);     % add filtering correction as well
    STA23tdate(:,2) = STA23tdate(:,2)-filtlf(3);
else
    STA23tdate(:,1) = STA23tdate(:,1)-filtlf(1);     
    STA23tdate(:,2) = STA23tdate(:,2)-filtlf(2);
end
...
STA23time = [STA23tdate STA23tbg STA23tct STA23cc];
%%%%%
...
[STA14meansec,STA14stdsec,STA134offsec,STA124offsec,STA134off,STA124off,STA14count,STA14tbg,STA14tct,STA14tdate,STA14cc] = ...
    Addcheck_meanstd(1,filtlf,countmat14,offmat14,tbgmat14,tctmat14,tdatemat14,ccmat14,mshift,matrange,sps,stdtol,countmin);

%%% STA14time has the same structure as STA23time
STA14time = [STA14tdate STA14tbg STA14tct STA14cc];


[STA15meansec,STA15stdsec,STA135offsec,STA125offsec,STA135off,STA125off,STA15count,STA15tbg,STA15tct,STA15tdate,STA15cc] = ...
    Addcheck_meanstd(2,filtlf,countmat15,offmat15,tbgmat15,tctmat15,tdatemat15,ccmat15,mshift,matrange,sps,stdtol,countmin);
STA15time = [STA15tdate STA15tbg STA15tct STA15cc];

       
[STA16meansec,STA16stdsec,STA136offsec,STA126offsec,STA136off,STA126off,STA16count,STA16tbg,STA16tct,STA16tdate,STA16cc] = ...
    Addcheck_meanstd(3,filtlf,countmat16,offmat16,tbgmat16,tctmat16,tdatemat16,ccmat16,mshift,matrange,sps,stdtol,countmin);
STA16time = [STA16tdate STA16tbg STA16tct STA16cc];


[STA17meansec,STA17stdsec,STA137offsec,STA127offsec,STA137off,STA127off,STA17count,STA17tbg,STA17tct,STA17tdate,STA17cc] = ...
    Addcheck_meanstd(4,filtlf,countmat17,offmat17,tbgmat17,tctmat17,tdatemat17,ccmat17,mshift,matrange,sps,stdtol,countmin);
STA17time = [STA17tdate STA17tbg STA17tct STA17cc];
...
fid = fopen(strcat(rstpath, '/MAPS/maporioffcount_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %d \n',savefile23');
fclose(fid);
fid = fopen(strcat(rstpath, '/MAPS/timeori_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f \n',STA23time');
fclose(fid);

print('-depsc',strcat(rstpath,'/FIGS/',PREFIX,'_','AddStaOffsetCheck', '_',num2str(lo),'-',num2str(hi),'_',strtrim(stasnew(1, :)),'&',strtrim(stasnew(2, :)),'&',strtrim(stasnew(3, :)),'&',strtrim(stasnew(4, :)),'_more.eps'));
...
fid = fopen(strcat(rstpath, '/MAPS/mapaddcheck_', stasnew(1,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d \n',savefile14');
fclose(fid);
fid = fopen(strcat(rstpath, '/MAPS/timeadd_', stasnew(1,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f \n',STA14time');
fclose(fid);


savefile15 = [STA125off STA135off STA125offsec STA135offsec STA15meansec STA15stdsec STA15count];
fid = fopen(strcat(rstpath, '/MAPS/mapaddcheck_', stasnew(2,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d \n',savefile15');
fclose(fid);
fid = fopen(strcat(rstpath, '/MAPS/timeadd_', stasnew(2,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f \n',STA15time');
fclose(fid);


savefile16 = [STA126off STA136off STA126offsec STA136offsec STA16meansec STA16stdsec STA16count];
fid = fopen(strcat(rstpath, '/MAPS/mapaddcheck_', stasnew(3,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d \n',savefile16');
fclose(fid);
fid = fopen(strcat(rstpath, '/MAPS/timeadd_', stasnew(3,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f \n',STA16time');
fclose(fid);
    

savefile17 = [STA127off STA137off STA127offsec STA137offsec STA17meansec STA17stdsec STA17count];
fid = fopen(strcat(rstpath, '/MAPS/mapaddcheck_', stasnew(4,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d \n',savefile17');
fclose(fid);
fid = fopen(strcat(rstpath, '/MAPS/timeadd_', stasnew(4,:), '_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
fprintf(fid,'%d %d %.4f %.4f %d %.4f %.1f %.4f \n',STA17time');
fclose(fid);
