%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script is especially to interpolate the lf detection results after
% adding additional station check, also after avoiding reduntant counting,
% in order to give a reference for hf detection
%   
% Either at the whole rectangle, or only the hf activated points
%
%
% Feature:
%   1. read result file
...
for istanew = 1: nstanew
    ...
    STA124off_lf = savefile14(:,1);
    STA134off_lf = savefile14(:,2);
    STA124offsec_lf = savefile14(:,3);
    STA134offsec_lf = savefile14(:,4);
    STA14meansec_lf = savefile14(:,5);
    STA14stdsec_lf = savefile14(:,6);
    ...
    tolmean = 10;
    STA14meansec_lf = STA14meansec_lf(abs(STA14meansec_lf) <= tolmean);
    STA124off_lf = STA124off_lf(abs(STA14meansec_lf) <= tolmean);
    STA134off_lf = STA134off_lf(abs(STA14meansec_lf) <= tolmean);
          
    % interpolation with 'scatteredInterpolant' class at a rectangle region
    F = scatteredInterpolant(STA124off_lf,STA134off_lf,STA14meansec_lf,'natural','linear');
    STA14meansecitp(:,istanew) = F(STA124offitp,STA134offitp);
    
    % interpolation only at lf detection points
    F = scatteredInterpolant(STA124offsec_lf,STA134offsec_lf,STA14meansec_lf,'natural','linear');
    ...
    tempind = find(abs(STA14meansec_lf)>0.1);
    scatter(STA124off_lf(tempind)/spslf, STA134off_lf(tempind)/spslf, msize, STA14meansec_lf(tempind), 'filled','s');    %, 'MarkerEdgeColor', 'w')
    hold on
    tempind = find(abs(STA14meansec_lf)<=0.1);
    scatter(STA124off_lf(tempind)/spslf, STA134off_lf(tempind)/spslf, msize, STA14meansec_lf(tempind), 'filled','s','MarkerEdgeColor', 'k');
    colormap(ax1,jet)
    colorbar
    caxis([-1.5, 1.5]);
    box on
    ...
    axis([-1 1.5 -1.5 1]);
    title(strcat(num2str(lolf),'-',num2str(hilf),'\_lf\_ori\_', strtrim(stasnew(istanew, :))));

    ax2=subplot(nstanew, 3, 3*istanew-1,'align');
    scatter(STA124offitp/spslf, STA134offitp/spslf, msize, STA14meansecitp(:,istanew), 'filled','s');    %, 'MarkerEdgeColor', 'w')
    colormap(ax2,jet)
    colorbar
    caxis([-1.5, 1.5]);
    box on
    ...
    ylabel('STA13 offset (s)')
    axis([-1 1.5 -1.5 1]);
    title(strcat(num2str(lolf),'-',num2str(hilf),'\_lf\_itp\_rect\_', strtrim(stasnew(istanew, :))));
    ...
    ylabel('STA13 offset (s)')
    axis([-1 1.5 -1.5 1]);
    title(strcat(num2str(lolf),'-',num2str(hilf),'\_lf\_itp\_lf\_', strtrim(stasnew(istanew, :))));
