...
%   7. functionalize GetDays, GetRots
%   8. Adding the option to use data without station response
%   9. modify the codes to use less memory by avoiding saving cumsum
%   matrix and do more computation instead.
%   10. STICK on PGC N*1
%
% NOTES:
%   24/12/2/0.45; 12/3/2/0.6; 20/10/2/0.5
%
% Difference to lf_PGCtrio_detection_resp_check.m 
...
FLAG = 'PGC';
[PERMROTS, POLROTS] = GetRotsCommon(FLAG,fam,datapath,sft2,sft3);
...
%     MO = 'SEP';
    ...
    IDENTIF=[YEAR,'.',JDAY,'.',fam,'.loff',num2str(loopoffmax),'.ccmin',num2str(xcmaxAVEnmin),...
            '.nponpa',int2str(npo),int2str(npa),'.ms', int2str(mshift)]
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
    ...
    fid = fopen([rstpath,'/MAPS/map',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'],'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistrace_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistraceoneday_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
    ...
    axis([0 86400/4 -mshift mshift]);
    ylabel('Samples converted to 40-sps base')
    ...
    plot(timswin,xmaxSTA13ntmp*40/sps,'ro','MarkerSize',2);
    %plot(timswin,xmaxSTA32ntmp,'k*','MarkerSize',2);
    axis([86400/4 86400/2 -mshift mshift]);
    ylabel('Samples converted to 40-sps base')
    box on
    ...
    plot(timswin,xmaxSTA13ntmp*40/sps,'ro','MarkerSize',2);
    %plot(timswin,xmaxSTA32ntmp,'k*','MarkerSize',2);
    axis([86400/2 3*86400/4 -mshift mshift]);
    ylabel('Samples converted to 40-sps base')
    box on
    ...
    plot(timswin,xmaxSTA13ntmp*40/sps,'ro','MarkerSize',2);
    %plot(timswin,xmaxSTA32ntmp,'k*','MarkerSize',2);
    axis([3*86400/4 86400 -mshift mshift]);
    xlabel('Sec')
    ylabel('Samples converted to 40-sps base')
    box on
    orient landscape    % is used to set up the paper orientation of a Figure or Model window for printing
    print('-depsc',[rstpath,'/FIGS/',IDENTIF,'_',num2str(winlen/sps),'s_',num2str(sps),'sps_',num2str(lo),'-',num2str(hi),'b.eps'])
    close(101)
...
%     %%% AND, xcmaxAVEnbang is more reasonable to save 
    ...
    title([IDENTIF,'_{',num2str(lo),'-',num2str(hi),'}'])
    print('-depsc',[rstpath,'/FIGS/',IDENTIF,'-',num2str(winlen/sps),'s_',num2str(sps),'sps_',num2str(lo),'-',num2str(hi),'e.eps'])
    close(102)
...
%                 [opt,ort,nzeros,timsSTAnew]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
                [opt,ort,nzeros,timsSTAnew]=readpermsv2(prename,PERMSTA,PERMROTS,idx,sps,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
            else
        ...
        if LIA
            ...
            if isfile(fname)    % if have the data file
%                 [opt,ort,nzeros]=readpols(prename,POLSTA,POLROTS,idx,sps,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
                [opt,ort,nzeros]=readpolsv2(prename,POLSTA,POLROTS,idx,sps,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
            else
                fileflag(istanew) = 0;   % change the file flag to 0, meaning abnormal
                fprintf('No data for additional station %s in day %s / %s, will be omitted. \n',POLSTA(idx,1:4), YEAR, JDAY);
                continue   % continue to next staion
        ...
        for n=1:nin     % 2nd, for each trio detection 
            ...
            if abs(icc12(istanew,n))<=offindthres && abs(icc13(istanew,n))<=offindthres && ...
                ...
                if ioff(istanew,n)>0    % shift to right -->
                    STAopt(istanew,istart+ioff(istanew,n):iend)=STAopt(istanew,istart:iend-ioff(istanew,n));
                    STAopt(istanew,istart:istart+ioff(istanew,n)-1)=0;
                elseif ioff(istanew,n)<0    % shift to left <--
                    STAopt(istanew,istart:iend+ioff(istanew,n))=STAopt(istanew,istart-ioff(istanew,n):iend);
...
%     %% METHOD 2 to add additional stations for checking 
%     %%%%%%%%%%%%%%%%%%%%%%%%%% METHOD 2, Begin %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     %%% this method doesn't seem to achieve our expectation, only results in a 
%     %%% small amount of detections pass the additional check.
%     %% Read a pre-existing offset map, if no, comment this section out to create one
%     %%% set parameters from pre-existing offset map
%     famold='002';
%     loopoffmaxold=2;
%     xcmaxAVEnminold=0.5;
%     spsold=20;     % samples per second
%     winlensecold=20;
%     winoffsecold=10;        % window offset in sec, which is the step of a moving window
%     winlenold=winlensecold*spsold;      % length in smaples
%     hiold=1.25;
%     loold=0.5;
%     npoold=2;     % poles, passes of filters
%     npaold=2;
%     cyclskipold = 20;
%     mshiftold=19+cyclskipold;
%     PREFIXold = strcat(famold,'.loff',num2str(loopoffmaxold),'.ccmin',num2str(xcmaxAVEnminold),'.nponpa',int2str(npoold),int2str(npaold),'.ms', int2str(mshiftold));
%     fname = strcat(workpath, '/PGCtrio/MAPS/mapaddcheck_', stasnew(1,:), '_',PREFIXold,'_',num2str(loold),'-',num2str(hiold),'_',num2str(winlenold/spsold),'s',num2str(spsold),'sps', num2str(nstanew), 'newsta');
%     savefile14 = load(fname);
% %     savefile14 = [STA124off STA134off STA124offsec STA134offsec STA14meansec STA14stdsec STA14count];
%     fname = strcat(workpath, '/PGCtrio/MAPS/mapaddcheck_', stasnew(2,:), '_',PREFIXold,'_',num2str(loold),'-',num2str(hiold),'_',num2str(winlenold/spsold),'s',num2str(spsold),'sps', num2str(nstanew), 'newsta');
%     savefile15 = load(fname);
%     fname = strcat(workpath, '/PGCtrio/MAPS/mapaddcheck_', stasnew(3,:), '_',PREFIXold,'_',num2str(loold),'-',num2str(hiold),'_',num2str(winlenold/spsold),'s',num2str(spsold),'sps', num2str(nstanew), 'newsta');
%     savefile16 = load(fname);
%     
%     %%% interpolation to denser region, supscript 4 means the additional
%     %%% station
%     STA124offrange = -20:1:20;
%     STA134offrange = -20:1:20;
%     
...
%     
%     figure('Position',[scrsz(3)/5 scrsz(4)/5 2*scrsz(3)/5 3*scrsz(4)/5]);
%     
%     for istanew = 1: nstanew
%         
%         % read file
%         fname = strcat(workpath, '/PGCtrio/MAPS/mapaddcheck_', stasnew(istanew,:), '_',PREFIXold,'_',num2str(loold),'-',num2str(hiold),'_',num2str(winlenold/spsold),'s',num2str(spsold),'sps', num2str(nstanew), 'newsta');
%         savefile14 = load(fname);
%         % savefile14 = [STA124off STA134off STA124offsec STA134offsec STA14meansec STA14stdsec STA14count];
%         STA124off = savefile14(:,1);
%         STA134off = savefile14(:,2);
%         STA14meansec = savefile14(:,5);
%         STA14stdsec = savefile14(:,6);
%         
%         % use the standard deviation as the constraint of points can be used
%         STA14meansec = STA14meansec(abs(STA14stdsec) <= 0.1);
%         STA124off = STA124off(abs(STA14stdsec) <= 0.1);
%         STA134off = STA134off(abs(STA14stdsec) <= 0.1);
%     
%         % interpolation with 'scatteredInterpolant' class
%         F = scatteredInterpolant(STA124off,STA134off,STA14meansec,'natural','linear');
%         STA14meansecitp(:,istanew) = F(STA124offitp,STA134offitp);
%     
%         %%% plot to check the interpolation result        
%         ax1=subplot(3, 2, 2*istanew-1,'align');
%         scatter(STA124offitp/sps, STA134offitp/sps, 15, STA14meansecitp(:,istanew), 'filled','s');    %, 'MarkerEdgeColor', 'w')
%         colormap(ax1,jet)
%         colorbar
%         caxis([-1.5, 1.5]);
%         box on
...
%         axis([-1 1.5 -1.5 1]);
%         title(strcat(num2str(lo),'-',num2str(hi),'\_interpolated\_mean\_', strtrim(stasnew(istanew, :))));
%         
%         ax2=subplot(3, 2, 2*istanew,'align');
%         scatter(STA124off/sps, STA134off/sps, 15, STA14meansec, 'filled','s');    %, 'MarkerEdgeColor', 'w')
%         colormap(ax2,jet)
%         colorbar
%         caxis([-1.5, 1.5]);
%         box on
...
%         ylabel('STA13 offset (s)')
%         axis([-1 1.5 -1.5 1]);
%         title(strcat(num2str(lo),'-',num2str(hi),'\_original\_mean\_', strtrim(stasnew(istanew, :))));
%     
%     end
%     
%     % construct the interpolation matrix for the reference in the following 
%     STA14ref = [STA124offitp STA134offitp STA14meansecitp];
%     
%     %% Add additional stations for checking 
%     clear STAopt    % clear these two variables is just to save memory space
%     clear STAnzeros
% %     stasnew=['LZB '
%     stasnew=['LZB  '
...
% %              'MGCB '];     
%     nstanew=size(stasnew,1);
%     STAoptnew=zeros(nstanew,nin_new*winlen);
%     for istanew=1:nstanew
%         found=0;
%         [LIA,idx]=ismember(stasnew(istanew,:),PERMSTA,'rows');
%         if LIA
...
% %         found=found
%         for n=1:nin_new
%             istart=istartkeep_new(n);   % saved istart of each successful window
%             iend=istart+winlen-1;
%             STAoptnew(istanew,(n-1)*winlen+1:n*winlen)=opt(istart:iend)';   % no need to divide scaleseisms here because they are all 1
%         end
%     end
%     
%     %%% NOTE: for mshiftnew 's performance, 2.2 > 2.0 > 1.5 > 1.0,
%     %%% meaningless to increase more
%     mshiftnew=ceil(2.2*mshift); %1.8 was used in first-year report   % ceil(a) gives integer >=a,
%     in=zeros(nstanew,nin_new);   % flag to indicate whether this win of this new sta is qualified (1)
%     ioff=zeros(nstanew,nin_new)+mshiftnew;    % some kind of weighted offset according to the ccmax coef
%     ccmaxave=zeros(nstanew,nin_new);
%     loff=zeros(nstanew,nin_new)+mshiftnew;    % average sum of abs offset difference 
%     count=zeros(nstanew,1);
%     tol = 0.2;    % the tolerance range of offset difference relative to the interpolation reference
%     
%     for istanew=1:nstanew   % 1st, for each additional station
% %     istanew = 2;
%         for n=1:nin_new     % 2nd, for each trio detection 
% %             n=235;
%             istart=(n-1)*winlen+1;  % start from 1
%             iend=n*winlen;
%             
%             % recall that STA1/2/3file saves the time and shifted seismogram of each station
%             %%% do xcorr between 1 new station to each of the 3-sta trio
%             [cc1, lag1] = xcorr(STA1file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
%             [cc2, lag2] = xcorr(STA2file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
%             [cc3, lag3] = xcorr(STA3file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
%             
%             ioffmat=zeros(2*mshiftnew+1,2*mshiftnew+1,2*mshiftnew+1);    % some kind of weighted offset according to the ccmax coef
%             for i=1:2*mshiftnew+1
%                 for j=1:2*mshiftnew+1
%                     for k=1:2*mshiftnew+1
%                         ioffmat(i,j,k)=cc1(i)*lag1(i) + cc2(j)*lag2(j) + cc3(k)*lag3(k)) / ...
%                                          (cc1(i)+cc2(i)+cc3(i));
%                     end
%                 end
%             end
%             
%             % find the index of row that one detection belongs to the interpolation reference
%             [~,ind1]=ismember([mapfile_new(n,3) mapfile_new(n,2)], STA14ref(:,1:2),'row');
%             
%             % find the index candidates of shift for 'ioff' that is close to the interpolation reference
%             ind2=find(abs(ioffmat/sps-STA14ref(ind1,2+istanew))<= tol);
%             
%             if ~ isempty(ind2)      % if it is not empty
%                 
%                 [ii,jj,kk]=ind2sub(size(ioffmat),ind2);
%                 
%                 % get the only one that have the highest cc coef
%                 [ccmax,ind3]=max((cc1(ii)+cc2(jj)+cc3(kk))/3);
%                 
%                 % compute the differential shift
%                 % NOTE:
%                 % In each deteciton, since sta 1,2,3 is already aligned to
%                 % denote one point, thus if it can be checked by the 4th station
%                 % the shifts between 4th station and
...
%                 % should be close to 0 theoratically. That is why it is a
%                 % constraint to filter suspicious detections
%                 icc12 = lag1(ii(ind3))-lag2(jj(ind3));
%                 icc13 = lag1(ii(ind3))-lag3(kk(ind3));
%                 icc23 = lag2(jj(ind3))-lag3(kk(ind3));
%                 loffave = (abs(icc12)+abs(icc13)+abs(icc23))/3;
%                 
%                 if abs(icc12)<=3 && abs(icc13)<=3 && abs(icc23)<=3 && loffave<=2 && ...
%                         ccmax >= 0.8*xcmaxAVEnmin && ...
%                         cc1(ii(ind3)) >= 0.4*xcmaxAVEnmin && ...
%                         cc2(jj(ind3)) >= 0.4*xcmaxAVEnmin && ...
%                         cc3(kk(ind3)) >= 0.4*xcmaxAVEnmin
%                     
%                     % Here, different from method 1, ioff,loff,ccmaxave is meaningful
%                     % only when this detection passed the check, i.e. flag
%                     % 'in' is 1
%                     in(istanew,n) = 1;
%                     count(istanew) = count(istanew)+1;  
%                     ioff(istanew,n) = round(ioffmat(ii(ind3), jj(ind3), kk(ind3)));
%                     loff(istanew,n) = loffave;
%                     ccmaxave(istanew,n) = ccmax;
%                     
%                     %Now shift new station (for plotting purposes)
%                     if ioff(istanew,n)>0    % shift to right -->
%                         STAoptnew(istanew,istart+ioff(istanew,n):iend)=STAoptnew(istanew,istart:iend-ioff(istanew,n));
%                         STAoptnew(istanew,istart:istart+ioff(istanew,n)-1)=0;
%                     elseif ioff(istanew,n)<0    % shift to left <--
%                         STAoptnew(istanew,istart:iend+ioff(istanew,n))=STAoptnew(istanew,istart-ioff(istanew,n):iend);
%                         STAoptnew(istanew,iend+ioff(istanew,n)+1:iend)=0;
%                     end
%                     
%                 end
%                 
...
%     %% METHOD 3 to add additional stations for checking 
%     %%%%%%%%%%%%%%%%%%%%%%%%%% METHOD 3, start %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     %%% This method is similar to method 2, difference is the order. Here
%     %%% we get the highest CC in each nin_new, compute the ioff, then use
%     %%% then use the interpolated the sta14off as a constraint to its reliability
%     %
%     %%% But it suffers the same problem as method 1, which means the
%     %%% interpolation thought is not a feasible way.
%     %% Read a pre-existing offset map, if no, comment this section out to create one
%     %%% set parameters from pre-existing offset map
%     famold='002';
%     loopoffmaxold=2;
...
%     mshiftnew=ceil(2.2*mshift); %1.8 was used in first-year report   % ceil(a) gives integer >=a,
%     in=zeros(nstanew,nin_new);   % flag to indicate whether this win of this new sta is qualified (1)
%     ioff=zeros(nstanew,nin_new);    % some kind of weighted offset according to the ccmax coef
%     ccmaxave=zeros(nstanew,nin_new);
%     loff=zeros(nstanew,nin_new);    % average sum of abs offset difference 
%     count=zeros(nstanew,1);
%     tol = 0.5;    % the tolerance range of offset difference relative to the interpolation reference
%     
%     for istanew=1:nstanew   % 1st, for each additional station
%         
%         for n=1:nin_new     % 2nd, for each trio detection 
...
%             [cc3, lag3] = xcorr(STA3file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
%             % get the max cc coef and index
%             [cc1max(istanew,n),icc1(istanew,n)]=max(cc1);
%             [cc2max(istanew,n),icc2(istanew,n)]=max(cc2);
%             [cc3max(istanew,n),icc3(istanew,n)]=max(cc3);
%             ccmaxave(istanew,n)=(cc1max(istanew,n)+cc2max(istanew,n)+cc3max(istanew,n))/3;
%             
%             % NOTE:
%             % In each deteciton, since sta 1,2,3 is already aligned to
%             % denote one point, thus if it can be checked by the 4th station
...
%             % should be close to 0 theoratically. That is why it is a
%             % constraint to filter suspicious detections
%             icc12(istanew,n) = lag1(icc1(istanew,n))-lag2(icc2(istanew,n));
%             icc13(istanew,n) = lag1(icc1(istanew,n))-lag3(icc3(istanew,n));
%             icc23(istanew,n) = lag2(icc2(istanew,n))-lag3(icc3(istanew,n));
%             % loff and lofftest are exactly the same, although different syntax, depends on user habit
%             % loff the absolute sum of the offset difference
%             loff(istanew,n)=(abs(icc12(istanew,n))+abs(icc13(istanew,n))+abs(icc23(istanew,n)))/3;
%             
%             tempoff=(cc1max(istanew,n) * lag1(icc1(istanew,n)) + ...
%                                        cc2max(istanew,n) * lag2(icc2(istanew,n)) + ...
%                                        cc3max(istanew,n) * lag3(icc3(istanew,n))) / ...
%                                       (cc1max(istanew,n)+cc2max(istanew,n)+cc3max(istanew,n));
%             ioff(istanew,n)=round(tempoff);
%             % find the index of row that one detection belongs to the interpolation reference
%             [~,ind1]=ismember([mapfile_new(n,3) mapfile_new(n,2)], STA14ref(:,1:2),'row');
%             
%             if abs(icc12(istanew,n))<=3 && abs(icc13(istanew,n))<=3 && ...
%                     abs(icc23(istanew,n))<=3 && loff(istanew,n)<=2 && ...
%                     abs(tempoff/sps-STA14ref(ind1,2+istanew))<= tol && ...
%                     ccmaxave(istanew,n)>=0.8*xcmaxAVEnmin && ... %0.8
%                     cc1max(istanew,n) >= 0.4*xcmaxAVEnmin && ...
%                     cc2max(istanew,n) >= 0.4*xcmaxAVEnmin && ...
%                     cc3max(istanew,n) >= 0.4*xcmaxAVEnmin
%                 
%                 in(istanew,n)=1; 
%                 count(istanew) = count(istanew)+1;
%                 
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%% METHOD 3, End %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    fid = fopen([rstpath,'/MAPS/mapaddcheck',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'],'w+');
    if nstanew == 3  
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistraceadd_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
...
%     %% WAY 1 to avoid double counting, consider each additional station separately
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 1 start %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     indext = ceil(winlensec/winoffsec)+2;   % index extension, the index range that needs to be checked
%     ind_ori=find(xmaxSTA12ntmp ~= mshift+1);
%     for istanew = 1: nstanew
%         %%% construct a more complete result matrix of all nin windows
%         allrst = [mapfile(1:nin, 1:5) mapfile(1:nin, 7:11) mapfile(1:nin, 21:23) addrstfile(:, istanew) addrstfile(:, istanew+3) addrstfile(:, istanew+6) addrstfile(:, istanew+9)];
%         allrst_new = [];
%         indskip = 0;    % skipped index
%         indreduced = [];    % index that is discarded
%         indsaved = [];  % index that is saved
%         for i = 1: nin
%             % i=4;
%             indfocus = i+ indskip;  % index that is being looked at
%             if indfocus < nin
%                 ind1=find(abs(allrst(indfocus+1: min(indfocus+indext, nin), 8)-allrst(indfocus,8)) <= dtmin);
%                 if isempty(ind1)
%                     allrst_new = [allrst_new; allrst(indfocus, :)];
%                     indskip = indskip+0;
%                     indsaved = [indsaved; indfocus];
%                 else
%                     indtemp = [indfocus; indfocus+ind1];
%                     [~,ind2] = max((allrst(indtemp, 4)+allrst(indtemp, 17))/2);
%                     allrst_new = [allrst_new; allrst(indtemp(ind2), :)];
%                     indsaved = [indsaved; indtemp(ind2)];
%                     indtemp(ind2)=[];
%                     indreduced = [indreduced; indtemp];
%                     indskip = indskip+length(ind1);
%                 end
%             elseif indfocus == nin
%                 allrst_new = [allrst_new; allrst(indfocus, :)];
%                 indsaved = [indsaved; indfocus];
...
%         indred_ori=ind_ori(indreduced);
%         xmaxSTA12ntmp(indred_ori) = mshift+1;
%         xmaxSTA13ntmp(indred_ori) = mshift+1;
%         xmaxSTA32ntmp(indred_ori) = mshift+1;
%         
%         nin_new = length(indsaved);
%         for i = 1: nin_new
%             STA1file_new((i-1)*winlen+1: i*winlen,:)= STA1file((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
%             STA2file_new((i-1)*winlen+1: i*winlen,:)= STA2file((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
%             STA3file_new((i-1)*winlen+1: i*winlen,:)= STA3file((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
%             STAopt_new((i-1)*winlen+1: i*winlen)= STAopt(istanew, (indsaved(i)-1)*winlen+1 : indsaved(i)*winlen);
%             
%             STAamp_new(i,:) = STAamp(indsaved(i),:);
%             
%             STA1bbfile_new((i-1)*winlen+1: i*winlen,:)= STA1bbfile((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
%             STA2bbfile_new((i-1)*winlen+1: i*winlen,:)= STA2bbfile((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
%             STA3bbfile_new((i-1)*winlen+1: i*winlen,:)= STA3bbfile((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
%             
%             
%             STA12file_new(i,:) = STA12file(indsaved(i), :);
%             STA13file_new(i,:) = STA13file(indsaved(i), :);
%             STA32file_new(i,:) = STA32file(indsaved(i), :);
%             
%             istartkeep_new(i) = istartkeep(indsaved(i));
%             aSTA12keep_new(i,:) = aSTA12keep(indsaved(i), :);
%             aSTA13keep_new(i,:) = aSTA13keep(indsaved(i), :);
%             aSTA32keep_new(i,:) = aSTA32keep(indsaved(i), :);
%             loopoffkeep_new(i,:) = loopoffkeep(indsaved(i), :);
%             
%             
%         end
%         STAopt_new = reshape(STAopt_new, [], 1);
%         
%         %%% Write results into files
%         fid = fopen([workpath,'/PGCtrio/MAPS/mapallrst_',stasnew(istanew,:),'_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'],'w+');
%         fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %7.2f %10.3e %10.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %d %.4f %d %.3f \n',allrst_new(1:nin_new,:)');
%         fclose(fid);
%     
%         fid = fopen(strcat(workpath, '/PGCtrio/MAPS/seistrace_',stasnew(istanew,:),IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 2 start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%% Write results into files
    fid = fopen([rstpath,'/MAPS/mapallrst',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'newsta'],'w+');
    if nstanew == 3
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistraceall_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'newsta'),'w+');
    tracefile = [STA1file_new(1:nin_new*winlen,:) STA2file_new(1:nin_new*winlen,2) STA3file_new(1:nin_new*winlen,2) STAopt_new(:, 1:nin_new*winlen)'];
    if nstanew == 3
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 2 end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for ifig=1:floor(nin_new/(nrow*mcol))+1
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(ifig),'.eps'])
        end
