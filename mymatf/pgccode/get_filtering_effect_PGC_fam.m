% function get_filtering_effect_PGC_fam(fam)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This is the function to get the filtering effect under different frequency
% band using PGC trio for any fam, like 243. Use the original seperated Bostock's
% catalog (BOSTOCK/NEW/002-246_2003.062') instead of the catalog in one file
% '/BOSTOCK/total_mag_detect_0000_cull_NEW.txt'. 
%
% take get_filtering_effect_PGCv4.m AND get_filtering_effect_LZBv2.m as a
% reference
%
%
% By Chao Song, chaosong@princeton.edu
% First created date:   2020/02/13
...
for ista = 1: nsta
    ...
    %          color(ista)); hold on
    text(1,1*ista+0.2,stas(ista,:),'fontsize',12);
end
text(7.5,3.6,'Broadband','fontsize',14);
...
lgdhf = legend(ax,[p1,p2,p3,p4,p5,p6,p7,p8,p9],{'PGC-SSIB (12)','PGC-SILB (13)','SILB-SSIB (32)',...
       'individual \Delta{t}_{12}',...
       'individual \Delta{t}_{13}', 'individual \Delta{t}_{32}',...
       'constrained \Delta{t}_{12}','constrained \Delta{t}_{13}',...
...
fignm = strcat(rstpath, '/FIGS/tempfeff_PGC_enc_',fam,'.lf',num2str(lolf),'-',num2str(hilf),...
    ...
    num2str(xcmaxAVEnmin),'.',num2str(sps),'sps.pdf');
print('-dpdf',fignm);
...
text(ax, 0.95,0.9,'0.5-1.25 Hz','fontsize',10,'unit','normalized','Horizontalalignment','right');
% text(ax, 0.5,0.2,'Cross-Correlation','FontSize',12,'unit','normalized','HorizontalAlignment','center');
lgdlf = legend(ax, [p1,p2,p3,p4,p5,p6,p7,p8,p9],{'PGC-SSIB(12)','PGC-SILB(13)','SILB-SSIB(32)',...
       'individual \Delta{t}_{12}',...
       'individual \Delta{t}_{13}', 'individual \Delta{t}_{32}',...
       'constrained \Delta{t}_{12}','constrained \Delta{t}_{13}',...
       'constrained \Delta{t}_{32}'},'location','south','fontsize',7,'numcolumns',2,...
