...
%               'BOSTOCK/NEW/002-246_2005.256'];
    ...
    %%% ADD new feature, shift the offset between different stations
    ...
    POLROTS=[6 85 33 86+sft2;  %SSIB from Yajun          
    ...
    %%% what is the meaning of zero crossings? template offsets in samples?? see notes
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
    ...
    dtmin = concentration;
    indext = ceil(winlensec/winoffsec)+2;
    mapfile_new = [];
    indskip = 0;
    indreduced = [];
    indsaved = [];
...
% i=4;
        indfocus = i+ indskip;
        if indfocus < nin
            ind1=find(abs(mapfile(indfocus+1: min(indfocus+indext, nin), 8)-mapfile(indfocus,8)) <= dtmin);
            if isempty(ind1)
                mapfile_new = [mapfile_new; mapfile(indfocus, :)];
                indskip = indskip+0;
                indsaved = [indsaved; indfocus];
            else
                indtemp = [indfocus; indfocus+ind1];
                [~,ind2] = max(mapfile(indtemp, 4));               
                mapfile_new = [mapfile_new; mapfile(indtemp(ind2), :)];
    ...
    xmaxSTA12ntmp(ind_ori(indreduced)) = mshift+1;
    xmaxSTA13ntmp(ind_ori(indreduced)) = mshift+1;
    xmaxSTA32ntmp(ind_ori(indreduced)) = mshift+1;
    ...
    fid = fopen([workpath,'/PGCtrio/MAPS/map_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'],'w+');
    fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %7.2f %10.3e %10.3e %10.3f %7.3f %7.3f %7.3f %10.3e %10.3e %10.3e %10.3e %7.3f %7.3f %7.3f %5.2f %5.2f %7.3f %7.3f %7.3f\n',mapfile_new(1:nin_new,:)');
    fclose(fid);
...
%              'MGCB '];     
    ...
    STAopt=zeros(nstanew,nin_new*winlen);
...
%         found=found
    ...
    icc12=zeros(nstanew,nin_new);
    icc13=zeros(nstanew,nin_new);
    icc23=zeros(nstanew,nin_new);
    ...
    for istanew=1:nstanew   % 1st, for each additional station
        ...
        for n=1:nin_new     % 2nd, for each trio detection 
            ...
            [cc1, lag1] = xcorr(STA1file_new(istart:iend,2),STAopt(istanew,istart:iend),mshiftnew,'coeff');
            [cc2, lag2] = xcorr(STA2file_new(istart:iend,2),STAopt(istanew,istart:iend),mshiftnew,'coeff');
            [cc3, lag3] = xcorr(STA3file_new(istart:iend,2),STAopt(istanew,istart:iend),mshiftnew,'coeff');
...
%         plot(icc3(istanew,:)-mshiftnew-1, cc3max(istanew,:), 'k+','Markersize',8);
    ...
    addrstfile = [in(:,1:nin_new); loff(:,1:nin_new) ; ioff(:,1:nin_new); ccmaxave(:,1:nin_new)]'; 
    allrst_new = [mapfile_new(1:nin_new, 1:5) mapfile_new(1:nin_new, 7:11) mapfile_new(1:nin_new, 21:23) addrstfile];
    fid = fopen([workpath,'/PGCtrio/MAPS/mapallrst_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'],'w+');
    ...
    fid = fopen(strcat(workpath, '/PGCtrio/MAPS/seistraceall_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
    tracefile = [STA1file_new(1:nin_new*winlen,:) STA2file_new(1:nin_new*winlen,2) STA3file_new(1:nin_new*winlen,2) STAopt(:, 1:nin_new*winlen)'];
    ...
    for ifig=1:floor(nin_new/(nrow*mcol))+1
            ...
            for m = 1:mcol
                ...
                if nt <= nin_new
                     ...
                     for istanew = 1: nstanew
                         ...
                         if sum(in(:,nt))>=1
                             plot(STA1file_new(winlen*(nt-1)+1:winlen*nt,1),STAopt(istanew,winlen*(nt-1)+1:winlen*nt),'c', 'linewidth',2);
                     ...
                     subplot((nstanew+1)* nrow, mcol,((nstanew+1)*(n-1)+3)*mcol+m,'align');
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',[workpath,'/PGCtrio/WIGS/',IDENTIF,'_','newcentroid_AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',[workpath,'/PGCtrio/WIGS/',IDENTIF,'_','newcentroid_AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',[workpath,'/PGCtrio/WIGS/',IDENTIF,'_','newcentroid_AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(ifig),'.eps'])
        end
