...
countmin = 1;   % min counts allowed in the vicinity

convflag = 0;   % input needed for the following called function
conv = zeros(2,1);
...
famarr = ceil(str2double(fam))*ones(size(STA14tdate(:,1)));
...
famarr = ceil(str2double(fam))*ones(size(STA15tdate(:,1)));
...
famarr = ceil(str2double(fam))*ones(size(STA16tdate(:,1))); 
...
famarr = ceil(str2double(fam))*ones(size(STA17tdate(:,1)));
...
temp = [STA12offsec STA13offsec STA23count];
temp = sortrows(temp,3);
scatter(temp(:,1), temp(:,2), msize, temp(:,3), 'filled','s');    %, 'MarkerEdgeColor', 'w')
