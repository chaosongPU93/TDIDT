...
% of templates
[~,stas,~,tempoffs] = GetDays(fam,freqflag);
...
%                   clear dummy
                    ...
                    tmp1 = STAoptbb(1,istart:iend)';    % unmoved HF segment
                    tmp2 = STAoptbb(2,istart:iend)';
                    tmp3 = STAoptbb(3,istart:iend)';
                    % CC between HF counterparts
                    [coef12, lag12] = xcorr(tmp1, tmp2, 'coeff',mshift); % cc relative to PGC
                    [maxcoef12, idx] = max(coef12);
                    offsamp12 = lag12(idx);
                    STA12bbfile(nin+1,1:2)=[offsamp12 maxcoef12];  
                    [coef13, lag13] = xcorr(tmp1, tmp3, 'coeff',mshift); % cc relative to PGC
                    [maxcoef13, idx] = max(coef13);
                    offsamp13 = lag13(idx);
                    STA13bbfile(nin+1,1:2)=[offsamp13 maxcoef13];  
                    STA1bbind(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptbb(1,istart:iend)'];    % independently aligned
                    STA2bbind(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptbb(2,istart-offsamp12:iend-offsamp12)'];
                    STA3bbind(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptbb(3,istart-offsamp13:iend-offsamp13)'];
                    
                    seglen = 4*sps;
                    tmp1 = STAoptbb(1,istart+idiff+1-1.5*sps: istart+idiff+2.5*sps)';    % the smaller segment around the largest dipole of unmoved segment
                    tmp2 = STAoptbb(2,istart+idiff+1-1.5*sps: istart+idiff+2.5*sps)';
                    tmp3 = STAoptbb(3,istart+idiff+1-1.5*sps: istart+idiff+2.5*sps)';
                    % CC between HF counterparts
                    [coef12, lag12] = xcorr(tmp1, tmp2, 'coeff',mshift); % cc relative to PGC
                    [maxcoef12, idx] = max(coef12);
                    offsamp12 = lag12(idx);
                    STA12bbafile(nin+1,1:2)=[offsamp12 maxcoef12];  
                    [coef13, lag13] = xcorr(tmp1, tmp3, 'coeff',mshift); % cc relative to PGC
                    [maxcoef13, idx] = max(coef13);
                    offsamp13 = lag13(idx);
                    STA13bbafile(nin+1,1:2)=[offsamp13 maxcoef13];
                    STA1bbaind(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptbb(1,istart:iend)'];   % independently aligned
                    STA2bbaind(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptbb(2,istart-offsamp12:iend-offsamp12)'];
                    STA3bbaind(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptbb(3,istart-offsamp13:iend-offsamp13)'];
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
    ...
    wave1bbind(winlen*(nd-1)+1:winlen*nd,:) = STA1bbind(winlen*(nt-1)+1:winlen*nt,:);
    wave2bbind(winlen*(nd-1)+1:winlen*nd,:) = STA2bbind(winlen*(nt-1)+1:winlen*nt,:);
    wave3bbind(winlen*(nd-1)+1:winlen*nd,:) = STA3bbind(winlen*(nt-1)+1:winlen*nt,:);
    
    offbb12(nd) = STA12bbfile(nt,1);    % offset samples, or index
    offbb13(nd) = STA13bbfile(nt,1);
    ccbb12(nd) = STA12bbfile(nt,2);     % cc coef
    ccbb13(nd) = STA13bbfile(nt,2);

    % the smaller segment around the largest dipole
    wave1bbaind(winlen*(nd-1)+1:winlen*nd,:) = STA1bbaind(winlen*(nt-1)+1:winlen*nt,:);
    wave2bbaind(winlen*(nd-1)+1:winlen*nd,:) = STA2bbaind(winlen*(nt-1)+1:winlen*nt,:);
    wave3bbaind(winlen*(nd-1)+1:winlen*nd,:) = STA3bbaind(winlen*(nt-1)+1:winlen*nt,:);
    offbba12(nd) = STA12bbafile(nt,1);    % offset samples, or index
    offbba13(nd) = STA13bbafile(nt,1);
    ccbba12(nd) = STA12bbafile(nt,2);     % cc coef
    ccbba13(nd) = STA13bbafile(nt,2);
...
%     text(ax,is+3, 0.66*yma, int2str(off13(nd)),'fontsize',8,'color','k');
%     text(ax,ien-3, 0.66*yma, int2str(off12(nd)),'fontsize',8,'color','b');
...
%         text(ax,is+3, 0.66*yma, int2str(off13(nd)),'fontsize',8,'color','k');
%         text(ax,ien-3, 0.66*yma, int2str(off12(nd)),'fontsize',8,'color','b');
    elseif option == 2	% 2. HF aligned independently
        plot(ax,wave1bbind(winlen*(nd-1)+1:winlen*nd,1),wave1bbind(winlen*(nd-1)+1:winlen*nd,2),'r','linew',0.6);
        plot(ax,wave2bbind(winlen*(nd-1)+1:winlen*nd,1),wave2bbind(winlen*(nd-1)+1:winlen*nd,2),'b','linew',0.6);
        plot(ax,wave3bbind(winlen*(nd-1)+1:winlen*nd,1),wave3bbind(winlen*(nd-1)+1:winlen*nd,2),'k','linew',0.6);
        is = wave1bbind(winlen*(nd-1)+1,1);
        ien= wave1bbind(winlen*nd,1);
%         text(ax,is+3, 0.66*yma, int2str(offbb13(nd)),'fontsize',8,'color','k');
%         text(ax,ien-3, 0.66*yma, int2str(offbb12(nd)),'fontsize',8,'color','b');
    elseif option == 3  % 3. HF aligned independently with the largest dipole only
        plot(ax,wave1bbaind(winlen*(nd-1)+1:winlen*nd,1),wave1bbaind(winlen*(nd-1)+1:winlen*nd,2),'r','linew',0.6);
        plot(ax,wave2bbaind(winlen*(nd-1)+1:winlen*nd,1),wave2bbaind(winlen*(nd-1)+1:winlen*nd,2),'b','linew',0.6);
        plot(ax,wave3bbaind(winlen*(nd-1)+1:winlen*nd,1),wave3bbaind(winlen*(nd-1)+1:winlen*nd,2),'k','linew',0.6);
        is = wave1bbaind(winlen*(nd-1)+1,1);
        ien= wave1bbaind(winlen*nd,1);
%         text(ax,is+3, 0.66*yma, int2str(offbba13(nd)),'fontsize',8,'color','k');
%         text(ax,ien-3, 0.66*yma, int2str(offbba12(nd)),'fontsize',8,'color','b');
    end
...
if option == 1  % 1. HF aligned with same offset as LF
    print(f.fig,'-dpdf','/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figures/waveforms_v2.pdf');
elseif option == 2	% 2. HF aligned independently
    print(f.fig,'-dpdf','/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figures/waveforms_hfalignind.pdf');
elseif option == 3  % 3. HF aligned independently with the largest dipole only
    print(f.fig,'-dpdf','/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figures/waveforms_hfalignarrind.pdf');
end
