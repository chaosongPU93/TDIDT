...
%
% distinguish from mig_linear_fit_v2, mig_linear_fit_v3 changes the panel d,
% and throws away the panel e and f as they are less useful now
%
% Chao Song, chaosong@princeton.edu
% First created date:   2021/01/20
...
respgc1 = load(fullfile(rstpath,'evtloc.002_locres_hf1spl'));
respgc2 = load(fullfile(rstpath,'evtloc.002_locres_hf2spl'));
...
slopehf = zeros(size(trange1,1),length(angle));
rmsehf = zeros(size(trange1,1),length(angle));
angrmsehf = zeros(size(trange1,1),1);
angslopehf = zeros(size(trange1,1),1);
...
%     i=3;
    ...
    for iang = 1: length(angle)
        ...
        [fitobj,gof,~] = fit(mighfdum(:,14)/3600, mighfdum(:,3),fttpfree,'Robust','Bisquare',...
                                  'StartPoint',[1 1]);
    ...
    mintime = max(min(mighfdum(:,14)), min(miglfdum(:,14)));
    maxtime = min(max(mighfdum(:,14)), max(miglfdum(:,14)));
    mighfdum2 = mighfdum(mighfdum(:,14)>=mintime & mighfdum(:,14)<=maxtime, :);
    miglfdum2 = miglfdum(miglfdum(:,14)>=mintime & miglfdum(:,14)<=maxtime, :);
    
    % assign the location resolution from one fam
    [dx, dy] = absloc2relaloc(respgc1(:,1),respgc1(:,2),lon0,lat0);
    vecrespgc1 = [dx dy];
    [dx, dy] = absloc2relaloc(respgc2(:,1),respgc2(:,2),lon0,lat0);
    vecrespgc2 = [dx dy];
...
%     c.Label.String = strcat(num2str(trange1(i,1)),' of HF',' (hr)');
    ...
    drawArrow(f.ax(1),xvect,yvect,xran,yran,'linewidth',1.5,'linestyle','--','color',...
              [0.4 0.4 0.4]);
    x = [25+ vecrespgc1(1,1), 25+ vecrespgc1(2,1)];
    y = [-5+ vecrespgc1(1,2), -5+ vecrespgc1(2,2)];  % loc resolution indicated by +-1 sample cross
    plot(f.ax(1),x,y,'color','r','linewidth',1);
    x = [25+ vecrespgc1(3,1), 25+ vecrespgc1(4,1)];
    y = [-5+ vecrespgc1(3,2), -5+ vecrespgc1(4,2)];  % loc resolution indicated by +-1 sample cross      
    plot(f.ax(1),x,y,'color','r','linewidth',1);
    ...
    text(f.ax(1),0.84,0.90,strcat({'in '},num2str(trange1(i,3)-trange1(i,2)),{' s'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
...
%     c.Label.String = strcat(num2str(trange1(i,1)),' of LF',' (hr)');
    ...
    drawArrow(f.ax(2),xvect,yvect,xran,yran,'linewidth',1.5,'linestyle','--','color',...
              [0.4 0.4 0.4]);
    x = [25+ vecrespgc2(1,1), 25+ vecrespgc2(2,1)];
    y = [-5+ vecrespgc2(1,2), -5+ vecrespgc2(2,2)];  % loc resolution indicated by +-1 sample cross
    plot(f.ax(2),x,y,'color','r','linewidth',1);
    x = [25+ vecrespgc2(3,1), 25+ vecrespgc2(4,1)];
    y = [-5+ vecrespgc2(3,2), -5+ vecrespgc2(4,2)];  % loc resolution indicated by +-1 sample cross
    plot(f.ax(2),x,y,'color','r','linewidth',1);
    ...
    text(f.ax(2),0.84,0.95,strcat(num2str(size(miglf,1)),{' detections'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f.ax(2),0.84,0.9,strcat({'in '},num2str(trange1(i,3)-trange1(i,2)),{' s'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    ...
    [fitobjhfprop,gofhfprop,outphfprop] = fit(mighfdum2(:,14)/3600, mighfdum2(:,3),fttpfree,'Robust',...
                                              'Bisquare','StartPoint',[1 1]);
    % output fit parameters
    coefprop = coeffvalues(fitobjhfprop);
    slopeprophf(i) = coefprop(1);
    intcptprophf(i) = coefprop(2);
    fitprophf = feval(fitobjhfprop,mighfdum2(:,14)/3600);
    plot(f.ax(3),mighfdum2(:,14)/3600,fitprophf,'-','linewidth',2,'color',[0.6 0.6 0.6]);
...
%     fitproplf = slopeprophf*miglfdum(:,end)/3600+intcptproplf;
    ...
    [fitobjlfprop,goflfprop,outplfprop] = fit(miglfdum2(:,14)/3600, miglfdum2(:,3),fttpfix,'Robust',...
...
%                                               'Bisquare','Weights',miglfdum(:,11));
    fitproplf = feval(fitobjlfprop,miglfdum2(:,14)/3600);
    intcptproplf(i) = coeffvalues(fitobjlfprop);
    offset(i) = intcptproplf(i)-intcptprophf(i);
    plot(f.ax(3),miglfdum2(:,14)/3600,fitproplf,'-','linewidth',2,'color',[0.6 1 1]);
    f.ax(3).Box = 'on';
    grid(f.ax(3), 'on');
...
%              round(max([mighfdum(:,3);miglfdum(:,3)]))+1];
    aa = round(prctile([mighfdum(:,3);miglfdum(:,3)], 98));
    bb = round(prctile([mighfdum(:,3);miglfdum(:,3)], 2));
    aap = aa + ceil((aa-bb)/6);
    bbp = bb - ceil((aa-bb)/3);
    yran1 = [bbp aap];
    xlim(f.ax(3),xran1);
    ...
    fittemp = feval(fitobjhfprop,miglfdum2(:,14)/3600);
    verttemp = miglfdum2(:,3) - fittemp;     % can be + or -
...
%     hold(f.ax(4),'on');
%     f.ax(4).FontSize = 9;
    numhf(i) = length(mighfdum2(:,3));
    numlf(i) = length(miglfdum2(:,3));
...
%     hold(f.ax(4),'off');
    ...
    %%% save figure
    print(f.fig,'-dpdf',strcat(rstpath,'/',fam,'migv3.proj.lfit.meddist',num2str(trange1(i,1)),...
