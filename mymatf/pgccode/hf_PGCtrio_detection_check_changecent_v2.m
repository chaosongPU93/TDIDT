...
% detections.
%
% Changing the centroid is to avoid the memory limit when trying to
% increase the mshift to enlarge the region in hf detection add check.
%
% BUT change the centroid of focused region by changing the 4th column of
% PERMROTS and POLROTS, i.e. after you align the data at different
% stations, centroid 0,0,0 denotes a different spot on the map.
% 
% SIMILAR to hf_PGCtrio_detection_check.m
%
% Features:
%   1. To exmine the hf detections, add new stations to do xcorr
%   2. CHANGE the centroid location
%   3. Modify several lines to save memory
%
% Based on chaoSTA.m & wdwSTAadd.m
%
...
sft2 = -20;  % according to the geometry, adding this 2 means shift to right down (southeast)
sft3 = 20;
FLAG = 'PGC';
CATA = 'new';
...
%     nd=1;
    ...
    bostsamp=round(bostsec*40);    % round to nearest integer, round(4.4)=4, round(4.5)=5
...
%                 [opt,ort,nzeros,timsSTA]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
                ...
                [opt,ort,nzeros,timsSTA]=readpermsv2(prename,PERMSTA,PERMROTS,idx,sps,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
            else
...
%             [opt,~,~,~]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lobb,hibb,npo,npa,fact,nwin,winlen,winoff,igstart);
            [opt,~,~,~]=readpermsv2(prename,PERMSTA,PERMROTS,idx,sps,lobb,hibb,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        [LIA,idx]=ismember(stas(ista,:),POLSTA,'rows');
        if LIA
...
%             [opt,~,~]=readpols(prename,POLSTA,POLROTS,idx,sps,lobb,hibb,npo,npa,fact,nwin,winlen,winoff,igstart);
            [opt,~,~]=readpolsv2(prename,POLSTA,POLROTS,idx,sps,lobb,hibb,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
                    %%% NOTICE: add by Chao, save xcmaxAVEnbang instead of xcmaxAVEn
    ...
    fid = fopen([rstpath,'/MAPS/map_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'],'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistrace_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
...
%     close(101)
    ...
    title([IDENTIF,'_{',num2str(lo),'-',num2str(hi),'}'])
%     print('-depsc',[rstpath,'/FIGS/',IDENTIF,'_',num2str(winlen/sps),'s_',num2str(sps),'sps_',num2str(lo),'-',num2str(hi),'e.eps'])
%     close(102)
...
%         plot(icc3(istanew,:)-mshiftnew-1, cc3max(istanew,:), 'k+','Markersize',8);
    ...
    addrstfile = [in(:,1:nin); loff(:,1:nin) ; ioff(:,1:nin); ccmaxave(:,1:nin)]'; 
    fid = fopen([rstpath,'/MAPS/mapaddcheck_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'],'w+');
    if nstanew == 3  
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistraceadd_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'newsta'),'w+');
...
%         
%         %%% Write results into files
%         fid = fopen([workpath,'/PGCtrio/MAPS/mapallrst_newcentroid_',stasnew(istanew,:),'_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'],'w+');
%         fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %7.2f %10.3e %10.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %d %.4f %d %.3f \n',allrst_new(1:nin_new,:)');
%         fclose(fid);
%     
%         fid = fopen(strcat(workpath, '/PGCtrio/MAPS/seistrace_newcentroid_',stasnew(istanew,:),IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 2 start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1: nin
        ...
        indfocus = i+ indskip;  % index that is being looked at
        if indfocus < nin
            ind1=find(abs(allrst(indfocus+1: min(indfocus+indext, nin), 7)-allrst(indfocus,7)) <= dtmin);
            if isempty(ind1)
                allrst_new = [allrst_new; allrst(indfocus, :)];
                indskip = indskip+0;
                indsaved = [indsaved; indfocus];
                ...
                indtemp = [indfocus; indfocus+ind1];
                if nstanew == 3
                    [~,ind2] = max(sum(allrst(indtemp, [4 23 24 25]), 2)/4);
                elseif nstanew == 4
                    [~,ind2] = max(sum(allrst(indtemp, [4 26 27 28 29]), 2)/5);
                end
                allrst_new = [allrst_new; allrst(indtemp(ind2), :)];
                indsaved = [indsaved; indtemp(ind2)];
                indtemp(ind2)=[];
    ...
    for i = 1: nin_new
        ...
        STA2file_new((i-1)*winlen+1: i*winlen,:)= STA2file((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
        STA3file_new((i-1)*winlen+1: i*winlen,:)= STA3file((indsaved(i)-1)*winlen+1 : indsaved(i)*winlen, :);
        STAopt_new(1:nstanew,(i-1)*winlen+1: i*winlen)= STAopt(1:nstanew, (indsaved(i)-1)*winlen+1 : indsaved(i)*winlen);
    ...
    %%% Write results into files
    fid = fopen([rstpath,'/MAPS/mapallrst_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'newsta'],'w+');
    if nstanew == 3
    ...
    fid = fopen(strcat(rstpath, '/MAPS/seistraceall_newcentroid_',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'newsta'),'w+');
    tracefile = [STA1file_new(1:nin_new*winlen,:) STA2file_new(1:nin_new*winlen,2) STA3file_new(1:nin_new*winlen,2) STAopt_new(:, 1:nin_new*winlen)'];
    if nstanew == 3
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 2 end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for ifig=1:floor(nin_new/(nrow*mcol))+1
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','newcentroid_AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','newcentroid_AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','newcentroid_AddStaComp','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(ifig),'.eps'])
        end
