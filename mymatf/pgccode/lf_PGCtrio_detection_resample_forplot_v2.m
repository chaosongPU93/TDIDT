...
% Using data at 40 sps, but down-sampled to 20 sps. 
%
% This is especially for plotting several examples of LF detection and their
% corresponding HF segments for figure 1 in the paper.
% Version 2:
%   1. It seems that the align of hf in version 1 was done independently between
%       stations, which means the circuit of CC offsets may not be 0. That was
%       WRONG.
%   2. Additionally, LF detection was using data downsampled from 40 sps
%       20 sps, which is different from hf. And to compare the offset difference
%       between the alighed hf and lf records with the filtering effect, it
%       may be necessary to upsample the records to 80 sps as done in obtaining
%       filtering effect.
% 
% Features:
...
bostname=['BOSTOCK/NEW/002-246_2004.199';
          'BOSTOCK/NEW/002-246_2005.255';
%           'BOSTOCK/NEW/002-246_2003.062';                  % directory name, path
          'BOSTOCK/NEW/002-246_2005.256';
          'BOSTOCK/NEW/002-246_2004.197'];
% timoffrot= [2005 255];
% bostname=['BOSTOCK/NEW/002-246_2005.255'];

% serial number of the object detections
detnum = [
          9;
          20;
%           33;
...
npa=2;
cyclskip = 20*(sps/20);
mshift=sps-1+cyclskip; %19; %maximum shift for the x-correlations. 19 for 002 Stanford,    % in sps, 0.5s*40sps=20
loopoffmax=4*(sps/20); %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.   % what is loopoffmax, the circuit of time offsets
...
% subplot(311)
% for ista=1:nsta
%     plot(temp1(ista,:)); hold on
% %     pause(5)
% end
% subplot(312)
% for ista=1:nsta
%     plot(temp2(ista,:)); hold on
% %     pause(5)
% end
% subplot(313)
% for ista=1:nsta
%     plot(temp3(ista,:)); hold on
% %     pause(5)
% end
%%%
...
%         n=1;
        ...
        if xcmaxAVEn(n)<xcmaxAVEnmin || abs(loopoff(n))>loopoffmax ...
            ...
            if abs(iSTA12bang-imaxSTA12n) <= loopoffmax && ...  %not sure if these 3 lines are satisfied automatically ...
               ...
               abs(tmp12) <= mshift-cyclskip && abs(tmp13) <= mshift-cyclskip && abs(tmp32) <= mshift-cyclskip
               ...
               %%% == xcmaxconprev according to codes
                ...
                xcmaxAVEnbang(nin+1)=(sumsSTA12n(n,iSTA12bang)+sumsSTA13n(n,iSTA13bang)+sumsSTA32n(n,iSTA32bang))/3;
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
                    ...
                    mapfile(nin,:)=[timswin(n) xmaxSTA13ntmp(n) xmaxSTA12ntmp(n) ...
                        xcmaxAVEnbang(nin) loopoff(n) Ampsq(nin) cumsumtrdiff timswin(n)-winlensec/2+idiff/sps cumsumtrdiff/cumsumtr(winlen) ...
                        match(nin,1) match(nin,2) Prev(nin) Post(nin) Prev15(nin) Prev30(nin) ...
                        ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) sumsSTA12n(n,iSTA12bang) sumsSTA13n(n,iSTA13bang) sumsSTA32n(n,iSTA32bang)];

                    
                    %%%%%%%%%%% START, align the corresonding hf records %%%%%%%%%%%%%%%%%%%%%%%%%%%                    
                    % use the largest arrival time in LF to define the window for HF detectetion
                    STAoptbba = [];
                    if nd <= 2
                        seglen = 4*sps+2*mshift;
                        STAoptbba(1,:) = STAoptbb(1,istart+idiff+1-1.5*sps-mshift: istart+idiff+2.5*sps+mshift)';    % the smaller segment around the largest dipole of unmoved segment
                        STAoptbba(2,:) = STAoptbb(2,istart+idiff+1-1.5*sps-mshift: istart+idiff+2.5*sps+mshift)';
                        STAoptbba(3,:) = STAoptbb(3,istart+idiff+1-1.5*sps-mshift: istart+idiff+2.5*sps+mshift)';
                    else
                        seglen = winlen+2*mshift;
                        STAoptbba(1,:) = STAoptbb(1,istart-mshift: iend+mshift)';    % the smaller segment around the largest dipole of unmoved segment
                        STAoptbba(2,:) = STAoptbb(2,istart-mshift: iend+mshift)';
                        STAoptbba(3,:) = STAoptbb(3,istart-mshift: iend+mshift)';
                    end
                    
                    STAautobba=STAoptbba.^2;
                    
                    lenxbba=seglen-2*mshift;     % 86400*sps-19*2
                    STA12xbba=zeros(lenxbba, 2*mshift+1);    % 19*2+1
                    STA13xbba=zeros(lenxbba, 2*mshift+1);    % stas: 1->PGC, 2->SSIB, 3->SILB
                    STA32xbba=zeros(lenxbba, 2*mshift+1);
                    %%% SEE NOTES, Chao
                    for nn=-mshift:mshift
                        % PGC corr SSIB, 1+mshift:tracelen-mshift == 1+mshift-n:tracelen-mshift-n == lenx
                        STA12xbba(:,nn+mshift+1)=STAoptbba(1,1+mshift:seglen-mshift).* ...
                            STAoptbba(2,1+mshift-nn:seglen-mshift-nn);
                        % PGC corr SILB
                        STA13xbba(:,nn+mshift+1)=STAoptbba(1,1+mshift:seglen-mshift).* ...
                            STAoptbba(3,1+mshift-nn:seglen-mshift-nn);
                        % SILB corr SSIB
                        STA32xbba(:,nn+mshift+1)=STAoptbba(3,1+mshift:seglen-mshift).* ...
                            STAoptbba(2,1+mshift-nn:seglen-mshift-nn);
                    end
                    
                    %     timswin=zeros(nwin,1);    %%% timswin refers to the central times of those small windows.
                    sumsSTA12bba=zeros(1,2*mshift+1);   % PGC-SSIB
                    sumsSTA13bba=zeros(1,2*mshift+1);   % PGC-SILB
                    sumsSTA32bba=zeros(1,2*mshift+1);   % SILB-SSIB
                    sumsSTA1sqbba=zeros(1,2*mshift+1);  % "sq" are the running cumulative sum, to be used later by differencing the window edpoints
                    sumsSTA2sqbba=zeros(1,2*mshift+1);
                    sumsSTA3sqbba=zeros(1,2*mshift+1);
                    sumsSTA3Bsqbba=zeros(1,2*mshift+1); % refers the shifting SSI for SIL is not moving?
                    kk=1;
                    istartbba=40;     % 2*sps+1 + (n-1)*3*sps, igstart is the index of the starting sample; istart is index of each sample window
                    iendbba=istartbba+lenxbba-1;              % + 12.5*sps, ADD -1 by Chao, 2019/02/17
                    %         timswin(n)=timsSTA(istart+winlen/2);    % timsSTA, time serie data of STA; timswin, center of win, also == (istart+iend)/2
                    sumsSTA12bba(kk,:)=sum(STA12xbba(istartbba-mshift: iendbba-mshift, :));
                    sumsSTA13bba(kk,:)=sum(STA13xbba(istartbba-mshift: iendbba-mshift, :));
                    sumsSTA32bba(kk,:)=sum(STA32xbba(istartbba-mshift: iendbba-mshift, :));
                    sumsSTA1sqbba(kk,:)=sum(STAautobba(1, istartbba: iendbba));
                    sumsSTA3Bsqbba(kk,:)=sum(STAautobba(3, istartbba: iendbba));
                    for nn=-mshift:mshift
                        sumsSTA2sqbba(kk,nn+mshift+1)=sum(STAautobba(2, istartbba-nn: iendbba-nn)); %+m??? (yes).
                        sumsSTA3sqbba(kk,nn+mshift+1)=sum(STAautobba(3, istartbba-nn: iendbba-nn));
                    end
                    
                    glitches=1.e-7;
                    sumsSTA1sqbba=max(sumsSTA1sqbba,glitches);
                    sumsSTA2sqbba=max(sumsSTA2sqbba,glitches);
                    sumsSTA3sqbba=max(sumsSTA3sqbba,glitches);    % return maximum between A and B
                    %
                    denomSTA12nbba=realsqrt(sumsSTA1sqbba.*sumsSTA2sqbba);    % Real square root, An error is produced if X is negative
                    denomSTA13nbba=realsqrt(sumsSTA1sqbba.*sumsSTA3sqbba);
                    denomSTA32nbba=realsqrt(sumsSTA3Bsqbba.*sumsSTA2sqbba);
                    %
                    sumsSTA12nbba=sumsSTA12bba./denomSTA12nbba;   % suffix 'n' means normalized
                    sumsSTA13nbba=sumsSTA13bba./denomSTA13nbba;
                    sumsSTA32nbba=sumsSTA32bba./denomSTA32nbba;
                    [xcmaxSTA12nbba,imaxSTA12bba]=max(sumsSTA12nbba,[],2);   %Integer-offset max cross-correlation
                    [xcmaxSTA13nbba,imaxSTA13bba]=max(sumsSTA13nbba,[],2);   % along row, max cc val and index in each window
                    [xcmaxSTA32nbba,imaxSTA32bba]=max(sumsSTA32nbba,[],2);
                    %Parabolic fit:
                    [xmaxSTA12nbba,ymaxSTA12nbba,aSTA12bba]=parabol(1,mshift,sumsSTA12nbba,imaxSTA12bba); %Interpolated max cross-correlation
                    [xmaxSTA13nbba,ymaxSTA13nbba,aSTA13bba]=parabol(1,mshift,sumsSTA13nbba,imaxSTA13bba);
                    [xmaxSTA32nbba,ymaxSTA32nbba,aSTA32bba]=parabol(1,mshift,sumsSTA32nbba,imaxSTA32bba);
                    
                    %Center them
                    imaxSTA12centbba=imaxSTA12bba-mshift-1;  % "cent" is "centered"; imaxSTA12 is original 1: 2*mshift+1, corresponds to -mshift: mshift
                    imaxSTA13centbba=imaxSTA13bba-mshift-1;
                    imaxSTA32centbba=imaxSTA32bba-mshift-1;
                    %%% NOTICE: the right order of a closed 3-sta pair is +13, -12, +32, where 13 means 1-->3
                    iloopoffbba=imaxSTA13centbba-imaxSTA12centbba+imaxSTA32centbba; %How well does the integer loop close?
                    %
                    xmaxSTA12nbba=xmaxSTA12nbba-mshift-1;
                    xmaxSTA13nbba=xmaxSTA13nbba-mshift-1;
                    xmaxSTA32nbba=xmaxSTA32nbba-mshift-1;
                    loopoffbba=xmaxSTA13nbba-xmaxSTA12nbba+xmaxSTA32nbba; %How well does the interpolated loop close?
                    xcmaxAVEnbba=(xcmaxSTA12nbba+xcmaxSTA13nbba+xcmaxSTA32nbba)/3;     % arithmetic average, == x-corr max average normalized
                    
%                     xmaxSTA12ntmpbba=xmaxSTA12nbba;    % tmp == temporary
%                     xmaxSTA13ntmpbba=xmaxSTA13nbba;
%                     xmaxSTA32ntmpbba=xmaxSTA32nbba;
                    
                    % assume this segment is a HF detection
                    xcmaxconprev=-99999.;  %used to be 0; not good with glitches
                    %%% NOTE here are using offset imaxSTA12/13/32 without centering, i.e. [1, 2shift+1]
                    imaxSTA12nbba=imaxSTA12bba; %This "n" for nth window; other "n's" for "normalized".  Unfortunately.
                    imaxSTA13nbba=imaxSTA13bba;
                    imaxSTA32nbba=imaxSTA32bba;
                    sumsSTA12nnbba=sumsSTA12nbba;   % xxx'n' for normalized,
                    sumsSTA13nnbba=sumsSTA13nbba;
                    sumsSTA32nnbba=sumsSTA32nbba;
                    % Usually, the loop happens between imaxSTA12n +- floor(loopoffmax+1),
                    % width of which is 2*floor(loopoffmax+1)
                    %%% floor(2.5)=2; floor(-2.6)=-3
                    for iSTA12bba =     max(1,imaxSTA12nbba-floor(loopoffmax+1)): min(imaxSTA12nbba+floor(loopoffmax+1),2*mshift+1)
                        for iSTA13bba = max(1,imaxSTA13nbba-floor(loopoffmax+1)): min(imaxSTA13nbba+floor(loopoffmax+1),2*mshift+1)
                            ibangonbba = (mshift+1)-iSTA13bba+iSTA12bba;
                            %%% i.e., -mshift <= -iSTA13+iSTA12 <= mshift
                            if ibangonbba >= 1 && ibangonbba <= 2*mshift+1
                                xcmaxcon=sumsSTA12nnbba(iSTA12bba)+sumsSTA13nnbba(iSTA13bba)+sumsSTA32nnbba(ibangonbba);
                                if xcmaxcon > xcmaxconprev
                                    xcmaxconprev=xcmaxcon;
                                    iSTA12bangbba=iSTA12bba;
                                    iSTA13bangbba=iSTA13bba;
                                end
                            end
                        end
                    end
                    ...
                    iSTA32bangbba=(mshift+1)-iSTA13bangbba+iSTA12bangbba;
                    tmp12 = iSTA12bangbba-(mshift+1);  % these 3 lines are added by Chao
                    tmp13 = iSTA13bangbba-(mshift+1);
                    tmp32 = iSTA32bangbba-(mshift+1);
                    
                    xmaxSTA12ntmpbba =tmp12; %without interpolation this is just centering.
                    xmaxSTA13ntmpbba =tmp13;
                    xmaxSTA32ntmpbba =tmp32;
                    
                    %%% xcmaxAVEnbang is added by Chao, to distinguish from
                    %%% xcmaxAVEn, because it is max average CC coef
                    xcmaxAVEnbangbba =(sumsSTA12nbba(iSTA12bangbba)+sumsSTA13nbba(iSTA13bangbba)...
                                          +sumsSTA32nbba(iSTA32bangbba))/3;
                    
                    %for plotting traces
                    imaxSTA12wrbba =round(xmaxSTA12ntmpbba); %without interpolation this is not needed.
                    imaxSTA13wrbba =round(xmaxSTA13ntmpbba);
                    
                    % save the result
                    STA1bbafile((nin-1)*winlen+1:nin*winlen,1:2)=[timsSTA(istart: iend)' ...
                                STAoptbb(1,istart: iend)'];   % independently aligned
                    STA2bbafile((nin-1)*winlen+1:nin*winlen,1:2)=[timsSTA(istart: iend)' ...
                                STAoptbb(2,istart-imaxSTA12wrbba: iend-imaxSTA12wrbba)'];
                    STA3bbafile((nin-1)*winlen+1:nin*winlen,1:2)=[timsSTA(istart: iend)' ...
                                STAoptbb(3,istart-imaxSTA13wrbba: iend-imaxSTA13wrbba)'];
                    STA12bbafile(nin,1:2)=[imaxSTA12wrbba xcmaxSTA12nbba];  
                    STA13bbafile(nin,1:2)=[imaxSTA13wrbba xcmaxSTA13nbba];
                    
                    mapfilebba(nin, :) = [xmaxSTA12ntmpbba xmaxSTA13ntmpbba xmaxSTA32ntmpbba ...
                                          xcmaxAVEnbangbba sumsSTA12nbba(iSTA12bangbba) sumsSTA13nbba(iSTA13bangbba) ...
                                          sumsSTA32nbba(iSTA32bangbba)];

                    %%%%%%%%%%% END, align the corresonding hf records %%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%% save the information for plotting afterwards
    nt = detnum(nd);
    wave1(winlen*(nd-1)+1:winlen*nd,:) = STA1file(winlen*(nt-1)+1:winlen*nt,:);
    wave2(winlen*(nd-1)+1:winlen*nd,:) = STA2file(winlen*(nt-1)+1:winlen*nt,:);
    wave3(winlen*(nd-1)+1:winlen*nd,:) = STA3file(winlen*(nt-1)+1:winlen*nt,:);
    
    arrt(nd) = STA32file(nt,3);     % arrival time of largest dipole in samples, or index
%     off12(nd) = STA12file(nt,1);    % offset samples, or index
%     off13(nd) = STA13file(nt,1);
    off12(nd) = mapfile(nt,3);    % offset samples, or index
    off13(nd) = mapfile(nt,2);

    wave1bb(winlen*(nd-1)+1:winlen*nd,:) = STA1bbfile(winlen*(nt-1)+1:winlen*nt,:);
    wave2bb(winlen*(nd-1)+1:winlen*nd,:) = STA2bbfile(winlen*(nt-1)+1:winlen*nt,:);
    wave3bb(winlen*(nd-1)+1:winlen*nd,:) = STA3bbfile(winlen*(nt-1)+1:winlen*nt,:);
    
%     cc12(nd) = STA12file(nt,2);     % cc coef
%     cc13(nd) = STA13file(nt,2);
%     cc32(nd) = STA32file(nt,2);
    cc12(nd) = mapfile(nt,21);     % cc coef
    cc13(nd) = mapfile(nt,22);
    cc32(nd) = mapfile(nt,23);

    
    % the smaller segment around the largest dipole
    wave1bba(winlen*(nd-1)+1:winlen*nd,:) = STA1bbafile(winlen*(nt-1)+1:winlen*nt,:);
    wave2bba(winlen*(nd-1)+1:winlen*nd,:) = STA2bbafile(winlen*(nt-1)+1:winlen*nt,:);
    wave3bba(winlen*(nd-1)+1:winlen*nd,:) = STA3bbafile(winlen*(nt-1)+1:winlen*nt,:);
    offbba12(nd) = mapfilebba(nt,1);    % offset samples, or index
    offbba13(nd) = mapfilebba(nt,2);
    ccbba12(nd) = mapfilebba(nt,5);     % cc coef
    ccbba13(nd) = mapfilebba(nt,6);
    ccbba32(nd) = mapfilebba(nt,7);
...
set(f.ax(1), 'position', [ 0.1, 0.74, 0.4, 0.18]);
set(f.ax(2), 'position', [ 0.56, 0.74, 0.4, 0.18]);
set(f.ax(3), 'position', [ 0.1, 0.56, 0.4, 0.18]);
set(f.ax(4), 'position', [ 0.56, 0.56, 0.4, 0.18]);
set(f.ax(5), 'position', [ 0.1, 0.3, 0.4, 0.18]);
set(f.ax(6), 'position', [ 0.56, 0.3, 0.4, 0.18]);
set(f.ax(7), 'position', [ 0.1, 0.12, 0.4, 0.18]);
set(f.ax(8), 'position', [ 0.56, 0.12, 0.4, 0.18]);

xlabstr = {'Time (s) on 17 Jul. 2004';
           'Time (s) on 12 Sep. 2005';
%            'Time (s) on 3 Jun. 2003';
           'Time (s) on 13 Sep. 2005';
           'Time (s) on 15 Jul. 2004'};
...
%     ax.FontSize = 8;
    ...
    yma=max(max([wave1(winlen*(nd-1)+1:winlen*nd,2) wave2(winlen*(nd-1)+1:winlen*nd,2) ...
        wave3(winlen*(nd-1)+1:winlen*nd,2)]));
    ymi=min(min([wave1(winlen*(nd-1)+1:winlen*nd,2) wave2(winlen*(nd-1)+1:winlen*nd,2) ...
        wave3(winlen*(nd-1)+1:winlen*nd,2)]));
    yma=2.4*max(yma,-ymi);
    
    is = wave1(winlen*(nd-1)+1,1);   % start time of each win
    ien= wave1(winlen*nd,1);         % end time of each win
    axis(ax,[is ien -yma yma]);
    
    %%% patch the main arrival window
    xarea=[is+arrt(nd)/sps is+(arrt(nd)+cncntr)/sps is+(arrt(nd)+cncntr)/sps is+arrt(nd)/sps ...
        is+arrt(nd)/sps];
    ...
    plot(ax,wave1(winlen*(nd-1)+1:winlen*nd,1),wave1(winlen*(nd-1)+1:winlen*nd,2),'r','linew',0.6);
    plot(ax,wave2(winlen*(nd-1)+1:winlen*nd,1),wave2(winlen*(nd-1)+1:winlen*nd,2),'b','linew',0.6);
    plot(ax,wave3(winlen*(nd-1)+1:winlen*nd,1),wave3(winlen*(nd-1)+1:winlen*nd,2),'k','linew',0.6);


    %%% plot freq. band
    plot(ax,[is+1/hi is+1/lo],[-0.8*yma -0.8*yma],'k','linew',3)
    %%% plot main arrival window
    % STA32file(nt,3), 3rd col. -> idiff, /sps to time instead of samples
%     xcvect=[is+arrt(nd)/sps is+(arrt(nd)+cncntr)/sps];
%     ycvect=[0.8*yma 0.8*yma];
%     plot(ax,xcvect,ycvect,'color',[0.6 0.6 0.6],'linew',3)

%     text(ax,is+3, 0.66*yma, num2str(off13(nd)),'fontsize',8,'color','k');
%     text(ax,ien-3, 0.66*yma, num2str(off12(nd)),'fontsize',8,'color','b');
    
%     set(ax,'XTick',[is (is+ien)/2],'fontsize',8);
%     set(ax,'XTick',floor(is):1:floor(ien),'fontsize',8);
    set(ax,'XTick',is:1:ien,'fontsize',8);
%     xtklb = is:1:ien;
%     xtklb = 
    ax.XTickLabel = [];
    ylabel(ax,'Amplitude','fontsize',9);
    text(ax,0.03,0.83,figlabstr{nd},'fontsize',10,'unit','normalized','EdgeColor','k','Margin',1);
%     ax.XMinorTick='on';
    hold(ax,'off');
...
%     ax.FontSize = 8;
    ...
    patch(ax,xarea,yarea,'k','Facealpha',0.15,'edgecolor','none');
    option = 2;
    if option == 1  % 1. HF aligned with same offset as LF
        plot(ax,wave1bb(winlen*(nd-1)+1:winlen*nd,1),wave1bb(winlen*(nd-1)+1:winlen*nd,2),'r','linew',0.6);
        plot(ax,wave2bb(winlen*(nd-1)+1:winlen*nd,1),wave2bb(winlen*(nd-1)+1:winlen*nd,2),'b','linew',0.6);
        plot(ax,wave3bb(winlen*(nd-1)+1:winlen*nd,1),wave3bb(winlen*(nd-1)+1:winlen*nd,2),'k','linew',0.6);
        is = wave1bb(winlen*(nd-1)+1,1);
        ien= wave1bb(winlen*nd,1);
%         text(ax,is+3, 0.66*yma, num2str(off13(nd)),'fontsize',8,'color','k');
%         text(ax,ien-3, 0.66*yma, num2str(off12(nd)),'fontsize',8,'color','b');
    elseif option == 2  % 2. HF aligned independently with the largest dipole only
        plot(ax,wave1bba(winlen*(nd-1)+1:winlen*nd,1),wave1bba(winlen*(nd-1)+1:winlen*nd,2),'r','linew',0.6);
        plot(ax,wave2bba(winlen*(nd-1)+1:winlen*nd,1),wave2bba(winlen*(nd-1)+1:winlen*nd,2),'b','linew',0.6);
        plot(ax,wave3bba(winlen*(nd-1)+1:winlen*nd,1),wave3bba(winlen*(nd-1)+1:winlen*nd,2),'k','linew',0.6);
        is = wave1bba(winlen*(nd-1)+1,1);
        ien= wave1bba(winlen*nd,1);
%         text(ax,is+3, 0.66*yma, num2str(offbba13(nd)),'fontsize',8,'color','k');
%         text(ax,ien-3, 0.66*yma, num2str(offbba12(nd)),'fontsize',8,'color','b');

        % offset: LF-HF, the values should be close to the filtering effect in theory, but
        % may be subject to the sampling rate used, a higher sps is preferred, but is limited
        % to the memory
%         if nd<=2
            text(ax,is+3, 0.66*yma, num2str((off13(nd)-offbba13(nd))/sps),'fontsize',8,'color','k');
            text(ax,ien-3, 0.66*yma, num2str((off12(nd)-offbba12(nd))/sps),'fontsize',8,'color','b');
%         end
    end

    plot(ax,[is+1/hibb is+1/lobb],[-0.8*yma -0.8*yma],'k','linew',3)
    axis(ax,[is ien -yma yma]);
%     set(ax,'XTick',[is (is+ien)/2],'fontsize',8);
%     set(ax,'XTick',floor(is):1:floor(ien),'fontsize',8);
    set(ax,'XTick',is:1:ien,'fontsize',8);
    ax.XTickLabel = {num2str(floor(is)),[],[],[],[],[],[],[],num2str(floor(is)+8),[],[],[],[],[],[],[]};

    xlabel(ax,xlabstr{nd},'fontsize',9);
%     ax.XMinorTick='on';
    hold(ax,'off');
...
keyboard
if option == 1  % 1. HF aligned with same offset as LF
    print(f.fig,'-dpdf','/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figure/waveforms_v2.pdf');
elseif option == 2  % 2. HF aligned independently with the largest dipole only
    print(f.fig,'-dpdf',strcat('/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figure/waveforms_hfalignarrind_',...
          num2str(sps),'sps.pdf'));
end
