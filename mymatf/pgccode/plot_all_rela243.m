...
SUFFIXhf = strcat('up.hf.count.',num2str(winlenhf),'_',num2str(winlenlf),'.',num2str(lofflf),...
                  '.',num2str(ccminlf));
...
SUFFIXlf = strcat('lf.count.',num2str(winlenhf),'_',num2str(winlenlf),'.',num2str(lofflf),...
                  '.',num2str(ccminlf));
...
loccont = hfmaplocall(ind,1:3);     % location of fam 243
if isempty(loccont)
    loccont = [-123.549500 48.540833 38.5600];
end

% also take location of fam 002 as a reference
lon0 = -123.5850;
lat0 = 48.4367;

relacont = loccont;
[dx, dy] = absloc2relaloc(loccont(:,1),loccont(:,2),lon0,lat0);
relacont(:,1) = dx;
relacont(:,2) = dy;

% convert the map location to relative ones to fam 002, 
