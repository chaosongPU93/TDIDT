function [STAEuse,STANuse,STAUuse,STAtuse,STAEpsig,STANpsig,STAUpsig,STAtpsig,...
    ...
    rdhinetevt_newdata(sacpath,stainfo,evt,sps,lo,hi,npo,npa,wlensec,wlensecbf,pltflag,velmod)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
%       2. this function to read the data that newly downloaded, and specific to each
%           event query which is not one-day data, 30 min instead.
%
%
...
%     staufnm = strcat(stnm(i,:),'.U');
    ...
    % the new downloaded data is specific to each event, so the path is different
    if evt(5) < 10
        hrstr = strcat('0',num2str(evt(5)));
    else
        hrstr = num2str(evt(5));
    end
    
    if evt(6) < 10
        mistr = strcat('0',num2str(evt(6)));
    else
        mistr = num2str(evt(6));
    end
    
    subdirnm = strcat(dirnm,hrstr,mistr);
    staepath = fullfile(sacpath,dirnm,subdirnm,'SAC',staefnm);
    stanpath = fullfile(sacpath,dirnm,subdirnm,'SAC',stanfnm);
    staupath = fullfile(sacpath,dirnm,subdirnm,'SAC',staufnm);
...
datast = evt(1,5)*3600+evt(1,6)*60 - 10*60;     % absolute start time of data
dataet = evt(1,5)*3600+evt(1,6)*60 + 20*60;     % absolute end time of data

% convert the current relative time vector to absolute time
time = time + datast; 

otsec = evt(1,5)*3600+evt(1,6)*60+evt(1,7);     % abs. origin time of event
otind = find(abs(otsec-time)<=dt,1,'first');    % abs. origin time index
if isempty(otind)
    return
end
