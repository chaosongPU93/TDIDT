...
%% for boundary 2 events, calculate the piercing points of events on the slab interface 
recalflag = 0;
dslab = 0.002;
evtsel = evtbnd2;
stasel = stasel2;
...
% velmod = 'ukawa83';
velmod = 'ak135';
rayinfo2 = evt_piercing_point(regflag,recalflag,slab,dslab,evtsel,stasel,kdense,velmod);
ttsevt2 =rayinfo2.tts;
raypevt2 = rayinfo2.rayp;
distevt2 = rayinfo2.dist;
pierptevt2 = rayinfo2.pierpt ;
poorresevt2 = rayinfo2.poorres;
edistevt2 = rayinfo2.edist ;
...
%        'N.OHCH';
%        'N.ISIF';
%        'N.TOKH'];     % these seems good for bound1
...
sta = ['N.MISH';
       ...
       'N.KWBH';
       'N.TBEH';
       'N.SJOH'];       % this seems good for bound2
% sta = 'N.SINH'; % IKWH      
...
nsta = size(pierptevt2,1);
nevt = size(pierptevt2,2);

pierpt(:,:) = pierptevt2(stanum,:,:);
...
% 
% f.ax = gca; 
% f.ax = plt_tmr_evt_pier_point3D(f.ax,regflag,slab,obara,pierpt,stasel2(stanum));
% 
% title(f.ax,strcat(prefix,'---',stasel2(stanum).nm,'---',velmod));
% 
% % save figure
% savefig(f.fig,strcat(figpath,'/tmr_pierpt_3dview.',prefix,'_',stasel2(stanum).nm,...
%         '_',velmod,'.fig'));
...
[f1,f2,f3,f4,f5]=tmr_pierpt_dist_onslab(regflag,slab,obara,evtsel,pierpt,stasel(stanum),range);
