function [f1,f2]=plt_tmr_pierpt_dist_diff(regflag,tmr,evt,sta,diffdist)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
colormap(f1.ax,'jet');
c=colorbar(f1.ax);
c.Label.String = strcat('difference in min dist to tremors via knnsearch (km)');
c.Label.FontSize = 12;
caxis(f1.ax,[-0.5 0.5]);
...
h = histogram(f2.ax,diffdist,'binwidth',0.1,'normalization','probability',...
              'facecolor','k','facea',0.5);
perc = sum(abs(diffdist)<=0.5)/size(diffdist,1);
% for i = 1: length(h.Values)
%     text(f2.ax,h.BinEdges(i)+0.1,h.Values(i),sprintf('%.2f',h.Values(i)));
% end
text(f2.ax,0.2,0.95,num2str(size(diffdist,1)),'fontsize',13,'unit','normalized');
text(f2.ax,0.4,0.95,sprintf('%.2f',perc),'fontsize',13,'unit','normalized');
xlim(f2.ax,[-0.5 0.5]);
% ylim(f2.ax,[0 0.22]);
xlabel(f2.ax,'Differential distance (km)');
