function [evtall,eventall] = Jmacatlogread(regflag,depran,slab)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%       eventall: a complete struct array of the events
%
%
...
for i = 1: len
    ...
    for i = 1:len
        ...
        if isinrect(i)
            ...
            if depran == inf
                logicdep = dep(i)>=depslab;
            else
                logicdep = dep(i)>=depslab && dep(i)<=depslab+depran;
            end
            ...
            if logicdep
                ...
                evtpercat = [evtpercat; evt];
                
                eventpercat = [eventpercat; event];
            end
        end
    end
    evtall = [evtall; evtpercat];
    eventall = [eventall; eventpercat];
    
end

% save the matrix to a matfile, in case of next use
save(strcat(evtpath,'/regevt',prefix,'dep',num2str(depran),'.mat'),'evtall');

% save the complete struct to a matfile, in case of next use
save(strcat(evtpath,'/regevtstruct',prefix,'dep',num2str(depran),'.mat'),'eventall');
