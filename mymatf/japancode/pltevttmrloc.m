function ax = pltevttmrloc(ax,regflag,slab,evtobj,tmrobj,minmag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
p1=scatter(ax,tmrobj(:,6),tmrobj(:,7),6,[1 0.2 0.2],'filled','o','MarkerEdgeColor',...
           [1 0.2 0.2]);
p2=scatter(ax,evtobj(:,8),evtobj(:,9),8,[0.2 0.2 1],'filled','o','MarkerEdgeColor',[0.2 0.2 1]);
p3=scatter(ax,evtobj(evtobj(:,11)>=3,8),evtobj(evtobj(:,11)>=3,9),35,...
        [0.8 0.8 1],'filled','o', 'MarkerEdgeColor',[0 0 0]);
p4=scatter(ax,evtobj(evtobj(:,11)>=4,8),evtobj(evtobj(:,11)>=4,9),40,...
        [0 0 0],'filled','o', 'MarkerEdgeColor',[0 0 0]);

% p1 = scatter(ax,obara(1,6),obara(1,7),8,[1 0 0],'filled','o','MarkerEdgeColor',[1 0 0]);
% p2 = scatter(ax,evtobj(1,8),evtobj(1,9),8,[0 0 1],'filled','o','MarkerEdgeColor',[0 0 1]);
% p3 = scatter(ax,evtobj(evtobj(:,11)>=3,8),evtobj(evtobj(:,11)>=3,9),evtobj(evtobj(:,11)>=3,11)+15,...
%         [0.5 0.5 1],'filled','o', 'MarkerEdgeColor',[0 0 0]);
% p4 = scatter(ax,evtmagm(8),evtmagm(9),30,[0 0 0],'filled','o','MarkerEdgeColor',[0 0 0]);

legend(ax,[p1,p2,p3,p4],{ strcat('Tremors, n=',num2str(size(tmrobj,1))), ...
       strcat('Regular >=',num2str(minmag),', n=',num2str(size(evtobj,1))),...
       strcat('Regular >=3, n=',num2str(sum(evtobj(:,11)>=3))),...
       strcat('Regular >=4, n=',num2str(sum(evtobj(:,11)>=4))) },...
       'location','southeast');
ax.Box = 'on';
grid(ax, 'on');
axis(ax, 'equal');
