...
text(ax,0.95,0.08,'TWKB','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2,...
    'horizontalalignment','right');
hold(ax,'off');
...
text(f.ax(2),0.96,0.15,strcat({'LF leads'}),'fontsize',11,'unit','normalized',...
     'horizontalalignment','right');
[mu,sigma,muCI,sigmaCI] = normfit(vdistelse, 1-conf/100,[], wtelse);
% xx = mu2+ax.XLim(1): 0.01: mu2++ax.XLim(2);
xx = -20:0.01:20;
synpdf = normpdf(xx,mu,sigma);
synct = synpdf;      % synthetic counts
plot(ax,xx, synct, 'k-', 'linew', 1.5);
text(ax, 0.05,0.62,strcat({'$\mu$= '},sprintf('%.2f',mu),' km'),'FontSize',12,...
     'unit','normalized','Interpreter','latex','color','k','horizontalalignment','left');
text(ax, 0.05,0.55,strcat({'$\sigma$= '},sprintf('%.2f',sigma),' km'),'FontSize',12,...
     'unit','normalized','Interpreter','latex','color','k','horizontalalignment','left');
