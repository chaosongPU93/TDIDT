% function mig_prop_direct_LZB
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script is used to find the best propagation direction for migration
%
% Chao Song, chaosong@princeton.edu
% First created date:   2019/12/02
% Last modified date:   2019/12/02
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% for i = 41: 63
    ...
    for iang = 1: length(angle)
        ...
        % linear robust least square
        [fitobj,gof,~] = fit(miglfdum(:,14)/3600, miglfdum(:,1),fttpfree,'Robust','Bisquare',...
                                  'StartPoint',[0.5 5]);
        % output fit parameters
        coef = coeffvalues(fitobj);
        slopelf(iang) = coef(1);
        fitproplf = feval(fitobj,miglfdum(:,14)/3600);
    ...
    angbesthf(i) = median([angbestl3(i),angbestl4(i),angbestl5(i)]);
    ...
    angbestl5(i) = angle(ind(ind5(1)));
    angbestlf(i) = median([angbestl3(i),angbestl4(i),angbestl5(i)]);
    
    % determine if more inspection is needed
    if abs(angbesthf(i)-angbestlf(i)) > 20
