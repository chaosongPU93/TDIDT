...
% angles from scratch, following the method illustrated in NOTES. This
% does not rely on which trio detector you want to focus on, because it
% just takes all data at any station you want, depending on fam.
% But if you want the historical fam 002 for PGC trio by reading in the 
% old days and old LFE catalog, you need to use 'calc_rots_noresp_pgc.m'
%
%
% INPUT:
%   fam: fam, string
...
figure
ista = 4;
plot(stackE(ista,:)*10 + 1, 'b-'); hold on
plot(stackN(ista,:)*10 + 3, 'r-');
text(1050,0.8,'LZB E','FontSize',10);
text(1050,2.8,'LZB N','FontSize',10);
...
ax = gca;
plot([1192 1192], ax.YLim, 'b--');
plot([1198 1198], ax.YLim, 'r--');
...
%         end
    ...
    figure   % plot individual correction results, fig 2~nsta+1 (2-8)
...
figure  % fig10, plot the shorter normalized split corrected seismogram before shifting between different stations
for ista = 1:nsta
...
for ista = 1: nsta
    ...
    [maxcoef, idx] = max(coef);
%     [maxcoef, idx] = max(abs(coef));    % in case the polarity is different
    lagsamp = lag(idx);
...
figure
ista = 4;
plot(STAoptlongmat(ista,:)*10 + 1, 'b-'); hold on
plot(STAortlongmat(ista,:)*10 + 3, 'r-');
text(1050,0.8,'LZB opt','FontSize',10);
text(1050,2.8,'LZB ort','FontSize',10);
text(0.05,0.95,'Velocity','Units','normalized','FontSize',12);
text(0.95,0.95,sprintf('Bandpassed by %.1f-%.1f Hz',lo,hi),'Units','normalized','FontSize',12,...
  'HorizontalAlignment','right');
xlabel('Samples at 40 Hz');
