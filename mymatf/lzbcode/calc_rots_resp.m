...
% ista=1;
        ...
        if maxlag >= 0   % if staslow is indeed slower 
            ROTS(ista, 1) = maxlag;
        else
            ROTS(ista, 1) = -maxlag;
        end
        
        if maxsplit == 0   % if splitting is not prominent
            ROTS(ista, 2) = 90;
        else
...
%         figure   % plot individual correction results
        ...
        STAfastslow = STA * exp(-1i * maxsplit*pi/180);
        STAslow = real(STAfastslow);
        STAfast = imag(STAfastslow);
...
%         plot(STAfast/max(STAfast) + 2, 'linewidth', 2); hold on
        ...
        STAslow(off: seglen-off) = STAslow(off+ maxlag: seglen- off+ maxlag); 
        STAsplitcorrected = (STAslow+ 1i * STAfast) * exp(1i* maxsplit*pi/180);
...
%         plot(STAort + 6, 'linewidth', 2); hold on
        ...
        STAfastslowlong = STAlong * exp(-1i * maxsplit*pi/180);
        STAslowlong = real(STAfastslowlong);
        STAfastlong = imag(STAfastslowlong);
        
        STAslowlong(off: templen-off) = STAslowlong(off+ maxlag: templen- off+ maxlag);
        STAsplitcorrectedlong = (STAslowlong+ 1i * STAfastlong) * exp(1i* maxsplit*pi/180);
        STAscrotlong = STAsplitcorrectedlong* exp(-1i* maxpolar*pi/180);
        STAoptlong = real(STAscrotlong);
        STAortlong = imag(STAscrotlong);
        STAoptmat(ista,:) = STAoptlong;
        STAortmat(ista,:) = STAortlong;
    ...
    for ista = 1: nsta
        ampmax = max(STAoptmat(ista, mid-5*sps+1: mid+5*sps));
        ampmin = min(STAoptmat(ista, mid-5*sps+1: mid+5*sps));
    ...
    for ista = 1: nsta
        ...
        lagmat(ista) = lagsamp;
        ROTS(ista,4) = lagmat(ista);
    end

    figure      % plot normalized final optimal results
    for ista = 1: nsta
        plot(STAoptnorm(ista, mid-5*sps+ROTS(ista,4): mid+5*sps+ROTS(ista,4)) + 1* ista, 'linewidth', 2); hold on
        text(50,1*ista,stas(ista,:));
    end
    
    %%% check if the orthogonal component is indeed very small
    figure  % plot individual optimal and orthogonal components
    for ista = 1: nsta
        subplot(1,nsta,ista)
        plot(STAoptmat(ista, mid-2*sps+ROTS(ista,4): mid+2*sps+ROTS(ista,4)), 'k-', 'linewidth', 2); hold on
        plot(STAortmat(ista, mid-2*sps+ROTS(ista,4): mid+2*sps+ROTS(ista,4)), 'r-', 'linewidth', 2); hold on
        text(50,0,stas(ista,:));
    end
