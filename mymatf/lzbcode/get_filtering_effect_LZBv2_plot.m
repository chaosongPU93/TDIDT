...
text(0.7,0.9,'1.25-6.5 Hz','fontsize',14,'unit','normalized');
...
text(0.7,0.9,'0.5-1.25 Hz','fontsize',14,'unit','normalized');
...
%     text(ax,100,1*ista-0.3,stas(ista,:),'fontsize',9);
    plot(ax,(1:10*sps+1)/sps,stackno(ista, mid-5*sps: mid+5*sps) + 1* ista, 'linewidth', ...
        0.7,'color',color(ista));
    text(ax,100/sps,1*ista-0.3,stas(ista,:),'fontsize',9);
end
...
%          color(ista));
%     text(ax,100,1*ista-0.3,stas(ista,:),'fontsize',9);
    plot(ax,(1:10*sps+1)/sps,stackhf(ista, mid-5*sps: mid+5*sps) + 1* ista, 'linewidth',... 
         0.7,'color',color(ista));
    text(ax,100/sps,1*ista-0.3,stas(ista,:),'fontsize',9);
end
text(ax,0.95,0.9,'1.25-6.5 Hz','fontsize',10,'unit','normalized','Horizontalalignment','right');
...
%          color(ista));
%     text(ax,100,1*ista-0.3,stas(ista,:),'fontsize',9);
    plot(ax,(1:10*sps+1)/sps,stacklf(ista, mid-5*sps: mid+5*sps) + 1* ista, 'linewidth',... 
         0.7,'color',color(ista));
    text(ax,100/sps,1*ista-0.3,stas(ista,:),'fontsize',9);
end
text(ax,0.95,0.9,'0.5-1.25 Hz','fontsize',10,'unit','normalized','Horizontalalignment','right');
...
%%%%%%%%%%%%%%%%%%%%%%%%%% for testing the frequency response of the filter %%%%%%%%%%%%%%%%%%%%%%%%
    ...
    lohf = 1.25;
    hihf = 6.5;
    [~,~,~,~,~,HABS2a,F,H] = Bandpass(stack(ista,:), sps, lohf, hihf, 2, 1, 'butter');
    [~,~,~,~,~,HABS2b,F,H] = Bandpass(stack(ista,:), sps, lohf, hihf, 2, 2, 'butter');
    [~,~,~,~,~,HABS2c,F,H] = Bandpass(stack(ista,:), sps, lohf, hihf, 4, 2, 'butter');
%     [~,~,~,~,~,HABS2d,F,H] = Bandpass(stack(ista,:), sps, lohf, hihf, 2, 4, 'butter');
...
%     plot(ax,F,decibel(HABS2),'b-','linew', 2); hold on
    p1=plot(ax,F,pow2db(HABS2a),'k--','linew', 1);  hold on
%     plot(ax,F,20*log10(abs(H)),'k--','linew', 1);
    p2=plot(ax,F,pow2db(HABS2b),'k-','linew', 1);  hold on
    p3=plot(ax,F,pow2db(HABS2c),'k-.','linew', 1);  hold on
%     plot(ax,F,pow2db(HABS2d),'c-.','linew', 1);  hold on
%     ylim(ax,[-40 0]);
%     xlim(ax,[1e-1 20]);
    xlim(ax,[1e-2, 1e2]);
    ylim(ax,[-100,0]);
    plot(ax,ax.XLim, [-3 -3], 'r-','linew', 1);
    plot(ax,[lohf lohf],ax.YLim, '--','color',[0.7 0.7 0.7],'linew', 1);
    plot(ax,[hihf hihf],ax.YLim, '--','color',[0.7 0.7 0.7],'linew', 1);
    text(ax,0.05,0.6,sprintf('Butterworth: %.2f-%.2f Hz',lohf,hihf),'unit','normalized');
%     text(ax,0.05,0.5,strcat({'Orders/Poles: '},num2str(npo)),'unit','normalized');
%     text(ax,0.05,0.4,strcat({'Passes: '},num2str(npa)),'unit','normalized');
    legend(ax,[p1,p2,p3],'2 poles, 1 pass','2 poles, 2 passes','4 poles, 2 passes');
...
text(0.04,0.7,'1.25-6.5 Hz','FontSize',12,'unit','normalized');
...
ax = f.ax(4);
hold(ax, 'on');
p1=plot(ax,(-mshift:mshift)/sps,sumstack12n,'r-','linewidth', 0.7);
p2=plot(ax,(-mshift:mshift)/sps,sumstack13n,'b-','linewidth', 0.7);
p3=plot(ax,(-mshift:mshift)/sps,sumstack32n,'k-','linewidth', 0.7);
p4=plot(ax,imaxstack12cent/sps,sumstack12n(imaxstack12),'ro','markersize',5);
p5=plot(ax,imaxstack13cent/sps,sumstack13n(imaxstack13),'b^','markersize',5);
p6=plot(ax,imaxstack32cent/sps,sumstack32n(imaxstack32),'ks','markersize',5);
% plot(xmaxstack12n,ymaxstack12n,'bs','markersize',10);
% plot(xmaxstack13n,ymaxstack13n,'ro','markersize',10);
% plot(xmaxstack32n,ymaxstack32n,'kd','markersize',10);
p7=plot(ax,xmaxstack12ntmphf/sps,sumstack12n(xmaxstack12ntmphf+mshift+1),'ro','markersize',2,...
        'markerfacec','r');
p8=plot(ax,xmaxstack13ntmphf/sps,sumstack13n(xmaxstack13ntmphf+mshift+1),'b^','markersize',2,...
        'markerfacec','b');
p9=plot(ax,xmaxstack32ntmphf/sps,sumstack32n(xmaxstack32ntmphf+mshift+1),'ks','markersize',2,...
        'markerfacec','k');
...
xlabel(ax,strcat('Lag time (s)'),'fontsize',10);
ylabel(ax,'Normalized CC Coefficient','fontsize',10);
xlim(ax,[-20/sps 20/sps]);
ylim(ax, [-1,1]);
ax.Box='on';
grid(ax, 'on');
ax.GridLineStyle = '--';
...
text(0.04,0.7,'0.5-1.25 Hz','FontSize',12,'unit','normalized');
text(0.5,0.2,'Cross-Correlation','FontSize',12,'unit','normalized','HorizontalAlignment','center');
xlim([-20,20]);
box on;
grid on;
...
% plot(xmaxstack13n,ymaxstack13n,'ro','markersize',10);
% plot(xmaxstack32n,ymaxstack32n,'kd','markersize',10);
p7=plot(ax, xmaxstack12ntmplf/sps,sumstack12n(xmaxstack12ntmplf+mshift+1),'ro','markersize',2,...
        'markerfacec','r');
p8=plot(ax, xmaxstack13ntmplf/sps,sumstack13n(xmaxstack13ntmplf+mshift+1),'b^','markersize',2,...
        'markerfacec','b');
p9=plot(ax, xmaxstack32ntmplf/sps,sumstack32n(xmaxstack32ntmplf+mshift+1),'ks','markersize',2,...
        'markerfacec','k');
...
% keyboard
% print(f.fig,'-depsc2',strcat('/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figures/filtereffect_',...
%       fam,'.eps'));
