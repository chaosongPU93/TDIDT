...
%   6. Use the days the same as in Peng et al. 2015 instead of different days
%       that bond to different fams;
%   7. Add the R_o/o ratio as another constaint to weed out false positives.
%
% Similar to hf_PGCtrio_detection_check_v2.m and v3
%
% Difference between hf_LZBtrio_detection_check_v2:
...
%   2. From Figure 3 in Peng et al. 2015, it seems that R_o/o is a powerful constaint
%       to weed out false positives, so implement this as illustrated in Peng et al. 2015
%   3. Comment lines that seems inrelevant to current analysis, no wiggles plot as well
%
% NOTES:
...
elseif isequal(fam,'001')
    tempoffs = [1171 1171 1171];
elseif isequal(fam,'019')
    tempoffs = [1194 1194 1194];    
elseif isequal(fam,'045')
    tempoffs = [1179 1179 1179];
else
    tempoffs = [1179 1179 1179];
end
...
%         end
    ...
    %
    xmaxSTA12n=xmaxSTA12n-mshift-1;     % note it is from parabolic fit
...
%%%%%%%%%% END, USE THIS SECTION IF USING UPSAMPLING %%%%%%%%%%%%%%%%%%%%%%% 
            ...
            if abs(iSTA12bang-imaxinterpSTA12) <= loopoffmax*iup && ...
               ...
               3*xcmaxAVEnmin
                ...
                tmp12 = (imaxinterpSTA12-(iup*mshift+1))/iup;
                tmp13 = (imaxinterpSTA13-(iup*mshift+1))/iup;
                tmp32 = (imaxinterpSTA32-(iup*mshift+1))/iup;
                ...
                sigma12(nin+1) = abs(iSTA12bang-imaxinterpSTA12)/iup;
                sigma13(nin+1) = abs(iSTA13bang-imaxinterpSTA13)/iup;
...
%                         %%% So, isdiff >= istart >= igstart, isdiff and iediff are global indexes 
%                         isdiff=istart+idiff; %Start of strongest 0.5s, DELETE -1 by Chao, 2019/02/17, see NOTES
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
                        ...
                        %%% 26 cols, 2020/08/27
                        mapfile(nin,:)=[timswin(n) xmaxSTA13ntmp(n) xmaxSTA12ntmp(n) ...
                            xcmaxAVEnbang(nin) loopoff(n) Ampsq(nin) cumsumtrdiff ...
                            ...
                            sumsSTA13n(n,ceil(iSTA13bang/iup)) sumsSTA32n(n,ceil(iSTA32bang/iup)) ...
    ...
    fid = fopen([rstpath,'/MAPS/map_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
    ...
    fid = fopen(strcat(rstpath, '/MAPS/trace_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                       num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/trace1day_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),...
...
%         found=found
    ...
    ioff=zeros(nstanew,nin);    % this is a approximate offset between 4th sta and 1st sta
    ...
    for istanew=1:nstanew   % 1st, for each additional station
        ...
        for n=1:nin     % 2nd, for each trio detection 
            ...
            % original 3 should be close enough, icc1~=icc2~=icc3, i.e. all differential shifts
            % should be close to 0 theoratically, icc12~=icc13~=icc23~=0. That is why it is a
            ...
            ioff(istanew,n)=round((cc1max * lag1(icc1) + ...
                                   cc2max * lag2(icc2) + ...
                                   cc3max * lag3(icc3)) / ...
                                   (cc1max+cc2max+cc3max));   % this is a approximate offset between 4th sta and 1st sta
            %%% NOTE:
            ...
            if abs(icc12(istanew,n))<=offindthres && abs(icc13(istanew,n))<=offindthres && ...
                ...
                % because ioff is offset between 4th sta and 1st sta, the following is to align them 
...
%         elseif ifig <= 99
%             print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','add_up','_',numcd 2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(ifig),'.eps'])
%         else
