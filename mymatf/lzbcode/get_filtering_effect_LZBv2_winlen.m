% function get_filtering_effect_LZBv2_winlen
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This is the function to get the filtering effect under different frequency
% band for LZB trio.
%
% modified from 'get_filtering_effect_LZBv2.m', in order to see how the
% filtering effect would vary with the window length that was actually used
% during CC between the templates. The original template is 60-s long, and 
% the window is centered roughly at the arrival of the dipole
%
% Use the Bostock's catalog in one file
...
wlensechf = 2:1:6;
wlenhfarr = wlensechf.*sps;
filthf = nan(nfam,length(wlenhfarr),2);

wlenseclf = 4: 4: 32;
wlenlfarr = wlenseclf.*sps;
filtlf = nan(nfam,length(wlenlfarr),2);
...
%     wlensechf = 4;
    ...
    for iwin = 1: length(wlenhfarr)
        wlenhf = wlenhfarr(iwin);
    istart = mid-wlenhf/2;
%     disp(istart)
...
%     filthf = [xmaxstack12ntmphf;  xmaxstack13ntmphf];
    filthf(ifam, iwin, :) = [xmaxstack12ntmphf;  xmaxstack13ntmphf];
...
%     wlenseclf = 16;
    ...
    for iwin = 1: length(wlenlfarr)
        wlenlf = wlenlfarr(iwin);
    istart = mid-wlenlf/2;
...
%     filtlf = [xmaxstack12ntmplf;  xmaxstack13ntmplf];
    filtlf(ifam, iwin,:) = [xmaxstack12ntmplf;  xmaxstack13ntmplf];
...
for ifam = 1: nfam
    ...
    for ii = 1: size(flf,1)
        for jj = 1: size(fhf,1)
            f12(kk) = flf(ii,1) - fhf(jj,1);
            kk = kk +1;
        end
    end
    p(ifam)=plot(f12, 'o-','linew',1.5);
end
% legend(p,nfampool,'location','northwest');
title('filtering effect (LF-HF) using diff. comb. in 12 direction');
...
for ifam = 1: nfam
    ...
    for ii = 1: size(flf,1)
        for jj = 1: size(fhf,1)
            f13(kk) = flf(ii,2) - fhf(jj,2);
            kk = kk +1;
        end
    end
    p(ifam)=plot(f13, 'o-','linew',1.5);
end
legend(p,nfampool,'location','northwest');
title('filtering effect (LF-HF) using diff. comb. in 13 direction');
...
for ifam = 1: nfam
    fhf(:,:) = filthf(ifam, :, :);
    p(ifam)=plot(fhf(:,1), 'o-','linew',1.5);
end
% legend(p,nfampool,'location','northwest');
title('filtering effect HF using diff. comb. in 12 direction');
...
for ifam = 1: nfam
    fhf(:,:) = filthf(ifam, :, :);
    p(ifam)=plot(fhf(:,2), 'o-','linew',1.5);
end
legend(p,nfampool,'location','northwest');
title('filtering effect HF using diff. comb. in 13 direction');
...
set(f.ax(2), 'position', [ 0.1, 0.08, 0.85, 0.4]);
set(f.ax(1), 'position', [ 0.1, 0.55, 0.85, 0.4]);
...
for ifam = 1: nfam
    flf(:,:) = filtlf(ifam, :, :);
    if ifam <= sizec
        p(ifam)=plot(ax,wlenseclf,flf(:,1),cpool(ifam,:),'LineStyle','-','linew',0.8,'markers',5,...
                     'marker',spool(1,:),'markerfacec',cpool(ifam,:),...
                     'markeredgec',cpool(ifam,:));
    elseif ifam <= sizec*2
        p(ifam)=plot(ax,wlenseclf,flf(:,1),cpool(ifam-sizec,:),'LineStyle','-','linew',0.8,...
                     'markers',4,...
                     'marker',spool(2,:),'markerfacec',cpool(ifam-sizec,:),...
                     'markeredgec',cpool(ifam-sizec,:));
    else
        p(ifam)=plot(ax,wlenseclf,flf(:,1),cpool(ifam-sizec*2,:),'LineStyle','-','linew',0.8,...
                     'markers',7,...
...
legend(p,nfampool,'location','best');
% ylabel(ax,'Differential travel time between TWKB-LZB in LF');
% title(ax,'Differential travel time between station pairs in LF passband');
ylabel(ax,'Diff. time between TWKB-LZB (sample)','fontsize',10);
xlim(ax,[4 40]);
xticks(ax, 4:4:40);
hold(ax, 'off');
...
for ifam = 1: nfam
    flf(:,:) = filtlf(ifam, :, :);
    if ifam <= sizec
        p(ifam)=plot(ax,wlenseclf,flf(:,2),cpool(ifam,:),'LineStyle','-','linew',0.8,'markers',4.5,...
                     'marker',spool(1,:),'markerfacec',cpool(ifam,:),...
                     'markeredgec',cpool(ifam,:));
    elseif ifam <= sizec*2
        p(ifam)=plot(ax,wlenseclf,flf(:,2),cpool(ifam-sizec,:),'LineStyle','-','linew',0.8,...
                     'markers',4,...
                     'marker',spool(2,:),'markerfacec',cpool(ifam-sizec,:),...
                     'markeredgec',cpool(ifam-sizec,:));
    else
        p(ifam)=plot(ax,wlenseclf,flf(:,2),cpool(ifam-sizec*2,:),'LineStyle','-','linew',0.8,...
                     'markers',7,...
                     'marker',spool(3,:),'markerfacec',cpool(ifam-sizec*2,:),...
                     'markeredgec',cpool(ifam-sizec*2,:));
    end
...
legend(p,nfampool,'location','best');
% ylabel(ax,'Differential travel time between TWKB-MGCB in LF (sample)');
ylabel(ax,'Diff. time between TWKB-MGCB (sample)','fontsize',10);
xlabel(ax,'Window length for CC (s)','fontsize',10);
xlim(ax,[4 40]);
xticks(ax, 4:4:40);
hold(ax, 'off');

print(f.fig,'-dpdf',...
      strcat('/home/data2/chaosong/CurrentResearch/Song_Rubin_2020/Figure/filtereffect_wlen',...
      '.pdf'));
