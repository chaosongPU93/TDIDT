...
%   5. Adding the option to use data without station response
%   6. Use the days and rots para from Chao's code: GetDays4Stack, GetRotsChao
%
% Similar to hf_PGCtrio_detection_check_v2.m and v3
%
% Difference between hf_LZBtrio_detection_check: upsample the detection results
%   4 times to 160 sps base
%
% NOTES:
...
%   4/1/1.5/0.4/29; 4/1/1.5/0.4/14;
%
% Modified by Chao Song, chaosong@princeton.edu
% First created date:   2019/11/22
...
for ifam = 3: length(nfampool)
fam = nfampool(ifam, :)
%     fam='017';     % family number
...
stas=['TWKB '
      'LZB  '
      'MGCB '];     % determine the trio and order, here the 1st sta is TWKB
...
npa=2;
mshift=14; %19; %maximum shift for the x-correlations. 19 for 002 Stanford,    % in sps, 0.5s*40sps=20
...
%     MO = 'SEP';
    ...
    IDENTIF=[YEAR,'.',JDAY,'.',fam,'.lo',num2str(loopoffmax),'.cc',num2str(xcmaxAVEnmin),'.',int2str(npo),int2str(npa),'.ms', int2str(mshift)]
...
%%%%%%%%%%%%% START, USE THIS SECTION IF USING UPSAMPLING %%%%%%%%%%%%%%%%%%%%%%%            
            ...
            for iSTA12=max(1,imaxinterpSTA12-3*iup):min(imaxinterpSTA12+3*iup,iup*(2*mshift+1)-(iup-1))
               ...
               interpSTA12n(iSTA12bang)+interpSTA13n(iSTA13bang)+interpSTA32n(iSTA32bang) >= 3*xcmaxAVEnmin
                ...
                if abovecut > 0.9*belowcut %-1 %This checks for frequency range; make sure it's not too narrow?
                    ...
                    if isdiff > winlen+mshift+offset   % >winsec*sps+19+10
                        ...
                        dummy(1,:)=STAopt(1,mshift:isdiff-1-offset);   % 19: isdiff-1    % MIGHT this part be a mistake? no symmetry, with offset, add -offset by Chao 20190901
                    ...
                    %CC in following 1.25 seconds, with 2*offset, editted by Chao 20190902, in case that Index in position 2 exceeds array bounds  
...
%                         ccprior125(nin) ccprior(nin) ccpost125(nin) STAamp(nin,2) STAamp(nin,3) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
                    ...
                    mapfile(nin,:)=[timswin(n) xmaxSTA13ntmp(n) xmaxSTA12ntmp(n) xcmaxAVEnbang(nin)...
                                    ...
                                    STAamp(nin,2) STAamp(nin,3) sumsSTA12n(n,ceil(iSTA12bang/iup))...
                                    sumsSTA13n(n,ceil(iSTA13bang/iup)) ...
    ...
    fid = fopen([rstpath,'/MAPS/map_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'],'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/trace_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
    ...
    fid = fopen(strcat(rstpath, '/MAPS/trace1day_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps'),'w+');
...
%     plot(timswin,xmaxSTA32ntmp,'k*','MarkerSize',2);
    ...
    print('-depsc',[rstpath,'/FIGS/up',IDENTIF,'_',num2str(winlen/sps),'s_',num2str(sps),'sps_',num2str(lo),'-',num2str(hi),'b.eps'])
    close(101)
...
%     %%% AND, xcmaxAVEnbang is more reasonable to save 
    ...
    title([IDENTIF,'_{',num2str(lo),'-',num2str(hi),'}'])
    print('-depsc',[rstpath,'/FIGS/up',IDENTIF,'_',num2str(winlen/sps),'s_',num2str(sps),'sps_',num2str(lo),'-',num2str(hi),'e.eps'])
    close(102)
...
%         plot(icc3(istanew,:)-mshiftnew-1, cc3max(istanew,:), 'k+','Markersize',8);
    ...
    fid = fopen([rstpath,'/MAPS/mapadd_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'],'w+');
    if nstanew == 3  
    ...
    fid = fopen(strcat(rstpath, '/MAPS/traceadd_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps', num2str(nstanew), 'nsta'),'w+');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 2 start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%% Write results into files
    fid = fopen([rstpath,'/MAPS/mapallrst_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'nsta'],'w+');
    if nstanew == 3
    ...
    fid = fopen(strcat(rstpath, '/MAPS/traceall_up',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s',num2str(sps),'sps',num2str(nstanew), 'nsta'),'w+');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WAY 2 end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for ifig=1:floor(nin_new/(nrow*mcol))+1
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','add_up','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','add_up','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',[rstpath,'/WIGS/',IDENTIF,'_','add_up','_',num2str(lo),'-',num2str(hi),'_',num2str(winlen/sps),'s_',num2str(sps),'sps','.',int2str(ifig),'.eps'])
        end
