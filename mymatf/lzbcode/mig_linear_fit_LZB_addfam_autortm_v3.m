...
%%% reposition
set(f123.ax(1), 'position', [ 0.07, 0.1, 0.5, 0.75]);
set(f123.ax(2), 'position', [ 0.62, 0.15, 0.35, 0.75]);
...
for i = 1: size(trange,1)
    ...
    drawArrow(ax,xvect,yvect,xran,yran,'linewidth',0.6,'linestyle','-','color',[0.7 0.7 0.7],...
              'HeadLength', 6, 'HeadWidth', 4);
    
end

for i = 1: size(trange,1)
    scatter(ax,medallhf(i,1),medallhf(i,2), 20, i, 'filled','o');  %, 'MarkerEdgeColor', 'w')
end
colormap(ax,'jet');
...
c.Label.FontSize = 11;
text(ax,0.95,0.06,'HF','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2,...
    'horizontalalignment','right');
text(ax,0.04,0.93,'a','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2);
text(ax,0.95,0.93,'TWKB','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2,...
    'horizontalalignment','right');
text(ax,0.5,0.06,'All RTMs','FontSize',12,'unit','normalized',...
...
text(ax,deg2rad(225),3.5,num2str(sum(angbest>180 & angbest<=270)),'HorizontalAlignment','center',...
     'FontSize',11);
text(ax,deg2rad(315),3.5,num2str(sum(angbest>270 & angbest<=360)),'HorizontalAlignment','center',...
     'FontSize',11); 
...
for i = 1: length(indelse)
    num = numlf(indelse(i));
    for j = 1: nbin
        if theta(i)>bincnt(j)-deg2rad(binw/2) && theta(i)<=bincnt(j)+deg2rad(binw/2)
            break
        else
            continue
        end
...
%%% save figure
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_mainSW+NE_all',num2str(nfam),'.pdf'));
...
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_mainSW+NE_ind',num2str(nfam),...
      '.pdf'));
...
%% use ones diff in offset is smaller than 0.5 km, plot the combined migrations
%%% All detections
% indplt = union(indsw,indne);    % index to plot
xran = [-15 10];
yran = [-15 15];
...
conf = 99;
[f,barsw,pdfxlocsw,pdfvalsw,barne,pdfxlocne,pdfvalne,lgd] = ...
    plt_hist_combined_RTMs(indne2,indsw2,ranvechf98,medallhf,angbest,xran,yran,vdistsw2,wtsw2,...
                           vdistne2,wtne2,binw,conf,'int');
text(f.ax(1),0.5,0.06,'All LF detections','FontSize',12,'unit','normalized',...
     'horizontalalignment','center');
text(f.ax(1),0.5,0.12,'Fewer migrations,','FontSize',12,'unit','normalized',...
     'horizontalalignment','center');
xlim(f.ax(2), [-12.5 12.5]); 
xvect = [-7 -12.5+0.5];
yvect = [0.015 0.015];
...
%%% save figure
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_mainSW+NE_all_smalldiff',num2str(nfam),'.pdf'));
...
conf = 99;
[f,barisw,pdfxlocisw,pdfvalisw,barine,pdfxlocine,pdfvaline,lgd] = ...
    plt_hist_combined_RTMs(indne2,indsw2,ranvechf98,medallhf,angbest,xran,yran,ivdistsw2,iwtsw2,...
                           ivdistne2,iwtne2,binw,conf,'int');
text(f.ax(1),0.5,0.06,'Independent LF detections','FontSize',12,'unit','normalized',...
     'horizontalalignment','center');
text(f.ax(1),0.5,0.12,'Fewer migrations,','FontSize',12,'unit','normalized',...
     'horizontalalignment','center'); 
...
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_mainSW+NE_ind_smalldiff',num2str(nfam),...
      '.pdf'));


%% plot one histogram for all the other unused migrations
%%% All detections
xran = [-15 25];
yran = [-20 15];
binw = 0.5;
conf = 99;
[f,barelse,pdfxlocelse,pdfvalelse] = ...
    plt_hist_combined_RTMs_otherrtm(indelse,ranvechf98,medallhf,angbest,xran,yran,vdistelse,wtelse,...
                           binw,conf,'int');
                                 
                       
%%% save figure
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_otherrtm_all',num2str(nfam),'.pdf'));
...
%     i=3;
    ...
    for iang = 1: length(angle)
        ...
        % output fit parameters
        coef = coeffvalues(fitobj);
        slopehf(ind(i),iang) = coef(1);
        rmsehf(ind(i),iang) = gof.rmse;
            
    end

    %%% best angle estimate from hf
    indpos = find(slopehf(ind(i),:)>0);
    ind3 = find(rmsehf(ind(i),indpos)==min(rmsehf(ind(i),indpos)));     % rmse, Root Mean Squared Error, the smaller, the better
    if length(ind3) > 1
        disp(strcat({'multiple angles have same rmse: '},num2str(angle(ind3))));
    end
    angrmsehf(ind(i)) = angle(indpos(ind3(1)));

    ind6 = find(slopehf(ind(i),:)==max(slopehf(ind(i),:))); % one with the largest slope, i.e., migrating speed
    if length(ind6) > 1
        disp(strcat({'multiple angles have same slope:'},num2str(angle(ind6))));
    end
    angslopehf(ind(i)) = angle(ind6(1));
...
%     ifam = find(famperchf(i,:) == max(famperchf(i,:))); % choose fam contributes most detections
    ...
    %%% reposition
    set(f8.ax(1), 'position', [ 0.08, 0.64, 0.36, 0.32]);
    set(f8.ax(2), 'position', [ 0.52, 0.64, 0.36, 0.32]);
    set(f8.ax(3), 'position', [ 0.08, 0.35, 0.36, 0.22]);
    set(f8.ax(4), 'position', [ 0.52, 0.35, 0.36, 0.22]);
...
%     c.TickLabels=[];
    ...
    text(f8.ax(1),0.04,0.1,'TWKB','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2);
    f8.ax(1).Box = 'on';
    grid(f8.ax(1), 'on');
    axis(f8.ax(1), 'equal');
    ...
    x = [16+ vecreslzb1(1,1), 16+ vecreslzb1(2,1)];
    y = [6+ vecreslzb1(1,2), 6+ vecreslzb1(2,2)];  % loc resolution indicated by +-1 sample cross
    plot(f8.ax(1),x,y,'color','r','linewidth',1);
    x = [16+ vecreslzb1(3,1), 16+ vecreslzb1(4,1)];
    y = [6+ vecreslzb1(3,2), 6+ vecreslzb1(4,2)];  % loc resolution indicated by +-1 sample cross
    plot(f8.ax(1),x,y,'color','r','linewidth',1);
    ...
    if i == 1
        text(f8.ax(1),0.5,0.1,'I','FontSize',12,'unit','normalized');
    elseif i == 2
    ...
    text(f8.ax(1),0.84,0.95,strcat(num2str(size(mighf,1)),{' detections'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f8.ax(1),0.84,0.90,strcat({'in '},num2str(trange(ind(i),3)-trange(ind(i),2)),{' s'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    rate = sprintf('%.3f',size(mighf,1)/(trange(ind(i),3)-trange(ind(i),2)));
    text(f8.ax(1),0.84,0.85,strcat({'rate: '},rate),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f8.ax(1),0.84,0.80,strcat(num2str(angbest(ind(i))),{'{\circ}'}),'FontSize',10,...
         'unit','normalized','horizontalalignment','center'); 
    hold(f8.ax(1),'off');
...
%     c.TickLabels=[];
    ...
    text(f8.ax(2),0.04,0.1,'TWKB','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2);
    f8.ax(2).Box = 'on';
    grid(f8.ax(2), 'on');
    axis(f8.ax(2), 'equal');
    ...
    x = [16+ vecreslzb2(1,1), 16+ vecreslzb2(2,1)];
    y = [6+ vecreslzb2(1,2), 6+ vecreslzb2(2,2)];  % loc resolution indicated by +-1 sample cross
    plot(f8.ax(2),x,y,'color','r','linewidth',1);
    x = [16+ vecreslzb2(3,1), 16+ vecreslzb2(4,1)];
    y = [6+ vecreslzb2(3,2), 6+ vecreslzb2(4,2)];  % loc resolution indicated by +-1 sample cross
    plot(f8.ax(2),x,y,'color','r','linewidth',1);
    ...
    if i == 1
        text(f8.ax(2),0.5,0.1,'I','FontSize',12,'unit','normalized');
    elseif i == 2
    ...
    text(f8.ax(2),0.84,0.95,strcat(num2str(size(miglf,1)),{' detections'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f8.ax(2),0.84,0.9,strcat({'in '},num2str(trange(ind(i),3)-trange(ind(i),2)),{' s'}),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    rate = sprintf('%.3f',size(miglf,1)/(trange(ind(i),3)-trange(ind(i),2)));
    text(f8.ax(2),0.84,0.85,strcat({'rate: '},rate),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f8.ax(2),0.84,0.80,strcat(num2str(angbest(ind(i))),{'{\circ}'}),'FontSize',10,...
         'unit','normalized','horizontalalignment','center');
    hold(f8.ax(2),'off');
...
%              round(max([mighfdum(:,1);miglfdum(:,1)]))+1];
    ...
    % the obtained CI is comfirmed to be correct by comparing it with the 'fitobjhfprop'
    slopesehf = gofhfprop.rmse./sqrt(sum((x-mean(x)).^2));
    est = slopeprophf(ind(i));
    slopeprophfCI(ind(i),:) = confidence_interval_general(est,slopesehf,length(x)-2,95);
    interceptsehf = slopesehf.*sqrt(sum(x.^2)./length(x));
    est = intcptprophf(ind(i));
    intcptprophfCI(ind(i),:) = confidence_interval_general(est,interceptsehf,length(x)-2,95);
    sehf(ind(i), :) = [gofhfprop.rmse slopesehf interceptsehf];     
    
    x = miglfdum2(:,15)/3600;
    slopeself = goflfprop.rmse./sqrt(sum((x-mean(x)).^2));
    interceptself = slopeself.*sqrt(sum(x.^2)./length(x));
    self(ind(i), :) = [goflfprop.rmse slopeself interceptself];
    ...
    y_var = sum(wthf.*(y-y_bar).^2) / sum(wthf);
    xy_cov = sum(wthf.*(x-x_bar).*(y-y_bar)) / sum(wthf);
    pearwthf(ind(i)) = xy_cov / sqrt(x_var*y_var);
    ...
    text(f8.ax(3),0.45,0.2,sprintf('Slope: %.1f km/h',slopeprophf(ind(i))),'FontSize',8,...
         'unit','normalized','horizontalalignment','left');
    text(f8.ax(3),0.97,0.2,sprintf('SE: %.2f',slopesehf),'FontSize',8,...
         'unit','normalized','horizontalalignment','right'); 
    text(f8.ax(3),0.45,0.13,sprintf('Pearson: %.3f',pearwthf(ind(i))),'FontSize',8,...
         'unit','normalized','horizontalalignment','left');
    text(f8.ax(3),0.97,0.12,sprintf('SE_{LF}: %.2f',slopeself),'FontSize',8,...
         'unit','normalized','horizontalalignment','right');  
    text(f8.ax(3),0.45,0.05,sprintf('LF-HF: %.2f km',offset(ind(i))),'FontSize',10,'unit','normalized'); 
    hold(f8.ax(3),'off');
    ...
    f8.ax(4).Box = 'on';
    yyaxis(f8.ax(4),'left');
    plot(f8.ax(4),angle,rmsehf(ind(i),:),'o-','linew',1,'color','k','markers',4);
    yranl = f8.ax(4).YLim;    
    if angbest(ind(i)) == angrmsehf(ind(i))
        plot(f8.ax(4),[angrmsehf(ind(i)) angrmsehf(ind(i))],[0 100],'--','linew',1.5,'color','k');
    else
        plot(f8.ax(4),[angrmsehf(ind(i)) angrmsehf(ind(i))],[0 100],'--','linew',0.5,'color','k');
    end
    f8.ax(4).YColor = 'k';
    f8.ax(4).YLim = yranl;
    ylabel(f8.ax(4),'RMSE of HF','fontsize',11);
    
    yyaxis(f8.ax(4),'right');
    plot(f8.ax(4),angle,slopehf(ind(i),:),'^:','linew',1,'color',[0.12 0.56 1],'markers',4);
    yranr = f8.ax(4).YLim;
    if angbest(ind(i)) == angslopehf(ind(i))
        plot(f8.ax(4),[angslopehf(ind(i)) angslopehf(ind(i))],f8.ax(4).YLim,'--','linew',1.5,...
             'color',[0.12 0.56 1]);    %[0 0 0.5];
    else
        plot(f8.ax(4),[angslopehf(ind(i)) angslopehf(ind(i))],f8.ax(4).YLim,'--','linew',0.5,...
             'color',[0.12 0.56 1]);
    end    
    f8.ax(4).YColor = [0.12 0.56 1]; %[0 0 0.5];
    f8.ax(4).YLim = yranr;
    ylabel(f8.ax(4),'Slope of HF (km/h)','fontsize',11);
    text(f8.ax(4),0.04,0.91,'d','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2);
    xlim(f8.ax(4),[0 360]);
%     ymax = f8.ax(4).YLim(2)+0.1;
%     ylim(f8.ax(4),[0 ymax]);
%     ylim(f8.ax(4),[0 1]);
    xlabel(f8.ax(4),'Trial propagation direction (deg)','fontsize',11);
    hold(f8.ax(4),'off');
...
% returns estimates of normal distribution parameters, mu and sigma
% under 95% confidence interval
    [muhf,sigmahf,muhfCI,sigmahfCI] = normfit((resprophf(ind(i),1:numhf(ind(i))))', 0.05, [], wthf);
    [mulf,sigmalf,mulfCI,sigmalfCI] = normfit((resproplf(ind(i),1:numlf(ind(i))))', 0.05, [], wtlf);
%     text(f8.ax(4),0.04,0.91,'d','FontSize',11,'unit','normalized','EdgeColor','k','Margin',2);
...
%     hold(f8.ax(4),'off');
    ...
    %%% save figure
    print(f8.fig,'-dpdf',strcat(rstpath,'/LZB',num2str(nfam),'autortmv3.sel.mig.proj.lfit.meddist',...
