...
%% load checked detections by other fams with or without a distance cutoff
SUFFIXhf = strcat('up.hf.time.',num2str(winlenhf),'_',num2str(winlenlf),'.',num2str(lofflf),...
    '.',num2str(ccminlf));
SUFFIXlf = strcat('lf.time.',num2str(winlenhf),'_',num2str(winlenlf),'.',num2str(lofflf),...
    '.',num2str(ccminlf));
...
for ista = 1: nstasnew
    ...
    if dcutflag
        hffname = strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.dcut.',SUFFIXhf);
        lffname = strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.dcut.',SUFFIXlf);
    else
        hffname = strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.nodcut.',SUFFIXhf);
        lffname = strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.nodcut.',SUFFIXlf);
    end
    ...
    %
    if dcutflag
        fidhf = fopen(strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.dcutnodou.',SUFFIXhf),'w+');
        fidlf = fopen(strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.dcutnodou.',SUFFIXlf),'w+');
    else
        fidhf = fopen(strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.nodcutnodou.',SUFFIXhf),'w+');
        fidlf = fopen(strcat(rstpath, '/evtloc.',stasnew(ista,:),'check.allfam.nodcutnodou.',SUFFIXlf),'w+');
    end
    
    
    fprintf(fidhf,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4e %.4e %10.3e %10.3f %.4f %d \n',...
        hfnew');
    fclose(fidhf);
    
    fprintf(fidlf,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4e %.4e %10.3e %10.3f %.4f %d \n',...
        lfnew');
    fclose(fidlf);
