function allsyn = decon_synth_fn(srcregion,reg,sat,distr,Twin,tdura,distrloc,...
  physicalsize,testsrcflag,testfreqflag,pltdataflag,pltgtflag,pltsrcflag,...
  pltsrc4thflag,normflag,tempflag,ftrans,nrun)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% impgrpk = cell(nnsat,nreg);
% impk = cell(nnsat,nreg);
% imp4thk = cell(nnsat,nreg);
...
madnsrcamprs4thall = [];
    ...
    % windowskall = [windowskall; windows-windows(1,1)+1+lsig*(irun-1)];
    % off1iwkall = [off1iwkall; off1iw];
    ...
    % text(f.ax(1),0.98,0.9,sprintf('Satur=%.1f',sat),'Units','normalized',...
...
%     [impindep,imppairf,indpair,sharp] = groupimptripdecon(sigdecon,ampit,rcc,loff_max,refsta);
    ...
    % nsrcgrpall = [nsrcgrpall; size(impindepst,1)];
    ...
    % impall = [impall; imp];
    % nsrcall = [nsrcall; size(impindepst,1)];
...
%       supertit(f.ax,'Secondary sources removed');
    ...
    % nsrc4thall = [nsrc4thall; size(impindepst,1)];
...
%% save file      
savefile = strcat('rst_decon_synth','_reg',num2str(xaxis),'-',num2str(yaxis),...
  '_nsat',num2str(sat),'_td',num2str(tdura),'.mat');
save(strcat(workpath,'/synthetics/',savefile), 'allsyn');

    % irccrank{insat,ireg} = irccranall;
    % windowsk{insat,ireg} = windowskall;
    % off1iwk{insat,ireg} = off1iwkall;
    % ccaliwk{insat,ireg} = ccaliwall;
    % ccwpairk{insat,ireg} = ccwpairall;
    % mrccwpairk{insat,ireg} = mrccwpairall;
    % rcccatk{insat,ireg} = rcccatall;
    % off1ik{insat,ireg} = off1iall;
    % rcck{insat,ireg} = rccall;
    % nitsynk{insat,ireg} = nitall;
    % nitsynsum{insat,ireg} = sum(nitall,1); %all runs are summed or concatenated
    % impgrpk{insat,ireg} = impgrpall;  %concatenation of all runs
    % nsrcgrpk{insat,ireg} = nsrcgrpall;
    % nsrcgrpsum{insat,ireg} = sum(nsrcgrpall); %all runs are summed or concatenated
    % impk{insat,ireg} = impall;  %concatenation of all runs
    % nsrck{insat,ireg} = nsrcall;
    % nsrcsum{insat,ireg} = sum(nsrcall); %all runs are summed or concatenated
    % mprojxnn1k{insat,ireg} = median(mprojxnn1all);  %a median of all runs, suppose each run has a similar value
    % mprojx2allk{insat,ireg} = median(mprojx2allall);  %a median of all runs
    % srcamprk{insat,ireg} = srcamprall;
    % msrcamprk{insat,ireg} = median(msrcamprall);
    % madsrcamprk{insat,ireg} = median(madsrcamprall);
    % mpsrcamprsk{insat,ireg} = median(mpsrcamprsall);
    % madpsrcamprsk{insat,ireg} = median(madpsrcamprsall);
    % mnsrcamprsk{insat,ireg} = median(mnsrcamprsall);
    % madnsrcamprsk{insat,ireg} = median(madnsrcamprsall);
    
    % imp4thk{insat,ireg} = imp4thall;  %concatenation of all runs
    % nsrc4thk{insat,ireg} = nsrc4thall;  %all runs are summed or concatenated
    % nsrc4thsum{insat,ireg} = sum(nsrc4thall);  %all runs are summed or concatenated
    % diffoff14k{insat,ireg} = diffoff14trall;
    % mprojxnn14thk{insat,ireg} = median(mprojxnn14thall);  %a median of all runs, suppose each run has a similar value
    % mprojx2all4thk{insat,ireg} = median(mprojx2all4thall);
    % srcampr4thk{insat,ireg} = srcampr4thall;    
    % msrcampr4thk{insat,ireg} = median(msrcampr4thall);
    % madsrcampr4thk{insat,ireg} = median(madsrcampr4thall);
    % mpsrcamprs4thk{insat,ireg} = median(mpsrcamprs4thall);
    % madpsrcamprs4thk{insat,ireg} = median(madpsrcamprs4thall);
    % mnsrcamprs4thk{insat,ireg} = median(mnsrcamprs4thall);
    % madnsrcamprs4thk{insat,ireg} = median(madnsrcamprs4thall);
