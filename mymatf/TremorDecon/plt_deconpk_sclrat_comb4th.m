function f=plt_deconpk_sclrat_comb4th(f,psrcamprsall,nsrcamprsall,color)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f=plt_deconpk_sclrat_comb4th(f,psrcamprsall,nsrcamprsall,color)
%
% Similar to 'plt_deconpk_sclrat_comb', this function is to plot the 
% scaled deconvolved positive and negative source peak ratios between 
% all station pairs (12,13, and 23) including 14. 
...
%     'Units','normalized','HorizontalAlignment','right');  %note here is linear scale
  ...
  elseif i ==4
    xlabel(ax,'log_{10}{src scaled pos amp ratio 1/4}');
  end
  xlim(ax,[-1 1]);
...
%     'Units','normalized','HorizontalAlignment','right');  %note here is linear scale
  ...
  elseif i ==4
    xlabel(ax,'log_{10}{src scaled neg amp ratio 1/4}');
  end
  xlim(ax,[-1 1]);
