function [impindep,imptripf,indtrip] = groupimptriplets_ref(sigdecon,irccran,rcccat,...
  off1i,off1iw,loff_max,method,refsta)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [impindep,imppairf,indpair] = groupimptriplets_ref(sigdecon,irccws,rcccat,off1i,off1iw,loff_max,method)
%
% As the version for refinement using subwins wrt to 'groupimptriplets.m', 
% this function is to group the nearest impulses that are deconvolved 
...
%
% --The difference is, it will try to group the impulses based on a fixed
% allowable searching boundary 'loff_max' relative to a best alignment 
% (offset) that is predetermined for overlapping sub-windows decomposed from
% the entire window. If the best alignment changes for consecutive subwins,
% the grouping range is actually 'moving' as well. This may permit the 
% grouping of triplets that were missed due to the single invariant grouping
% range used in 'groupimptriplets.m'.
%
...
npair = sum(sigdecon(:,refsta)>0); % choose station 1 as the reference
