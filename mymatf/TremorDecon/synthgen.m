function [synths,mommax,sources,greensts]=synthgen(writes,winlen,skiplen,synth,Greens,b,xgrid,...
  ygrid,pdfgrid,sps,fracelsew,seed,timetype,ftrans,stas)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   sources: source info, [indtarvl rnoff12 rnoff13 indtori indttrvl moment];
...
%generate random samples for the time of sources inside the restricted region
rn = rand(writes(end),1);
size(rn)
if strcmp(timetype,'tarvl')
  indtarvl = ceil(rn*winlen); % arrival time is uniform within the span of window length
  
  %info of synthetic sources
  sources=[indtarvl rnoff]; % return the source, 3 cols
elseif strcmp(timetype,'tori')
  indtori = ceil(rn*winlen); % origin time is uniform within the span of window length
  ...
  %info of synthetic sources
  sources=[indtarvl rnoff indtori indttrvl]; %5 cols
end
% keyboard
...
if nsta > 3
  for ista = 4: nsta
    ...
    [~,off14] = pred_tarvl_at4thsta(stas(ista,:),rnoff(:,1),rnoff(:,2),indtarvl);
    rnoff = [rnoff off14];
  end
end
...
for n = 1: writes(end)
  ...
  if ind <= winlen
    ...
    %     end
    %   else
    for ista = 2: nsta %now nsta can be more than 3, as 'rnoff' has cols for 4th stas too
      %ind - rnoff is the arrival time in index at sta 2 and 3
...
% keyboard
%make sure to discard related sources
sources(sources(:,1)>winlen,:)=[];
% keyboard
