function [sigdecon,pred,res,dresit,mfitit,ampit,nit,fighdl,rf] = ...
  iterdecon_rcc(sig,wlet,rcc,rccthr,dt,twlet,width,dres_min,mfit_min,nit_max,nimp_max,fpltit,fpltend,fpltchk)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [sigdecon,pred,res,dresit,mfitit,nit,rf] = ...
%   iterdecon_rcc(sig,wlet,dt,twlet,width,dres_min,mfit_min,nit_max,fpltit,fpltend)
%
% This is a mutant version of 'iterdecon.m', in order to use the rcc as the stopping
% criterion ONLY, so basically the noise is not involved and evaluated.
%
...
mwtcoef = 1;  % intial amplitude of max master CC weighted by rcc, value doesn't really matter 
...
while rccimp>rccthr && nit<nit_max  
...
%   [pkhgt, pkind] = findpeaks(coefeff,'MinPeakHeight',ampnoi,'MinPeakProminence',ampnoi);
  ...
  if fpltchk
    figure
    p1=plot((1+round(ldiff/2)): (nfft-round(ldiff/2)),coefeff,'-','color',[.3 .3 .3]); hold on % master cc
    scatter(pkind+round(ldiff/2),pkhgt,20,'r'); % peaks
    p2=plot((1+round(ldiff/2)): (nfft-round(ldiff/2)),rcc,'o','color',[.7 .7 .7],'markersize',2);  % rcc
    scatter(pkind+round(ldiff/2),rcc(pkind),20,'r');  % rcc value at peaks
    p3=scatter(pkind+round(ldiff/2),rcc(pkind).* pkhgt,20,'b');  % peak heights weighted by rcc
%     scatter(idximp,pkhgt(mwtcoefidx),10,'b','filled');  % chosen peak
%     scatter(idximp,rcc(pkind(mwtcoefidx)),10,'b','filled'); % chosen peak
    scatter(idximp,rcc(pkind(mwtcoefidx)).*pkhgt(mwtcoefidx),20,'b','filled'); % chosen peak
    ax = gca;
    p4=plot(ax.XLim, [ampnoiwt ampnoiwt],'k--'); % noise level
%     p4=plot(ax.XLim, [ampnoi ampnoi],'k--'); % noise level
%     plot(ax.XLim,[mwtcoef mwtcoef],'b--');  % indeed the max?
    legend([p1,p2,p3,p4],'Residual-template CC','3-station RCC','Weighted peaks','Noise level');
    box on; grid on
...
%     plot(ax.XLim,[ampnoi ampnoi],'--','color',[.5 .5 .5]);
    ...
    yyaxis right;
    p2=plot((1+round(ldiff/2)): (nfft-round(ldiff/2)),rcc,'o','color',[.8 .8 .8],'markersize',2);
    plot(ax.XLim,[rccthr rccthr],'--','color',[.3 .3 .3]);
    ylim([-1 1]);
...
%   p1=stem(ax,sigdecon, 'b','MarkerSize',4);
  ...
  plot(ax,[0 nfft],[rccthr rccthr],'--','color',[.3 .3 .3]);
  ylim(ax,[-1 1]);
  ...
  yyaxis(ax,'right');
  [irccsp, rccsp] = RunningCC(sig, pred, ldiff);
