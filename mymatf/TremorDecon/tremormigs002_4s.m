% tremormigs002_4s.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the script to obtain the migrations (time windows that contain a
% substantial number of 4-s tremor detections) from the 4-s tremor catalog
% autmatically using a grouping scheme.
%
% --Since the catalog obtained from 'detection002_4s', 'mergeday002_4s',
% 'locinterp002_4s' are not necessarily just the proximity of 002, so this 
% script won't give migrations that focus only at 002.
%
% --Technically, this script is similar to 'tremorbursts002_4s' to a large
% extent, the main difference is the choice of 'ttol', which is the max.
% allowable separation of time between consecutive 4-s detections during 
% grouping. Here the grouping will use a longer 'ttol' than the current used
% in 'tremorbursts002_4s', 35 s (as of 2022/07/13).
% 
%
%
% Chao Song, chaosong@princeton.edu
% First created date:   2022/07/13
...
i03 = find(hfall(:,daycol) < 2004*1000);
hfall(i03,ncol+1) = (hfall(i03,daycol)-2003060)+hfall(i03,seccol)./(3600.*24);
i04 = find(hfall(:,daycol) < 2005*1000 & hfall(:,daycol) > 2004*1000);
hfall(i04,ncol+1) = (hfall(i04,daycol)-2004194)+hfall(i04,seccol)./(3600.*24);
i05 = find(hfall(:,daycol) > 2005*1000);
hfall(i05,ncol+1) = (hfall(i05,daycol)-2005254)+hfall(i05,seccol)./(3600.*24);
...
hfinter03 = interevt_time(hfall(i03,end));
hfinter04 = interevt_time(hfall(i04,end));
hfinter05 = interevt_time(hfall(i05,end));
...
ttol = 1e-3*ones(3,1); 
% ttol = 8*[mad(hfinter03(:,2),1); mad(hfinter04(:,2),1); mad(hfinter05(:,2),1)];
ntol = 15;
...
hfnew = refine_catalog_by_trange(hfall,daycol,seccol,trange);
hfnew = hfnew(:,1:end-1); % ignore the last column of relative time in days

perchfnew = size(hfnew,1)/size(hfall,1)
...
fid = fopen(strcat(rstpath, '/MAPS/migran',num2str(round(ttol(1)*86400)),'s.pgc002'),'w+');
