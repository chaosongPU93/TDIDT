% deconvbursts002_4s.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The next script to run after 'seisbursts002_4s.m', to actually do 
% deconvolution to those selected windows. All the struggling in 
% 'seisbursts002_4s.m' was trying to find a common criterion to stop the 
% iteration properly, hoping we could pinpoint a proper proxy for noise of 
% each window. However, the effort seems to be not rewarded well, as the
% low-amp/envelope part can sometimes has high coherence between stations
% as well, so simply decide upon the amplitude is not reasonable enough.
%
% The relation between the running amplitude/envelope and CC is complicated,
% but looks like the median rcc for the signal window is almost always no 
% smaller than that of the 'noise', whereas the counter examples are usually
% very close in value. This could be one evidence that rcc is a better measure
% for when to stop the iteration.
% 
...
% overfit the noise due to more iters.
%
...
%
% --After cutting out the data window from the 'tremorbursts002_4s.m' and 
% 'seisbursts002_4s.m', we aligned the data with a single overall best alignment.
% To the aligned, compute the RCC, using which to guide the deconvolution at
% each station independently.
%
% --Grouping the deconvolved impulses at 3 stations into triplets based upon the 
% sorted impulse by their weighted amp. Then we remove the small-amp and secondary
% sources that are too close to the primary ones in time.
% 
% --Estimate if there is any obvious migration of sources, which might indicate
% the invariant grouping boundary could be improper, in order to provide insights
% for the next script 'deconvbursts002_ref_4s.m' to see if it is necessary to do 
% a 2nd round of decon, grouping, etc, using a moving boundary. 
% Or, if there is no obvious movment, whether the current boundary is too small.
%
%
% Chao Song, chaosong@princeton.edu
% First created date:   2022/04/28
% Last modified date:   2022/04/28
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%       [f] = plt_rcccat(rccpair,sps);
      ...
      figure
      subplot(211)
      plot(rcc12,'r','linew',1); hold on ;
      plot(rcc13,'b','linew',1);
      plot(rcc23,'k','linew',1);
      xlabel('Samples');
      ylabel('Running CC');
      subplot(212)
      [cdfval,x] = ecdf(rcc12);
...
%       medrcc = median(rcc([indnodet inddeto]));
      ...
      for ista = 1:nsta
        ...
        fpltchk = 1; % plot flag for intermediate computations
...
%       [impindep,imppairf,indpair] = groupimptriplets(sigdecon,rcc,loff_max,'wtamp',refsta);
      ...
      ax1=gca;
      [ax1] = plt_decon_imp_scatter(ax1,impindepst,xran,yran,cran,offxran,offyran,...
        sps,75,'mean','tori');
...
%       [ax,hf] = plt_detections_inmap(ax,hfbst,[rangetemp(j,1) tstbuf tedbuf],[tstbuf tedbuf],colnum);
      ...
      %compute running CC between 3 stations
      [irccp,rccp12] = RunningCC(predgrpl(:,1), predgrpl(:,2), mwlen);
      [~,rccp13] = RunningCC(predgrpl(:,1), predgrpl(:,3), mwlen);
      [~,rccp23] = RunningCC(predgrpl(:,2), predgrpl(:,3), mwlen);
...
status = system('rm -f /home/data2/chaosong/matlab/allan/data-no-resp/PGCtrio/FIGS/decon002.pdf');
for i = 1:size(fignm,1)
  mergenm{i} = fullfile(rstpath, '/FIGS/',fignm{i});
end
append_pdfs(fullfile(rstpath, '/FIGS/decon002.pdf'),mergenm);
