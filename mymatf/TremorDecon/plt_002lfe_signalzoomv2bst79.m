function f=plt_002lfe_signalzoomv2bst79(greenf,sigsta,rcc,impindepst,sps,xzoom,...
    tstbuf,dy,mo,yr,tbostplt,idxbst,yran2l,saveflag)
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % This is a function specific designed for burst 79, to plot the seismograms
  % of the stacked LFE templates 'greenf' that are bandpassed. And a zoom-in 
  % window of the signal seismograms 'signal', this is supposed to be a figure
  % shown in the paper. 
  % --burst 79 is short, like 17 s, and we want to fit a shortened template of
  % 7 s with this burst, so that it will be one row. This way, in total it spans
  % 24 s, shorter than a typical 25-s for plotting purpose, such that they can 
  % be scaled equally.
  %
  %
  %
  ...
  axpos = [0.06 0.62 0.87*lwletzoom/reflen 0.35;
          ...
          %  0.07 0.13 0.87*lsigzoom/reflen 0.35];
  for i = 1:nrow*ncol
...
%   xlim(ax,[0 range(wletzoom)]);
  xticks(ax,wletzoom(1): 2: wletzoom(2));
  tkval = 0: 2: range(wletzoom);
...
%   text(ax,0.01,0.85,'b','FontSize',11,'unit','normalized','EdgeColor','k','Margin',1);
  ...
  lgd=legend(ax,p,{'PGC','SSIB','SILB'},'Location','northwest','NumColumns',3,...
    'fontsize',8);
