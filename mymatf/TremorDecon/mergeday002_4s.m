% mergeday002_4s.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The next script to run after 'detection002_4s.m', is to merge detections
% of all days for fam 002 using PGC trio.
%
% --Largely the same as 'mergeday_hf_PGC.m', but that is a function written 
%   to automate a massive job under /proj2021
% --I shall reiterate the order of the entire flow of detection here: 
%   'detection002_4s.m' --> 'mergeday002_4s.m' --> DO NOT
...
% --Do not do filtering effect correction!!
% --
%
%
...
fam = '002';  
fcflag = 0; % Do not do filtering effect correction
convflag = 0; % do not convert to the same offset frame
...
%     nd=1;
    ...
    if iup == 1     
        fname1 = strcat(rstpath,'/MAPS/tdecmapall',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                        num2str(winlen/sps),'s',num2str(sps),'sps', '4add');   
        fname2 = strcat(rstpath,'/MAPS/tdecmapall',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                        num2str(winlen/sps),'s',num2str(sps),'sps', '3add');
    else
        fname1 = strcat(rstpath,'/MAPS/tdecmapallup',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                        num2str(winlen/sps),'s',num2str(sps),'sps', '4add');   
        fname2 = strcat(rstpath,'/MAPS/tdecmapallup',IDENTIF,'_',num2str(lo),'-',num2str(hi),'_',...
                        num2str(winlen/sps),'s',num2str(sps),'sps', '3add');
    end
...
fid = fopen(strcat(rstpath, '/MAPS/tdectimeori_',PREFIX,'_',num2str(lo),'-',num2str(hi),'_',...
