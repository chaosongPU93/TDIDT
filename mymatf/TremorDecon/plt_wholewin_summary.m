function f=plt_wholewin_summary(sigsta,imp,timevec,locxy,locxyproj,stats,...
  imp4th,timevec4th,locxy4th,locxyproj4th,stats4th,tstbuf,dy,mo,yr,sps,ttype)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% f=plt_wholewin_summary(sigsta,imp,timevec,locxy,locxyproj,stats,...
%   imp4th,timevec4th,locxy4th,locxyproj4th,stats4th,tstbuf,dy,mo,yr,sps,ttype)
%
% Similar to 'plt_wholewin_mapandproj.m', this script would plot the detections
% BOTH after 2ndary removed AND after 4th-sta checked, for DATA detection.
% --See also 'plt_shortwin_summary'
% --This is to plot a summary figure for EACH burst window, should be called
% in 'deconv_4s_exp_4thsta_fn.m'
%
%
...
%   'Units','normalized','FontSize',9,'interpreter','latex');
...
%   'HorizontalAlignment','left','Units','normalized','FontSize',9,...
%   'interpreter','latex');
% %%%add the indication of the reasonable physical source size
hold(ax,'off');
...
if ~isempty(locxyproj4th) && ~isempty(stats4th) && ~isempty(locxy) && ~isempty(locxyproj)
  ...
  if length(indremove) >=2
    ...
    text(ax,0.99,0.20,sprintf('Speed: %.1f m/s',statsrem.slope*1e3),...
      'HorizontalAlignment','right','Units','normalized','FontSize',9,'color','r');
