% plt_tclustering_morenoise.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the script in particular to plot a summary figure for the clustering
% of LFEs in time, similar to what we did in 'plt_clusterintime.m'. 
% --However, we specifically deal with the noise catalogs that were created by
% deconvolution to synthetic noises generated by diff. seed numbers.
% --The original catalog is also plotted as comparison. 
%
%
%
% Chao Song, chaosong@princeton.edu
% First created date:   2024/11/11
% Last modified date:   2024/11/11
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% load syn noise catalogs from diff. seed numbers
seedsclpool = 3: 10: 63;
...
for i = 1: npool
  ...
  nbst = size(trange,1);
  dtcut = 0.25*m+0.125;
  [ampn{i+1},dtintern{i+1}]=med_amp_incluster(nbst,impn,nsrcn,m);
  [ampn4th{i+1},dtintern4th{i+1}]=med_amp_incluster(nbst,impn4th,nsrcn4th,m);
...
for i = 1: npool
  seedscl=seedsclpool(i);   
  dtinterplt = dtintern{i+1};   
  Nn=histcounts(dtinterplt(:,1)/sps,binedge,'normalization','count');
  Nnn = reshape(Nn,[],1)./length(dtintern{1});
  fracn = sum(dtinterplt(:,1)/sps<=dtcut)/length(dtinterplt);
  p(i+1)=plot(ax,bincnt,Nnn,'color',color(i,:),'LineWidth',1.5);
  label{i+1}=sprintf('seed+%d',seedscl);
end
...
for i = 1: npool
  seedscl=seedsclpool(i);   
  dtinterplt = dtintern4th{i+1};   
  Nn4th=histcounts(dtinterplt(:,1)/sps,binedge,'normalization','count');
  Nn4thn = reshape(Nn4th,[],1)./length(dtintern{1});
  fracn4th = sum(dtinterplt(:,1)/sps<=dtcut)/length(dtinterplt);
  p(i+1)=plot(ax,bincnt,Nn4thn,'color',color(i,:),'LineWidth',1.5);
  label{i+1}=sprintf('seed+%d',seedscl);
end
% fracdif = (sum(dtarvl/sps<=dtcut)-sum(dtarvln/sps<=dtcut)) / ...
%   (length(dtarvl)-length(dtarvln));
...
for i = 1: npool
  ...
  aa=sort(dtinterplt(:,1)/sps);
  bb = prob_geq(aa);  %probability of inter-event time > t0
  p(i+1)=plot(ax,bb(:,1),bb(:,2),'-','color',color(i,:),'LineWidth',1.5);
  label{i+1}=sprintf('seed+%d',seedscl);
end
