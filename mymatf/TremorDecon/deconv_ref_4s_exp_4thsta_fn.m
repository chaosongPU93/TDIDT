% function deconv_ref_4s_exp_4thsta_fn(idxburst,normflag,pltflag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
off1ia = zeros(size(trange,1),nsta);  % single best alignment 'computed' between sta 2, 3 wrt 1 for entire win
...
%         optcc(:,3) = STAopt(max(isubwst*sps,1): min(isubwed*sps,86400*sps), 4);
        optcc = optseg(isubwst: isubwed, 2:4);
...
%       off1ia(k,4:end) = off14pred(k,:);
      ...
      for ista = 4: nsta  % 4th stas
        optdat(:, ista+1) = optseg(1+msftaddm-off1ia(k,ista): end-msftaddm-off1ia(k,ista), ista+1); 
        ortdat(:, ista+1) = ortseg(1+msftaddm-off1ia(k,ista): end-msftaddm-off1ia(k,ista), ista+1);
      end
