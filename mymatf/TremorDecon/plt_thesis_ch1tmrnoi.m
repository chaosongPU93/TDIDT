% plt_thesis_ch1tmrnoi.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the code to plot some examples of 4-s tremor detections, and 
% windows that didn't pass the detection thresholds.
% The resulting figure is used in the PhD thesis, chapter 1 intro, section
% 1.2. 
% 
%
%
...
%% let's just choose the top 100 in terms of CC
ntrace = 25;
cccol = 17;
wlen = 4*sps;
tmp = sortrows(hfbnd, [cccol],'descend');
hfuse = sortrows(tmp(1:ntrace,:), [daycol, cntcol]);

%% read daily data, break into windows of segments, plot
%filtering passband for reading data
hisig=10; % this will give a similar spectral shape between template and signal
losig=1;
...
dates = unique(hfuse(:,daycol));
years = unique(floor(dates/1000));
nets = length(years);

tracetmr = zeros(ntrace,wlen,3,nsta);  % 'ntrace' with each being 'wlen' long, 3 components, at 'nsta' stations
traceopttmr = zeros(ntrace,wlen,nsta); % 'ntrace' with each being 'wlen' long, 1 opt comp., at 'nsta' stations
k = 0;
for iets = 1: nets
  % dates in each ets
  year = years(iets);
...
%         STAZ(:, ista) = Bandpass(dataZ,sps,lo,hi,npo,npa,'butter');
    ...
    hfdayi = hfuse(hfuse(:,daycol)==datesets(i), :);
    
    for j = 1: size(hfdayi,1)
      tcnt = hfdayi(j,cntcol);
      tst = tcnt*sps-wlen/2+1;
      ted = tcnt*sps+wlen/2;
      
      k = k+1;
      tracetmr(k,:,:,:) = STANEZ(tst:ted, :, :);
      traceopttmr(k,:,:) = STAopt(tst:ted, 2:end);
...
tracenoi = zeros(ntrace,wlen,3,nsta);  % 'ntrace' with each being 'wlen' long, 3 components, at 'nsta' stations
traceoptnoi = zeros(ntrace,wlen,nsta); % 'ntrace' with each being 'wlen' long, 1 opt comp., at 'nsta' stations
k = 0;
for iets = 1: nets
  % dates in each ets
  year = years(iets);
...
%         STAZ(:, ista) = Bandpass(dataZ,sps,lo,hi,npo,npa,'butter');
    ...
    for j = 1: size(tnoiusei,1)
      ...
      k = k+1;
      tracenoi(k,:,:,:) = STANEZ(tst:ted, :, :);
      traceoptnoi(k,:,:) = STAopt(tst:ted, 2:end);
...
%% plot the tremor detections and noise examples together
...
figxran = [0.08 0.98]; figyran = [0.08 0.96];
...
for j = 1: ntrace
  p(1)=plot(ax,(1:wlen)/sps,traceopttmr(j,:,1)+j*vsep,'r-','linew',1);
  p(2)=plot(ax,(1:wlen)/sps,traceopttmr(j,:,2)+j*vsep,'b-','linew',1);
  p(3)=plot(ax,(1:wlen)/sps,traceopttmr(j,:,3)+j*vsep,'k-','linew',1);
end
text(ax,0.08,0.98,sprintf('%.1f--%.1f Hz',losig,hisig),'Units','normalized',...
  'HorizontalAlignment','left');
text(ax,0.98,0.98,sprintf('%s',comp(1,:)),'Units','normalized',...
  'HorizontalAlignment','right');
lgd=legend(ax,p,stas,'orientation','horizontal','location','north','FontSize',8);
set(lgd.BoxFace, 'ColorType','truecoloralpha', 'ColorData',uint8(255*[1;1;1;.8]));  %make background transparent
xlabel(ax,'Time (s)');
title(ax,'Tremor detection');
ylim(ax,[0 (ntrace+4)*vsep]);
xlim(ax,[0 ceil(wlen/sps)]);
longticks(ax,4);
ylabel(ax,'Amplitude');
text(ax,0.02,0.97,'a','FontSize',11,'unit','normalized','EdgeColor','k',...
  'Margin',1,'backgroundcolor','w');

ax=f.ax(2); hold(ax,'on'); ax.Box = 'on'; grid(ax,'on');
for j = 1: ntrace
  p(1)=plot(ax,(1:wlen)/sps,1.5*traceoptnoi(j,:,1)+j*vsep,'r-','linew',1);
  p(2)=plot(ax,(1:wlen)/sps,1.5*traceoptnoi(j,:,2)+j*vsep,'b-','linew',1);
  p(3)=plot(ax,(1:wlen)/sps,1.5*traceoptnoi(j,:,3)+j*vsep,'k-','linew',1);
end
text(ax,0.08,0.98,sprintf('%.1f--%.1f Hz',losig,hisig),'Units','normalized',...
  'HorizontalAlignment','left');
text(ax,0.98,0.98,sprintf('%s',comp(1,:)),'Units','normalized',...
...
set(lgd.BoxFace, 'ColorType','truecoloralpha', 'ColorData',uint8(255*[1;1;1;.8]));  %make background transparent
xlabel(ax,'Time (s)');
title(ax,'No detection');
ylim(ax,[0 (ntrace+4)*vsep]);
xlim(ax,[0 ceil(wlen/sps)]);
longticks(ax,4);
% ylabel(ax,'Amplitude');
text(ax,0.02,0.97,'b','FontSize',11,'unit','normalized','EdgeColor','k',...
  'Margin',1,'backgroundcolor','w');


% orient(f.fig,'landscape');
fname = strcat('tmrnoiexpopt.pdf');
