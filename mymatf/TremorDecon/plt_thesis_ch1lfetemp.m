% plt_thesis_ch1lfetemp.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the code to plot the LFE templates at 7 stations, broadband and
% band-passed in one panel for each station, so a 7-panel figure. Just to 
% give a sense how they look like
...
ppeaksf = zeros(nsta,1); % positive peaks of filtered
npeaksf = zeros(nsta,1); % negative peaks of filtered
...
%%
%%%plot the unfiltered and filtered templates in one panel, diff stas in diff panels
staplt = 1:7;
f=plt_templates_bbbp_bysta(green,greenf,stas,staplt,lowlet,hiwlet,sps);

% orient(f.fig,'landscape');
fname = strcat('lfetemp002.pdf');
