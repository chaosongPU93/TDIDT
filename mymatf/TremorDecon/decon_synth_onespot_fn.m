function allsyn=decon_synth_onespot_fn(perc,sat,distr,Twin,tdura,testfreqflag,...
...
% --See also 'decon_synth_fn.m'.\
% --2024/05/11, given how long the previous code structure takes to run for
% all parameter combos, now we revise the code to separate out sat level and
% noise level, and the function ONLY implement decon to 'nrun' of synthetics 
% from the SAME sat level and noise level. In the end, results from diff combo 
% are not combined together, so the difficulty of cross-comparison is not a big
% headache. Especially, this way can permit using 'parfor' loop to shorten the
% excution time.
%
%
%
...
%   windows = windows-windows(1,1)+1+lsig*(irun-1);
  ...
  for ista = 1:3
    ...
    %         nit_max = round(1.5*1/tdura*tlen*sat);  % max numer of iterations
    %         nimp_max = round(1/tdura*tlen*sat);%a single peak is ~20 samples wide; maybe a little less (at 100 sps). ~0.4s, 1/0.4=2.5
    %         tdura = 0.25;
...
%combine results from same sat and noise ratio, but diff runs into same array
%     irccrank{insat,iperc} = irccranall;
%     windowsk{insat,iperc} = windowskall;
%     off1iwk{insat,iperc} = off1iwkall;
%     ccaliwk{insat,iperc} = ccaliwall;
%     ccwpairk{insat,iperc} = ccwpairall;
%     mrccwpairk{insat,iperc} = mrccwpairall;
%     rcccatk{insat,iperc} = rcccatall;
%     off1ik{insat,iperc} = off1iall;
%     rcck{insat,iperc} = rccall;
%     mrcck{insat,iperc} = mrccall;
%     cck{insat,iperc} = ccall;
%     impgtk{insat,iperc} = impgtall;
%     nitsynk{insat,iperc} = nitall;
%     nitsynsum{insat,iperc} = sum(nitall,1); %all runs are summed or concatenated
%     impgrpk{insat,iperc} = impgrpall;  %concatenation of all runs
%     nsrcgrpk{insat,iperc} = nsrcgrpall;
%     nsrcgrpsum{insat,iperc} = sum(nsrcgrpall); %all runs are summed or concatenated
%     impk{insat,iperc} = impall;  %concatenation of all runs
%     nsrck{insat,iperc} = nsrcall;
%     nsrcsum{insat,iperc} = sum(nsrcall); %all runs are summed or concatenated
%     mprojxnn1k{insat,iperc} = median(mprojxnn1all);  %a median of all runs, suppose each run has a similar value
%     mprojx2allk{insat,iperc} = median(mprojx2allall);  %a median of all runs
%     srcamprk{insat,iperc} = srcamprall;
%     msrcamprk{insat,iperc} = median(msrcamprall);
%     madsrcamprk{insat,iperc} = median(madsrcamprall);
%     mpsrcamprsk{insat,iperc} = median(mpsrcamprsall);
%     madpsrcamprsk{insat,iperc} = median(madpsrcamprsall);
%     mnsrcamprsk{insat,iperc} = median(mnsrcamprsall);
%     madnsrcamprsk{insat,iperc} = median(madnsrcamprsall);
%
%     imp4thk{insat,iperc} = imp4thall;
%     nsrc4thk{insat,iperc} = nsrc4thall;
%     nsrc4thsum{insat,iperc} = sum(nsrc4thall);  %all runs are summed or concatenated
%     diffoff14k{insat,iperc} = diffoff14trall;
%     mprojxnn14thk{insat,iperc} = median(mprojxnn14thall);  %a median of all runs, suppose each run has a similar value
%     mprojx2all4thk{insat,iperc} = median(mprojx2all4thall);
%     srcampr4thk{insat,iperc} = srcampr4thall;
%     msrcampr4thk{insat,iperc} = median(msrcampr4thall);
%     madsrcampr4thk{insat,iperc} = median(madsrcampr4thall);
%     mpsrcamprs4thk{insat,iperc} = median(mpsrcamprs4thall);
%     madpsrcamprs4thk{insat,iperc} = median(madpsrcamprs4thall);
%     mnsrcamprs4thk{insat,iperc} = median(mnsrcamprs4thall);
%     madnsrcamprs4thk{insat,iperc} = median(madnsrcamprs4thall);
