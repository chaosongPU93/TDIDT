...
mshift=1; %19; %16 %11 %14 %12_for_+085+020 %15_for_+059-073  %19 for 128-s?
hi=6.;
lo=1.5;
% hi=12.;
% lo=1;
...
% lo=0.75;
loopoffmax=1.5
xcmaxAVEnmin=0.36 %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
winlen=4*40;
%winlen=128*40;
winoff=40/1;
%winoff=40*8;
...
% for n=-mshift:mshift;
    PGSSx=realPGC.*realSSIB;
    PGSIx=realPGC.*realSILB;
    SISSx=realSILB.*realSSIB;
% end
cumsumPGSS=cumsum(PGSSx);
cumsumPGSI=cumsum(PGSIx);
cumsumSISS=cumsum(SISSx);
cumsumav=(cumsumPGSS+cumsumPGSI+cumsumSISS)/3;
figure
hold on
plot(timsPGC,cumsumPGSS,'r');
plot(timsPGC,cumsumPGSI,'b');
plot(timsPGC,cumsumSISS,'k');
plot(timsPGC,cumsumav,'c','linewidth',3)
print('-depsc',['RUNSUM/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'.eps'])
cumsumPGSS=cumsum(PGSSx)/cumsumPGSS(end);
cumsumPGSI=cumsum(PGSIx)/cumsumPGSI(end);
cumsumSISS=cumsum(SISSx)/cumsumSISS(end);
cumsumav=(cumsumPGSS+cumsumPGSI+cumsumSISS)/3;
figure
subplot(4,1,1,'align'); 
hold on
...
plot(timsPGC,cumsumav,'c','linewidth',3)
is=1; ie=lenx/4;
minPGSS=min(cumsumPGSS(is:ie));
minPGSI=min(cumsumPGSI(is:ie));
minSISS=min(cumsumSISS(is:ie));
maxPGSS=max(cumsumPGSS(is:ie));
maxPGSI=max(cumsumPGSI(is:ie));
maxSISS=max(cumsumSISS(is:ie));
ylim([min([minPGSS minPGSI minSISS]) max([maxPGSS maxPGSI maxSISS])])
xlim([0 timwin/2]);
subplot(4,1,2,'align'); 
hold on
plot(timsPGC,cumsumPGSS,'r');
...
maxSISS=max(cumsumSISS(is:ie));
ylim([min([minPGSS minPGSI minSISS]) max([maxPGSS maxPGSI maxSISS])])
xlim([3*timwin/2 2*timwin]);
% title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['RUNSUM/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'rs.eps'])
