...
%             text(1, 0.,[int2str(winlen/40),' ',int2str(n)])
        sumsPGC2(n,:)=PGC2(iend)-PGC2(istart-1);  %PGC doesn't shift, so do this here.  PGC2 is cumsummed. 
        sumsSILB2b(n,:)=SILB2(iend)-SILB2(istart-1); %Similar, for the SILB-SSIB connection.
        for m=-mshift:mshift;
            sumsSSIB2(n,m+mshift+1)=SSIB2(iend-m)-SSIB2(istart-1-m); 
            sumsSILB2(n,m+mshift+1)=SILB2(iend-m)-SILB2(istart-1-m);
        end
    end
...
%         figure
%         hold on
%         plot(sumsPGSSn(n,:),'bo-','MarkerSize',5);
%         plot(xmaxPGSSn(n),ymaxPGSSn(n),'b*','MarkerSize',5);
%         plot(xloPGSSn(n),0.9*ymaxPGSSn(n),'b*','MarkerSize',5);
%         plot(xhiPGSSn(n),0.9*ymaxPGSSn(n),'b*','MarkerSize',5);
% 
%         plot(sumsPGSIn(n,:),'ro-','MarkerSize',5);
%         plot(xmaxPGSIn(n),ymaxPGSIn(n),'r*','MarkerSize',5);
%         plot(xloPGSIn(n),0.9*ymaxPGSIn(n),'r*','MarkerSize',5);
%         plot(xhiPGSIn(n),0.9*ymaxPGSIn(n),'r*','MarkerSize',5);
% 
%         plot(sumsSISSn(n,:),'ko-','MarkerSize',5);
%         plot(xmaxSISSn(n),ymaxSISSn(n),'k*','MarkerSize',5);
%         plot(xloSISSn(n),0.9*ymaxSISSn(n),'k*','MarkerSize',5);
%         plot(xhiSISSn(n),0.9*ymaxSISSn(n),'k*','MarkerSize',5);
% 
%         plot(zeros(2*mshift+1,1),'k:');
%         text(1, 0.,[int2str(winlen/40),' ',int2str(n)])
%     end
    ...
    subplot(kdiv,1,k,'align'); 
    hold on
    text(timlook-0.97*winbig/80,0.65*5,[int2str(winlen/40),' sec'],'FontSize',9)
    plot([timlook-winbig/80 timlook+winbig/80],zeros(2,1),'k:');
    plot(timswin,ampSISS*5/ampmax,'k+','MarkerSize',4);
    plot(timswin,ampPGSS*5/ampmax,'b+','MarkerSize',4);
    plot(timswin,ampPGSI*5/ampmax,'r+','MarkerSize',4);
    ...
    for n=1:nwin;
        plot([timswin(n),timswin(n)],[-5,-5+(xhiSISSn(n)-xloSISSn(n))],'k-');    
        plot([timswin(n)+1.6,timswin(n)+1.6],...
            [-5,-5+(xhiPGSIn(n)-xloPGSIn(n))],'r-');    
        plot([timswin(n)-1.6,timswin(n)-1.6],...
            [-5,-5+(xhiPGSSn(n)-xloPGSSn(n))],'b-');    
    end
    xlabel('sec')
    ylabel('samples')
    box on
    axis([timlook-timbig timlook+timbig -5 5]);
end
% orient landscape
% print('-dpdf',[JDAY,'_',int2str(timlook),'_new.pdf'])
print('-depsc',[JDAY,'_',int2str(timlook),'_wiggle2.eps'])
...
plot(timswin,(xmaxPGSIn-xmaxPGSSn+xmaxSISSn)*ampmax/5,'k+','MarkerSize',7);
plot(timswin,(xmaxPGSSn+0.15)*ampmax/5,'b+','MarkerSize',7);
plot(timswin,(xmaxPGSIn-0.15)*ampmax/5,'r+','MarkerSize',7);
