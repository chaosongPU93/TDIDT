...
%                     STA32file(nin+1,1:3)=[cumsumtrdiff/cumsumtr(winlen) xcmaxSTA32n(n) idiff];
                    ...
                    nin=nin+1;
                    %median ccmax w/in +/- 10,15,30 seconds
                    if timswin(n) > timswin(1)+10 && timswin(n) < timswin(end)-10 
                        ccmed20(nin)=median(xcmaxAVEn(n-10:n+10));
                    elseif timswin(n) > timswin(1)+10 
                        ccmed20(nin)=median(xcmaxAVEn(n-10:end));
                    else
                        ccmed20(nin)=median(xcmaxAVEn(1:n+10));
                    end
                    if timswin(n) > timswin(1)+15 && timswin(n) < timswin(end)-15 
                        ccmed30(nin)=median(xcmaxAVEn(n-15:n+15));
                    elseif timswin(n) > timswin(1)+15 
                        ccmed30(nin)=median(xcmaxAVEn(n-15:end));
                    else
                        ccmed30(nin)=median(xcmaxAVEn(1:n+15));
                    end
                    if timswin(n) > timswin(1)+30 && timswin(n) < timswin(end)-30 
                        ccmed60(nin)=median(xcmaxAVEn(n-30:n+30));
                    elseif timswin(n) > timswin(1)+30 
                        ccmed60(nin)=median(xcmaxAVEn(n-30:end));
                    else
                        ccmed60(nin)=median(xcmaxAVEn(1:n+30));
                    end
                    if timswin(n) > timswin(1)+60 && timswin(n) < timswin(end)-60 
                        ccmed120(nin)=median(xcmaxAVEn(n-60:n+60));
                    elseif timswin(n) > timswin(1)+60 
                        ccmed120(nin)=median(xcmaxAVEn(n-60:end));
                    else
                        ccmed120(nin)=median(xcmaxAVEn(1:n+60));
                    end
...
%                     loopoffkeep(nin,:)=[timswin(n) loopoff(n)];
                    ccmedfile(nin,:)=[timswin(n) xmaxSTA13ntmp(n) xmaxSTA12ntmp(n) xcmaxAVEn(n) ...
                        Ampsq(nin) Prev(nin) ccmed20(nin) ccmed30(nin) ccmed60(nin) ccmed120(nin) ];
                else
    ...
    fid = fopen(['ARMMAP/MAPS/NEW/ccmed',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
    fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %10.3e %10.3e %10.3f %10.3f %10.3f  %10.3f\n',ccmedfile(1:nin,:)');
    fclose(fid);
