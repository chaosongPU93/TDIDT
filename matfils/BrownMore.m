...
timoffrot=[254 07 41 00 +085 +020 -002 -034 -079 -182 -144  85 110  45  85 340]; %particular front
timoffrot=[246 04 26 36 +059 -085 +055 -174  000  000  000  90  60  45  140 30]; %good for KLN?
timoffrot=[192 03 25 16 +059 -085 +055 -174  000  000  000  90  60  45  140 30]; %good for KLN?
%***********************
jday=timoffrot(1);
JDAY=int2str(jday)
...
%plot(timsPGC,real(KLNBrot),'c','LineWidth',2);
plot(timsPGC,real(VGZrot),'c','LineWidth',2);
...
KLNBauto=realKLNB.*realKLNB;
KLNB2=cumsum(KLNBauto);
% 
mshift=0;
...
VGZtx=zeros(tracelenPGC-templen+1,ntemp); %no +1 so you can subtract ___(j-1)
VGZtn=zeros(tracelenPGC-templen+1,ntemp);
...
KLNBtx=zeros(tracelenPGC-templen+1,ntemp); %no +1 so you can subtract ___(j-1)
KLNBtn=zeros(tracelenPGC-templen+1,ntemp);
...
for n=1:ntemp;
    ...
    %for j=istart+1:tracelenPGC-templen+1 
        ...
        PGCtarg2=PGC2(j+templen-1)-PGC2(j-1);
        PGCtn(j,n)=PGCtx(j,n)/sqrt(PGCt2*PGCtarg2);
        VGZtx(j,n)=dot(VGZtempl,realVGZ(j:j+templen-1));
        VGZtarg2=VGZ2(j+templen-1)-VGZ2(j-1);
        VGZtn(j,n)=VGZtx(j,n)/sqrt(VGZt2*VGZtarg2);
        SSIBtx(j,n)=dot(SSIBtempl,realSSIB(j:j+templen-1));
        SSIBtarg2=SSIB2(j+templen-1)-SSIB2(j-1);
        SSIBtn(j,n)=SSIBtx(j,n)/sqrt(SSIBt2*SSIBtarg2);
        SILBtx(j,n)=dot(SILBtempl,realSILB(j:j+templen-1));
        SILBtarg2=SILB2(j+templen-1)-SILB2(j-1);
        SILBtn(j,n)=SILBtx(j,n)/sqrt(SILBt2*SILBtarg2);
        KLNBtx(j,n)=dot(KLNBtempl,realKLNB(j:j+templen-1));
        KLNBtarg2=KLNB2(j+templen-1)-KLNB2(j-1);
        KLNBtn(j,n)=KLNBtx(j,n)/sqrt(KLNBt2*KLNBtarg2);
        alltx(j,n)=PGCtx(j,n)+SSIBtx(j,n)+SILBtx(j,n);
        alltn(j,n)=alltx(j,n)/sqrt((PGCt2+SSIBt2+SILBt2)*(PGCtarg2+SSIBtarg2+SILBtarg2));
        tottn(j,n)=(PGCtn(j,n)+SSIBtn(j,n)+SILBtn(j,n))/3.0;
        if alltn(j,n) > 0.45
        %if tottn(j,n) > 0.42
            ...
            targfindt(f05,:)=timsPGC(j:j+templen-1);
            tempfindPGC(:,f05)=PGCtempl;
            tempfindVGZ(:,f05)=VGZtempl;
            tempfindSSIB(:,f05)=SSIBtempl;
            tempfindSILB(:,f05)=SILBtempl;
            tempfindKLNB(:,f05)=KLNBtempl;
            targfindPGC(:,f05)=realPGC(j:j+templen-1);
            targfindVGZ(:,f05)=realVGZ(j:j+templen-1);
            targfindSSIB(:,f05)=realSSIB(j:j+templen-1);
            targfindSILB(:,f05)=realSILB(j:j+templen-1);
            targfindKLNB(:,f05)=realKLNB(j:j+templen-1);
            find05(f05,:)=[n,j];
        end
    end
end
...
%                     targkeepSILB(:,mksi(icount))=0.;
        ...
        hold on
        plot(targkeepKLNB(:,mksi(icount)),'k');
        plot(tempkeepKLNB(:,mksi(icount)),'g');
        plot(crossoff,'k--')
        text(10, 0.8*ltmax, int2str(icount));
        text(100, 0.8*ltmax, num2str(maxkeep(mksi(icount))),'fontsize',7);
        text(100, 0.8*ltmin, num2str(keepKLNBtn(mksi(icount))),'fontsize',7);
        text(10, 0.8*ltmin, 'KLNB','fontsize',7);
        text(60, 0.6*ltmax, int2str(tempkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        text(110, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        %text(100, 0.6*ltmax, int2str(keepnj(mksi(icount),:)));
        axis([0 160 ltmin ltmax]);
        ...
        hold on
        plot(targkeepVGZ(:,mksi(icount)),'k');
        plot(tempkeepVGZ(:,mksi(icount)),'g');
        plot(crossoff,'k--')
        text(10, 0.8*ltmax, int2str(icount));
        text(100, 0.8*ltmax, num2str(maxkeep(mksi(icount))),'fontsize',7);
        text(100, 0.8*ltmin, num2str(keepVGZtn(mksi(icount))),'fontsize',7);
        text(10, 0.8*ltmin, 'VGZ','fontsize',7);
        text(60, 0.6*ltmax, int2str(tempkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        text(110, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        %text(100, 0.6*ltmax, int2str(keepnj(mksi(icount),:)));
        axis([0 160 ltmin ltmax]);
