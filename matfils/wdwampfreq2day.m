...
[PGCEf2nd]=1.6e-4*bandpass(PGCE,40,lo2nd,hi2nd,npo,npa,'butter');
[PGCNf2nd]=1.6e-4*bandpass(PGCN,40,lo2nd,hi2nd,npo,npa,'butter');
%[PGCZf]=1.6e-4*bandpass(PGCZ,40,lo2nd,hi2nd,npo,npa,'butter');
if year==2003 && jday<213
    [SSIBEf2nd]=20.0e-3*bandpass(SSIBE,100,lo2nd,hi2nd,npo,npa,'butter'); %times 5 until Aug 2003 5.0*
    [SSIBNf2nd]=20.0e-3*bandpass(SSIBN,100,lo2nd,hi2nd,npo,npa,'butter'); %times 5 until Aug 2003 5.0*
    [SILBEf2nd]=20.0e-3*bandpass(SILBE,100,lo2nd,hi2nd,npo,npa,'butter'); %times 5 until Aug 2003 5.0* 
    [SILBNf2nd]=20.0e-3*bandpass(SILBN,100,lo2nd,hi2nd,npo,npa,'butter'); %times 5 until Aug 2003 5.0*
else
    [SSIBEf2nd]=4.0e-3*bandpass(SSIBE,100,lo2nd,hi2nd,npo,npa,'butter'); 
    [SSIBNf2nd]=4.0e-3*bandpass(SSIBN,100,lo2nd,hi2nd,npo,npa,'butter'); 
    [SILBEf2nd]=4.0e-3*bandpass(SILBE,100,lo2nd,hi2nd,npo,npa,'butter'); 
    [SILBNf2nd]=4.0e-3*bandpass(SILBN,100,lo2nd,hi2nd,npo,npa,'butter'); 
end
...
SSIBEfd2nd = resample(SSIBEf2nd,2,5);
SSIBNfd2nd = resample(SSIBNf2nd,2,5);
SILBEfd2nd = resample(SILBEf2nd,2,5);
SILBNfd2nd = resample(SILBNf2nd,2,5);
...
PGC2nd=PGCEf2nd+1i*PGCNf2nd;
SSIB2nd=SSIBEfd2nd+1i*SSIBNfd2nd;
SILB2nd=SILBEfd2nd+1i*SILBNfd2nd;
PGCrot2nd=PGC2nd*exp(1i*rotPGC);
SSIBrot2nd=SSIB2nd*exp(1i*rotSSIB);
SILBrot2nd=SILB2nd*exp(1i*rotSILB);

if SSIBtoff > 0
    SSIBrot(1:tracelenPGC-SSIBsoff)=SSIBrot(SSIBsoff+1:tracelenPGC);
    SSIBrot(tracelenPGC-SSIBsoff+1:tracelenPGC)=0;
    SSIBrot2nd(1:tracelenPGC-SSIBsoff)=SSIBrot2nd(SSIBsoff+1:tracelenPGC);
    SSIBrot2nd(tracelenPGC-SSIBsoff+1:tracelenPGC)=0;
else
    SSIBrot(-SSIBsoff+1:tracelenPGC)=SSIBrot(1:tracelenPGC+SSIBsoff);
    SSIBrot(1:-SSIBsoff)=0;
    SSIBrot2nd(-SSIBsoff+1:tracelenPGC)=SSIBrot2nd(1:tracelenPGC+SSIBsoff);
    SSIBrot2nd(1:-SSIBsoff)=0;
end
if SILBtoff > 0
    SILBrot(1:tracelenPGC-SILBsoff)=SILBrot(SILBsoff+1:tracelenPGC);
    SILBrot(tracelenPGC-SILBsoff+1:tracelenPGC)=0;
    SILBrot2nd(1:tracelenPGC-SILBsoff)=SILBrot2nd(SILBsoff+1:tracelenPGC);
    SILBrot2nd(tracelenPGC-SILBsoff+1:tracelenPGC)=0;
else
    SILBrot(-SILBsoff+1:tracelenPGC)=SILBrot(1:tracelenPGC+SILBsoff);
    SILBrot(1:-SILBsoff)=0;
    SILBrot2nd(-SILBsoff+1:tracelenPGC)=SILBrot2nd(1:tracelenPGC+SILBsoff);
    SILBrot2nd(1:-SILBsoff)=0;
end
...
realPGC2nd=real(PGCrot2nd);
realSSIB2nd=real(SSIBrot2nd);
realSILB2nd=real(SILBrot2nd);
imagSILB2nd=imag(SILBrot2nd);
...
% instantaneous amplitude and frequecy using hi2nd, lo2nd filtering.
    hSI=hilbert(realSILB2nd(plotstart:plotend)); %For amplitude information, keep it as before
    SIamp=abs(hSI);
    hSIort=hilbert(imagSILB(plotstart:plotend));
    SIortamp=abs(hSIort);
%     SIfreq = 40*diff(unwrap(angle(hSI)))/(2*pi);
%     SIfreq(end+1)=SIfreq(end);
    hSI=hilbert(realSILB2nd(plotstart:plotend)); %For frequency information, use different passband
    xprime=0*(1:longlen);
    yprime=0*(1:longlen);
    ...
    SIfreq=((40/2)/(2*pi))*(realSILB2nd(plotstart:plotend).*yprime'-xprime'.*imag(hSI))./abs(hSI).^2; %divided by 2 b.c. of centered f.d.
    ...
    hSS=hilbert(realSSIB2nd(plotstartSS:plotendSS)); %longer because of potential shifts
    SSlen=plotendSS-plotstartSS+1;
    SSamp=abs(hSS);
    hSS=hilbert(realSSIB2nd(plotstartSS:plotendSS)); 
    xprime=0*(1:SSlen);
    yprime=0*(1:SSlen);
    ...
    SSfreq=((40/2)/(2*pi))*(realSSIB2nd(plotstartSS:plotendSS).*yprime'-xprime'.*imag(hSS))./abs(hSS).^2; %divided by 2 b.c. of centered f.d.
...
%    timoffrot(1,:)=[2005 257 07 42 12 +086 +020  80 115  50];
   ...
   %rubins=load('ARMMAP/MAPS/map2003.063.86.20.80.115.50_2-6-ms12-4s.dx.onepk.5');
   %rubins=load('ARMMAP/MAPS/map2003.064.86.20.80.115.50_2-6-ms12-4s.dx.onepeak0.5');
   %rubins=load('ARMMAP/MAPS/map2004.196.86.20.80.115.50_2-6-ms12-4s.dx.onepeak0.5'); 
   %rubins=load('ARMMAP/MAPS/map2004.199.86.20.80.115.50_2-6-ms19-4s.dx.onepk.5');
    rubins=load('ARMMAP/MAPS/map2005.254.86.20.80.115.50_2-6-ms12-4s.dx.onepeak0.5');
   %rubins=load('ARMMAP/MAPS/map2005.255.86.20.80.115.50_2-6-ms12-4s.dx.onepeak0.5');
   %rubins=load('ARMMAP/MAPS/map2005.256.86.20.80.115.50_2-6-ms12-4s.dx.onepeak0.5');
...
xlabel('log envelope amplitude')
ylabel('log frequency (Hz)')
title(['SILB  ',YEAR,'.','254-255','  ',num2str(lo),'-',num2str(hi),' Hz'])
print('-depsc',['Amp_v_FreqSILB_',YEAR,'.','254-255','_',num2str(lo),'-',num2str(hi),'.eps'])
...
xlabel('log envelope amplitude')
ylabel('log frequency (Hz)')
title(['SSIB  ',YEAR,'.','254-255','  ',num2str(lo),'-',num2str(hi),' Hz'])
print('-depsc',['Amp_v_FreqSSIB_',YEAR,'.','254-255','_',num2str(lo),'-',num2str(hi),'.eps'])
