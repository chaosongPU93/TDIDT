...
%    timoffrot(4,:)=[2003 068 07 42 12 +086 +020  80 115  50];
%    
...
%    timoffrot(9,:)=[2004 200 07 42 12 +086 +020  80 115  50];
% 
      timoffrot(1,:)=[2005 254 07 42 12 +086 +020  80 115  50];
%     timoffrot(11,:)=[2005 255 07 42 12 +086 +020  80 115  50];
%     timoffrot(12,:)=[2005 256 07 42 12 +086 +020  80 115  50];
%     timoffrot(13,:)=[2005 260 07 42 12 +086 +020  80 115  50];
...
mshift=11; %16 %11 %14 %12_for_+085+020 %15_for_+059-073  %19 for 128-s?
hi=6.;
lo=1.5;
%hi=12.;
%lo=1;
...
% lo=0.75;
loopoffmax=1.5
xcmaxAVEnmin=0.4 %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
winlen=5*40;
%winlen=128*40;
winoff=40/1;
%winoff=40*8;
...
timswin=3600.*(BostCat(:,3)-1)+BostCat(:,4);
nwin=length(timswin);
imid=40*timswin;
istart=imid-winlen/2;
...
%    timswin(n)=timsPGC(istart+winlen/2); 
    sumsPGSS(n,:)=cumsumPGSS(iend(n),:)-cumsumPGSS(istart(n)-1,:); 
    sumsPGSI(n,:)=cumsumPGSI(iend(n),:)-cumsumPGSI(istart(n)-1,:);
    sumsSISS(n,:)=cumsumSISS(iend(n),:)-cumsumSISS(istart(n)-1,:);
    sumsPGC2(n,:)=PGC2(iend(n)+mshift)-PGC2(istart(n)+mshift-1);  %PGC2 is cumsummed. Yes, +mshift.
    sumsSILB2b(n,:)=SILB2(iend(n)+mshift)-SILB2(istart(n)+mshift-1); %Similar, for the SILB-SSIB connection.
    for m=-mshift:mshift;
        sumsSSIB2(n,m+mshift+1)=SSIB2(iend(n)+mshift-m)-SSIB2(istart(n)+mshift-1-m); %+m??? (yes).
        sumsSILB2(n,m+mshift+1)=SILB2(iend(n)+mshift-m)-SILB2(istart(n)+mshift-1-m);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%            if abovecut > 0.9*belowcut %-1
                PGSStr=realPGC(istart(n):iend(n)).*realSSIB(istart(n)-imaxPGSSwr:iend(n)-imaxPGSSwr);
                PGSItr=realPGC(istart(n):iend(n)).*realSILB(istart(n)-imaxPGSIwr:iend(n)-imaxPGSIwr);
                SISStr=realSILB(istart(n)-imaxPGSIwr:iend(n)-imaxPGSIwr).*realSSIB(istart(n)-imaxPGSSwr:iend(n)-imaxPGSSwr);
                cumsumtr=cumsum(PGSStr)+cumsum(PGSItr)+cumsum(SISStr);
                [cumsumtrdiff idiff]=max(cumsumtr(41:winlen)-cumsumtr(1:winlen-40));

                PGCfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsPGC(istart(n):iend(n))' realPGC(istart(n):iend(n))];
                SSIBfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsPGC(istart(n):iend(n))' realSSIB(istart(n)-imaxPGSSwr:iend(n)-imaxPGSSwr)];
                SILBfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsPGC(istart(n):iend(n))' realSILB(istart(n)-imaxPGSIwr:iend(n)-imaxPGSIwr)];
...
fid = fopen(['BOSTOCK/NEW/BOSMAP/map',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa), ...
    '-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['BOSTOCK/NEW/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'b.eps'])
...
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['BOSTOCK/NEW/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'c.eps'])
...
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['BOSTOCK/NEW/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'d.eps'])

figure
colormap(jet)
scatter(xmaxPGSIn-xmaxPGSSn+xmaxSISSn,xcmaxAVEn,3,AmpComp(1:nwin))  
hold on 
...
box on
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
print('-depsc',['BOSTOCK/NEW/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'e.eps'])
        ...
        for m = 1:mcol
            ...
            if nt <= nin
                 %if PGSSfile(nt,1) >= 10 && PGSSfile(nt,1) <= 16 && PGSIfile(nt,1) >= 2 && PGSIfile(nt,1) <= 8
                    ...
                    box on
                    axis([is ien -yma yma])
                    set(gca,'XTick',[is is+(winlen/2)/40],'fontsize',6);
...
% The above for running-sum dot product
    ...
    orient landscape
    if ifig <= 9
        print('-depsc',['BOSTOCK/NEW/BOSMAP/WIGS/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'.',int2str(0),int2str(0),int2str(ifig),'.eps'])
    elseif ifig <= 99
        print('-depsc',['BOSTOCK/NEW/BOSMAP/WIGS/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'.',int2str(0),int2str(ifig),'.eps'])
    else
        print('-depsc',['BOSTOCK/NEW/BOSMAP/WIGS/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'.',int2str(ifig),'.eps'])
    end

end
%fid = fopen(['ARMMAP/MAPS/pks',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
fid = fopen(['BOSTOCK/NEW/BOSMAP/pks',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-nponpa',int2str(npo),int2str(npa),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
