...
SSIBSELFx=zeros(lenx, 2*mshift+1);
SILBSELFx=zeros(lenx, 2*mshift+1);
for n=-mshift:mshift;
    ...
    SISSx(:,n+mshift+1)=realSILB(1+mshift:tracelenPGC-mshift).* ...
        realSSIB(1+mshift-n:tracelenPGC-mshift-n);
    SSIBSELFx(:,n+mshift+1)=realSSIB(1+mshift:tracelenPGC-mshift).* ...
        SSIBZ(1+mshift-n:tracelenPGC-mshift-n);
    SILBSELFx(:,n+mshift+1)=realSILB(1+mshift:tracelenPGC-mshift).* ...
        SILBZ(1+mshift-n:tracelenPGC-mshift-n);
end
cumsumPGSS=cumsum(PGSSx);
cumsumPGSI=cumsum(PGSIx);
cumsumSISS=cumsum(SISSx);
cumsumSSIBSELF=cumsum(SSIBSELFx);
cumsumSILBSELF=cumsum(SILBSELFx);
xcshifts=cputime-t
t=cputime;
...
%Z
sumsSSIBSELF=zeros(nwin,2*mshift+1);
sumsSILBSELF=zeros(nwin,2*mshift+1);
sumsSILBZ2=zeros(nwin,2*mshift+1);
sumsSSIBZ2=zeros(nwin,2*mshift+1);
...
%Z
    sumsSSIB2b(n,:)=SSIB2(iend+mshift)-SSIB2(istart+mshift-1); %Inefficient!
    sumsSSIBSELF(n,:)=cumsumSSIBSELF(iend,:)-cumsumSSIBSELF(istart-1,:);
    sumsSILBSELF(n,:)=cumsumSILBSELF(iend,:)-cumsumSILBSELF(istart-1,:);
    ...
    for m=-mshift:mshift;
        ...
        sumsSSIBZ2(n,m+mshift+1)=SSIBZ2(iend+mshift-m)-SSIBZ2(istart+mshift-1-m);   %Inefficient!
        sumsSILBZ2(n,m+mshift+1)=SILBZ2(iend+mshift-m)-SILBZ2(istart+mshift-1-m);   %Inefficient!
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Z
denomSSIBSELFn=realsqrt(sumsSSIB2b.*sumsSSIBZ2);
denomSILBSELFn=realsqrt(sumsSILB2b.*sumsSILBZ2);

sumsPGSSn=sumsPGSS./denomPGSSn;
sumsPGSIn=sumsPGSI./denomPGSIn;
sumsSISSn=sumsSISS./denomSISSn;
%Z
sumsSSIBSELFn=sumsSSIBSELF./denomSSIBSELFn;
sumsSILBSELFn=sumsSILBSELF./denomSILBSELFn;
...
[xcmaxSSIBSELFn,imaxSSIBSELF]=max(sumsSSIBSELFn,[],2);
[xcmaxSILBSELFn,imaxSILBSELF]=max(sumsSILBSELFn,[],2);
%Parabolic fit:
...
[xmaxSSIBSELFn,ymaxSSIBSELFn]=parabol(nwin,mshift,sumsSSIBSELFn,imaxSSIBSELF);
[xmaxSILBSELFn,ymaxSILBSELFn]=parabol(nwin,mshift,sumsSILBSELFn,imaxSILBSELF);
...
imaxSSIBSELF=imaxSSIBSELF-mshift-1;
imaxSILBSELF=imaxSILBSELF-mshift-1;
...
xmaxSSIBSELFn=xmaxSSIBSELFn-mshift-1;
xmaxSILBSELFn=xmaxSILBSELFn-mshift-1;
...
%             xmaxSISSn(n)
%             xcmaxAVEn(n)
%             if abs(xmaxPGSSntmp(n))<=2 && abs(xmaxPGSIntmp(n))<=2
%                 figure
%                 plot(1:23,sumsPGSSn(n,:),'b*')
%                 hold on
%                 plot(1:23,sumsPGSIn(n,:),'r*')
%                 plot(1:23,sumsSSIBSELFn(n,:),'bo')
%                 plot(1:23,sumsSILBSELFn(n,:),'ro')
%             end
            nin=nin+1;
        else
            xmaxPGSSntmp(n)=20; xmaxPGSIntmp(n)=20; xmaxSISSntmp(n)=20; 
...
colormap(jet)
caxis([-1 1])
scatter(xmaxPGSIn-xmaxPGSSn+xmaxSISSn,xcmaxAVEn,3,xcmaxSSIBSELFn)
axis([-5 5 -0.2 1.0])
colorbar
...
colormap(jet)
caxis([-1 1])
scatter(xmaxPGSIn-xmaxPGSSn+xmaxSISSn,xcmaxAVEn,3,xcmaxSILBSELFn)
axis([-5 5 -0.2 1.0])
colorbar

figure
colormap(jet)
scatter(xmaxPGSSntmp,xmaxPGSIntmp,3,xmaxSSIBSELFn)
axis([-5 5 -0.2 1.0])
colorbar

figure
colormap(jet)
scatter(xmaxPGSSntmp,xmaxPGSIntmp,3,xmaxSILBSELFn)
caxis([-10 10])
axis([-5 5 -5 5])
colorbar
