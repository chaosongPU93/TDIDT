...
for nd=1:length(timoffrot(:,1))
    ...
    for ista=1:nstaP
        ...
        if LIA
            ...
            [optP]=readpermsP(prename,PERMSTA,PERMPROTS,idx,sps,lo,hi+4,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        ...
        if LIA
            ...
            [optP]=readpolsP(prename,POLSTA,POLPROTS,idx,sps,lo,hi+4,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
...
%                    %%%%%%%Above is for comparison to STRETCHED template, + only                   
                    ...
                    STA1file(nin*wintot+1:(nin+1)*wintot,1:2)=[timsSTA(istart-prewin:iend)' STAopt(1,istart-prewin:iend)'];
                    STA2file(nin*wintot+1:(nin+1)*wintot,1:2)=[timsSTA(istart-prewin:iend)' STAopt(2,istart-prewin-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3file(nin*wintot+1:(nin+1)*wintot,1:2)=[timsSTA(istart-prewin:iend)' STAopt(3,istart-prewin-imaxSTA13wr:iend-imaxSTA13wr)'];
                    for ii=1:nstaP
                        STAPfile(ii,nin*wintot+1:(nin+1)*wintot)=STAoptP(ii,istart-prewin:iend);
                    end
...
%                    STAamp(nin+1,:)=STAamp(nin+1,:)/STAamp(nin+1,1);
                    STA1bbfile(nin*wintot+1:(nin+1)*wintot,1:2)=[timsSTA(istart-prewin:iend)' STAoptbb(1,istart-prewin:iend)'];
                    STA2bbfile(nin*wintot+1:(nin+1)*wintot,1:2)=[timsSTA(istart-prewin:iend)' STAoptbb(2,istart-prewin-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3bbfile(nin*wintot+1:(nin+1)*wintot,1:2)=[timsSTA(istart-prewin:iend)' STAoptbb(3,istart-prewin-imaxSTA13wr:iend-imaxSTA13wr)'];
...
%    fid = fopen(['ARMMAP/MAPS/2018/map',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
%    fclose(fid);
%    fid = fopen(['ARMMAP/MAPS/2018/str',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
            ...
            for m = 1:mcol
                ...
                if nt <= nin
                     %if STA12file(nt,1) >= 10 && STA12file(nt,1) <= 16 && STA13file(nt,1) >= 2 && STA13file(nt,1) <= 8
                        ...
                        yma=max(max([STA1file(wintot*(nt-1)+1:wintot*nt,2) STA2file(wintot*(nt-1)+1:wintot*nt,2) ...
                            STA3file(wintot*(nt-1)+1:wintot*nt,2)]));
                        ymi=min(min([STA1file(wintot*(nt-1)+1:wintot*nt,2) STA2file(wintot*(nt-1)+1:wintot*nt,2) ...
                            STA3file(wintot*(nt-1)+1:wintot*nt,2)]));
                        fact=1.4;
                        yma=fact*max(yma,-ymi);
% Lines below plot + or - template; unstretched:
%                         if match(nt,1) >= match(nt,2) %if positive cc with template is larger ...
%                             plot(timstemp((nt-1)*templen+1:nt*templen),STAtemps(whichtoplot,:)*yma/fact,'c','linewidth',2)
%                         else %if cc with negative template is larger ...
%                             plot(timstempneg((nt-1)*templen+1:nt*templen),-STAtemps(whichtoplot,:)*yma/fact,'m','linewidth',2)
%                         end
...
% Lines above plot + stretched template:
                        hold on
                        plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),STA1file(wintot*(nt-1)+1:wintot*nt,2),'r')
                        plot(STA2file(wintot*(nt-1)+1:wintot*nt,1),STA2file(wintot*(nt-1)+1:wintot*nt,2),'b')
                        plot(STA3file(wintot*(nt-1)+1:wintot*nt,1),STA3file(wintot*(nt-1)+1:wintot*nt,2),'k')
                        is = STA1file(wintot*(nt-1)+1,1);
                        ien= STA1file(wintot*nt,1);
                        axis([is ien -yma yma])
                        ...
                        xcvect=[is+(prewin+STA32file(nt,3))/sps is+(prewin+STA32file(nt,3)+cncntr)/sps];
                        ycvect=[0.95*yma 0.95*yma];
                        plot(xcvect,ycvect,'b','linewidth',3)
                        plot([mean(xcvect)-POLPROTS(4)/sps,mean(xcvect)-POLPROTS(4)/sps],[-10 10],'k--')
...
%                         text(ien-0.6, 0.66*yma, int2str(STA12file(nt,1)),'fontsize',6);
                        ...
                        plot(STA1bbfile(wintot*(nt-1)+1:wintot*nt,1),STA1bbfile(wintot*(nt-1)+1:wintot*nt,2),'r')
                        hold on
                        plot(STA2bbfile(wintot*(nt-1)+1:wintot*nt,1),STA2bbfile(wintot*(nt-1)+1:wintot*nt,2),'b')
                        plot(STA3bbfile(wintot*(nt-1)+1:wintot*nt,1),STA3bbfile(wintot*(nt-1)+1:wintot*nt,2),'k')
% The following for running cc
                    STA12tr=STA1file(wintot*(nt-1)+1:wintot*nt,2).*STA2file(wintot*(nt-1)+1:wintot*nt,2);            
                    STA13tr=STA1file(wintot*(nt-1)+1:wintot*nt,2).*STA3file(wintot*(nt-1)+1:wintot*nt,2);
                    STA23tr=STA2file(wintot*(nt-1)+1:wintot*nt,2).*STA3file(wintot*(nt-1)+1:wintot*nt,2);
                    ...
                    cumsumSTA23tr=cumsum(STA23tr);
                        ...
                        ST1auto=STA1file(wintot*(nt-1)+1:wintot*nt,2).*STA1file(wintot*(nt-1)+1:wintot*nt,2);
                        ST1sq=cumsum(ST1auto);
                        ST2auto=STA2file(wintot*(nt-1)+1:wintot*nt,2).*STA2file(wintot*(nt-1)+1:wintot*nt,2);
                        ST2sq=cumsum(ST2auto);
                        ST3auto=STA3file(wintot*(nt-1)+1:wintot*nt,2).*STA3file(wintot*(nt-1)+1:wintot*nt,2);
                        ST3sq=cumsum(ST3auto);
                        ST12num=cumsumSTA12tr(cclen+1:wintot)-cumsumSTA12tr(1:wintot-cclen);
                        ST13num=cumsumSTA13tr(cclen+1:wintot)-cumsumSTA13tr(1:wintot-cclen);
                        ST23num=cumsumSTA23tr(cclen+1:wintot)-cumsumSTA23tr(1:wintot-cclen);
                        ST1den=ST1sq(cclen+1:wintot)-ST1sq(1:wintot-cclen);
                        ST2den=ST2sq(cclen+1:wintot)-ST2sq(1:wintot-cclen);
                        ST3den=ST3sq(cclen+1:wintot)-ST3sq(1:wintot-cclen);
                        ...
                        plot(STA1file(wintot*(nt-1)+cclen/2+1:wintot*nt-cclen/2,1),(yma+ymi)/2+(yma-ymi)*alln/2,'co','markersize',1)
% The above for running cc
                        plot([mean(xcvect)-POLPROTS(4)/sps,mean(xcvect)-POLPROTS(4)/sps],[-10 10],'k--')
                        is = STA1bbfile(wintot*(nt-1)+1,1);
                        ien= STA1bbfile(wintot*nt,1);
%                         yma=max(max([STA1bbfile(wintot*(nt-1)+1:wintot*nt,2) STA2bbfile(wintot*(nt-1)+1:wintot*nt,2) ...
%                             STA3bbfile(wintot*(nt-1)+1:wintot*nt,2)]));
%                         ymi=min(min([STA1bbfile(wintot*(nt-1)+1:wintot*nt,2) STA2bbfile(wintot*(nt-1)+1:wintot*nt,2) ...
%                             STA3bbfile(wintot*(nt-1)+1:wintot*nt,2)]));
%                         xvect=[is is+2*(yma-ymi)*(wintot/160.)]; %amplitude bar originally scaled for 4-s window
%                         yma=fact*max(yma,-ymi);
%                         yvect=[-0.9*yma -0.9*yma];
%                         plot(xvect,yvect,'r','linewidth',3)
...
%                     ymi=min(min([cumsumSTA12tr cumsumSTA13tr cumsumSTA23tr]));
                    hold on
                    %plot(STA1file(wintot*(nt-1)+cclen/2+1:wintot*nt-cclen/2,1),ST12n,'b')
                    %plot(STA1file(wintot*(nt-1)+cclen/2+1:wintot*nt-cclen/2,1),ST13n,'r')
                    %plot(STA1file(wintot*(nt-1)+cclen/2+1:wintot*nt-cclen/2,1),ST23n,'k')
                    %%plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),0.75*maxxc*ones(wintot,1),'k:');
                    %%plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),0.65*maxxc*ones(wintot,1),'k:');
% The following for running-sum dot product
%                     plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),cumsumSTA12tr,'k') %Use the color of the excluded station
%                     hold on
%                     plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),cumsumSTA13tr,'b')
%                     plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),cumsumSTA23tr,'r')
% The above for running-sum dot product

                    yma=max(STAPfile(1,wintot*(nt-1)+1:wintot*nt));
                    ymi=min(STAPfile(1,wintot*(nt-1)+1:wintot*nt));
                    fact=1.4;
                    yma=fact*max(yma,-ymi);
                    hold on
                    plot(STA1file(wintot*(nt-1)+1:wintot*nt,1),STAPfile(1,wintot*(nt-1)+1:wintot*nt),'k')
                    plot([mean(xcvect)-POLPROTS(4)/sps,mean(xcvect)-POLPROTS(4)/sps],[-10 10],'k--')
                    axis([is ien -yma yma])
...
%                    pkfile(nt,:)=[STA1file(winlen*(nt-1)+1+(winlen/2)) pks(imaxpk,1) maxpk pksid12 pksid13 pkwid rat14];
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',['ARMMAP/WIGS/2018/P/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',['ARMMAP/WIGS/2018/P/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',['ARMMAP/WIGS/2018/P/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(ifig),'.eps'])
        end
