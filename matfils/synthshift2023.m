...
%Builds a synthetic seismogram from the templates (could use a user-defined window (ginput)).
%shifts 2 and 3 w.r.t. 1 over a pre-defined range.
%This verison is very similar to 'chaosynth.m' and 'synthshift.m', the main
...
%Get templates.  ...le14sh0.75-15Hz_le14sh0.5-15Hz_le14sh are only modestly broad-band
datfiles=['PGCopt_002_1-15Hz_2pass_100sps.le14sh0.75-15Hz_le14sh0.5-15Hz_le14sh';
...
zcrosses=round(0.5*(imaxses+iminses));
sampbef=4*sps;
sampaft=10*sps;
is=zcrosses-sampbef;
ie=zcrosses+sampaft;
...
% [tlim,~]=ginput(2);
% tlim=round(tlim)
%tlim=[237 845] %just for consistency; could go back to picking
...
for ista=1:nsta
    ...
    green(ista,:)=green(ista,:)-mean(green(ista,:));
    x=(0:pi/(2*smooth):pi/2-pi/(2*smooth));
    green(ista,1:smooth)=sin(x).*green(ista,1:smooth); 
    x=fliplr(x);
    green(ista,end-(smooth-1):end)=sin(x).*green(ista,end-(smooth-1):end);
    green(ista,:)=green(ista,:)-mean(green(ista,:)); 
    green(ista,:)=green(ista,:)/max(abs(green(ista,:)));
end
mean(green,2)
subplot(2,1,1)
hold on
plot((1:length(green))/sps,green(1,:),'r')
plot((1:length(green))/sps,green(2,:),'b')
plot((1:length(green))/sps,green(3,:),'k')
...
for ista=1:nsta
%     greenf(ista,:)=bandpass(green(ista,:),100,1.25,6.5,2,2,'butter');
    greenf(ista,:)=Bandpass(green(ista,:),100,1.8,18,2,2,'butter');   % change 'bandpass' to 'Bandpass'
end
subplot(2,1,2)
hold on
plot((1:length(green))/sps,greenf(1,:),'r')
plot((1:length(green))/sps,greenf(2,:),'b')
plot((1:length(green))/sps,greenf(3,:),'k')
...
fracelsew=0.0; %0.25 %0.5; %0.6; %The ratio of elsewhere events to local events.  Zero for Discrete Ide.
...
synth=2.e-7*(rand(winlen+greenlen+2*40,nsta)-0.5); %+2*10 a little extra, for jiggering 2nd & 3rd stations. Plus making room for no immediate repeats.
...
%         xygrid(xygrid(:,5)<ll(1) | xygrid(:,5)>ur(1) | xygrid(:,6)<ll(2) | xygrid(:,6)>ur(2),:)=[]; %This limits xygrid to the desired range.  PGSS, PGSI, x, y, x', y'
    elseif strcmp(sourcestyle,'ellipse') %The following adds 0.5 km to y, rotates 45˚, and looks for an elliptical region with major semi-axis y' < 3 km and minor semi-axis < 2 km
        ...
        shiftor=[0 -0.5]; %(in km)
        %shiftor=[0 -0.]; %(in km)
        loc=complex(hexgrid(:,3)-shiftor(1),hexgrid(:,4)-shiftor(2));
        locrot=loc*exp(1i*angrot);
        hexgrid(:,5)=real(locrot);
        hexgrid(:,6)=imag(locrot);
        ...
        hexgrid(sqrt((hexgrid(:,5)/xaxis).^2 + (hexgrid(:,6)/yaxis).^2) > 1,:)=[]; %This limits xygrid to the desired range.  PGSS, PGSI, x, y, x', y'
    end
    ...
    xasp=asprad*cos(ang);
    yasp=asprad*sin(ang);
    plot(hexgrid(:,3),hexgrid(:,4),'.')
    axis equal
    hold on
    for hexi=1:size(hexgrid,1)
        plot(hexgrid(hexi,3)+xdiam,hexgrid(hexi,4)+ydiam,'b--')
        plot(hexgrid(hexi,3)+xasp,hexgrid(hexi,4)+yasp,'r')
    end
    xlim([-3 3])
    ylim([-3 3])
    ...
    fclose(fid);
    size(hexgrid)
    [synths,mommax,sources]=plaw3d(writes,winlen,skiplen,synth,green,b,hexgrid,sps,fracelsew,seed); 
end
if strcmp(distr,'LN')
    sig=1.;
...
if strcmp(distr,'UN')
    for inwrites=1:length(writes)
        ...
        if strcmp(sourcestyle,'ellipse')
            fid = fopen(['3STAS23/STAS.',distr,'.',int2str(sps),'sps.',sourcestyle(1:3),'_',num2str(xaxis),'-',num2str(yaxis),'.diam',num2str(diam),'.else',num2str(fracelsew,2),'nsat',num2str(nsat(inwrites))],'w');
        elseif strcmp(sourcestyle,'rectangle')
            fid = fopen(['3STAS23/STAS.',distr,'.',int2str(sps),'sps.',sourcestyle(1:3),'_',num2str(ll(1)),num2str(ll(2)),num2str(ur(1)),num2str(ur(2)),'.diam',num2str(diam),'.else',num2str(fracelsew,2),'nsat',num2str(nsat(inwrites))],'w');
        end
        towrite=squeeze(synths(:,:,inwrites));
        fprintf(fid,'%13.5e %13.5e %13.5e\n', towrite');
        fclose(fid);
        if strcmp(sourcestyle,'ellipse')
            fid = fopen(['3STAS23/STAS.',distr,'.',int2str(sps),'sps.',sourcestyle(1:3),'_',num2str(xaxis),'-',num2str(yaxis),'.diam',num2str(diam),'.else',num2str(fracelsew,2),'nsat',num2str(nsat(inwrites)),'_sources'],'w');
        elseif strcmp(sourcestyle,'rectangle')
            fid = fopen(['3STAS23/STAS.',distr,'.',int2str(sps),'sps.',sourcestyle(1:3),'_',num2str(ll(1)),num2str(ll,2),num2str(ur(1)),num2str(ur,2),'.diam',num2str(diam),'.else',num2str(fracelsew,2),'nsat',num2str(nsat(inwrites)),'_sources'],'w');
        end
        truncsources=squeeze(sources(:,:,inwrites));
        truncsources(truncsources(:,1)==0,:)=[];
        fprintf(fid,'%13i %5i \n', truncsources');
        fclose(fid);
    end
end
