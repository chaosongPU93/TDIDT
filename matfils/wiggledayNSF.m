...
%    timoffrot=[261 05 19 48 +085 +020  80 115  55]; % Sept. 18.  Nothing at this spot.
   ...
   %timoffrot=[2003 063 14 15 32 +085 +020  80 115  55]; %faked
...
% [PGpks,PGlocs]= findpeaks(realPGC);
% findpkPG=cputime-t
% t=cputime;
% [SSpks,SSlocs]= findpeaks(realSSIB);
% [SIpks,SIlocs]= findpeaks(realSILB);
ampmax=max([ampPGSS; ampPGSI; ampSISS]);
figure 
%subplot(4,1,1,'align'); 
axes('position',[0.08 0.8 0.9 0.12]) %,'FontSize',6,'DataAspectRatio',[1 1 1],'XLim',[-4 4],'YLim',[-4 4])
hold on
plot(timswin,xcmaxAVEnmin*10+zeros(nwin,1),'k:');
plot(timsPGC(winlen:2*winlen),7+zeros(winlen+1,1),'k','linewidth',2);
plot(timswin,zeros(nwin,1),'k:');
plot(timswin,xcmaxAVEn*10,'g');
axis([0 timbig/2 -10 10]);
hrf = plotreflinesr(gca,detects,'x','c'); 
hrf = plotreflinesr(gca,Detects,'x','m'); 
hrf = plotreflinesr(gca,detects2_8,'x','c'); 
hrf = plotreflinesr(gca,Detects2_8,'x','m'); 
plot(timsPGC,4*realPGC-10,'c');
plot(timsPGC,4*realSSIB-10,'c');
plot(timsPGC,4*realSILB-10,'c');
% plot(PGlocs,0.2*PGpks-10,'r');
% plot(SSlocs,0.2*SSpks-10,'b');
% plot(SIlocs,0.2*SIpks-10,'k');
...
box on
%subplot(4,1,2,'align'); 
axes('position',[0.08 0.63 0.9 0.12])
hold on
plot(timswin,xcmaxAVEnmin*10+zeros(nwin,1),'k:');
plot(timswin,zeros(nwin,1),'k:');
plot(timswin,xcmaxAVEn*10,'g');
...
plot(timswin,xmaxSISSntmp,'k*','MarkerSize',2);
ylabel('samples')
xlabel('seconds on Sept. 12, 2005')
box on
% % subplot(4,1,3,'align'); 
% % hold on
% % plot(timswin,xcmaxAVEnmin*10+zeros(nwin,1),'k:');
...
% % hrf = plotreflinesr(gca,Detects2_8,'x','m'); 
% % plot(timsPGC,4*realPGC-10,'c');
% % plot(timsPGC,4*realSSIB-10,'c'); %, 2003
% % plot(timsPGC,4*realSILB-10,'c');
% % plot(timswin,xmaxPGSSntmp,'bs','MarkerSize',2);
% % plot(timswin,xmaxPGSIntmp,'ro','MarkerSize',2);
% % plot(timswin,xmaxSISSntmp,'k*','MarkerSize',2);
