% A new version of wdwcohere.m, from wdwSTAdisplay.m on 08.30.18
% 5 panels, including phase (each station individually) and coherence (3 pairs).
...
if isequal(fam,'002')  %DON'T FORGET additional family-specific delcarations around line 156
    %timoffrot= [2005 255];
%    displaytim=[43019 43020 43021 57544 57545 57546];
...
loopoffmax=1.9; %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.
xcmaxAVEnmin=0.4; %0.4; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Get the templates for the stations. Won't be always required.  PGCopt_002_1.25-6.5Hz_2pass_100sps.le14shift.resamp
for ista=1:nsta
    if stas(ista,4) == ' '
        temptemps(ista,:)=load([stas(ista,1:3),'opt_',fam,'_','1.25','-','6.5', ...  %1.25-6.5 a kludge
                         'Hz_',int2str(npa),'pass_',int2str(sps),'sps.input'],'w');
    else
        temptemps(ista,:)=load([stas(ista,1:4),'opt_',fam,'_','1.25','-','6.5', ...  %1.25-6.5 a kludge
                         'Hz_',int2str(npa),'pass_',int2str(sps),'sps.input'],'w');
    end
end
% tempbef=70;
...
%     sumsSILB2b=zeros(nwin,2*mshift+1);
    ...
    %  The following looks like it has the proper relative shifts but the
    %  absolute time isn't registered to the global igstart properly.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for n=1:nwin;
...
%                   clear dummy
    ...
    fid = fopen(['ARMMAP/MAPS/2018Aug/map',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
%                     pkfile(nt,:)=[STA1file(winlen*(nt-1)+1+(winlen/2)) pks(imaxpk,1) maxpk pksid12 pksid13 pkwid rat14];
                    ...
                    [STA1bbx f1] = pmtm(STA1bb,[],[],sps);
                    [STA2bbx f1] = pmtm(STA2bb,[],[],sps);
                    [STA3bbx f1] = pmtm(STA3bb,[],[],sps);
                    loglog(f1,sqrt(STA1bbx),'r')
                    hold on
                    loglog(f1,sqrt(STA2bbx),'b')
                    loglog(f1,sqrt(STA3bbx),'k')
                    yma=max(max([sqrt(STA1bbx) sqrt(STA2bbx) sqrt(STA3bbx)]));
                    ymi=0.05*yma;
                    xlim([0.02 20])
                    ylim([ymi yma])
                    ...
                    subplot(npanels*nrow,mcol,npanels*(n-1)*mcol+4*mcol+m,'align');
                    NFFT = 2^nextpow2(winlen); % Next power of 2 from length of y
                    fft1=fft(STA1bb,NFFT)/winlen;
                    fft2=fft(STA2bb,NFFT)/winlen;
                    fft3=fft(STA3bb,NFFT)/winlen;
                    f = sps/2*linspace(0,1,NFFT/2+1);
                    semilogx(f,angle(fft1(1:NFFT/2+1))/pi,'r') %Don't know if this plots phase of + or - frequencies
                    hold on
                    semilogx(f,angle(fft2(1:NFFT/2+1))/pi,'b')
                    semilogx(f,angle(fft3(1:NFFT/2+1))/pi,'k')
%                     loglog(f,2*abs(fft1(1:NFFT/2+1)),'r')
%                     hold on
%                     loglog(f,2*abs(fft2(1:NFFT/2+1)),'b')
%                     loglog(f,2*abs(fft3(1:NFFT/2+1)),'k')
%                     yma=max(max([2*abs(fft1) 2*abs(fft2) 2*abs(fft3)]));
%                     ymi=0.05*yma;
                    xlim([0.2 20])
                    ylim([-1 1])
                    
                    %S1 = spectrogram(STA1bb,32,[],[],40);
                    %S2 = spectrogram(STA2bb,32,[],[],40);
                    %S3 = spectrogram(STA3bb,32,[],[],40);
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',['ARMMAP/WIGS/2018Aug/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',['ARMMAP/WIGS/2018Aug/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',['ARMMAP/WIGS/2018Aug/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(ifig),'.eps'])
        end
...
%     bostsecfile(2,:)=1;    
%     fid = fopen(['ARMMAP/WIGS/2018Aug/bostsec',IDENTIF(1:8)],'w');
%     fprintf(fid,'%10.3f %12.4e \n',bostsecfile(:,:));
%     fclose(fid);
%     fid = fopen(['ARMMAP/WIGS/2018Aug/wigdat',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
%     fprintf(fid,'%10.3f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e \n',datfile(:,:));
%     fclose(fid);
%     fid = fopen(['ARMMAP/WIGS/2018Aug/guide',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
