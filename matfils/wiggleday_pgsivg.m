...
%timoffrot=[246 04 22 20 +060 -085  85  60  40];
   %timoffrot=[246 04 25 16 +060 -085  90  60  45]; %agu
...
%    timoffrot=[253 05 19 48 +085 +020  80 115  55]; 
%    timoffrot=[254 05 19 48 +085 +020  80 115  55]; %Starts today
    timoffrot=[255 05 19 48 +085 +020  80 115  55]; %bingo
%    timoffrot=[255 09 42 04 +089 +031  80 105  50]; %nearby
%    timoffrot=[256 05 19 48 +085 +020  80 115  55]; %v. little
...
[xmaxPGSSn,ymaxPGSSn]=parabol(nwin,mshift,sumsPGSSn,imaxPGSS);
...
for n=1:nwin;
    if xcmaxAVEn(n)<0.33 || abs(xmaxPGSIn(n)-xmaxPGSSn(n)+xmaxSISSn(n))>1.5 
