...
SSIBROTS=[6 pi*85/180 pi*33/180 86];  %SSIB
SSIB=SSIBEfd+1i*SSIBNfd;
SSIBfastslow=SSIB*exp(-1i*SSIBROTS(2));
SSIBslow=real(SSIBfastslow);
SSIBfast=imag(SSIBfastslow);
SSIBslow(7:tracelenPGC-7)=SSIBslow(7+SSIBROTS(1):tracelenPGC-7+SSIBROTS(1));
SSIBsplitcorrected=(SSIBslow+1i*SSIBfast)*exp(1i*SSIBROTS(2));
SSIBrot=SSIBsplitcorrected*exp(-1i*SSIBROTS(3));
...
%     ltmin=min([realSSIB(plotstart:plotend); realSILB(plotstart:plotend)]);
    ...
    for j=1:npages
       ...
       orient landscape
       print(h,'-depsc',['ARMMAP/SSIBWIGS/',YEAR,'.',JDAY,'.',int2str(timsPGC(istart)),'_',num2str(lo),'-',num2str(hi),'eye.eps'])
    end
end
