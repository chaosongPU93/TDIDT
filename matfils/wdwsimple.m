...
%    timoffrot(39,:)=[2003 065 07 42 12 +086 +027  80 120  45];
   ...
   timoffrot(5,:)=[2005 258 07 42 12 +086 +020  80 120  50];
  
   timoffrot(6,:)=[2004 196 07 42 12 +086 +020  80 115  50];
   timoffrot(7,:)=[2004 197 07 42 12 +086 +020  80 115  50];
   timoffrot(8,:)=[2004 198 07 42 12 +086 +020  80 115  50];
   timoffrot(9,:)=[2004 199 07 42 12 +086 +020  80 115  50];
...
shorterm=16; %short-term window for signal strength
for n=1:nwin
%     if timswin(n) > 35204
        ...
        if abs(iPGSSbang-imaxinterpPGSS) <= loopoffmax*iup && ...
           ...
           interpPGSSn(iPGSSbang)+interpPGSIn(iPGSIbang)+interpSISSn(iSISSbang) >= 3*xcmaxAVEnmin
            ...
            if abovecut > 0.9*belowcut %-1
                ...
                [cumsumtrdiff idiff]=max(cumsumtr(2*shorterm+1:winlen-shorterm)-cumsumtr(shorterm+1:winlen-2*shorterm));  %shorterm added 6/24/2015
...
[sortedsimp isimp]=sort(mapfile(1:nin,9),'descend');
...
for ifig=1:floor(nin/(nrow*mcol))+1
    ...
    for n = 1:nrow
        for m = 1:mcol
            nfict=nfict+1;
            if nfict <= nin
                    nt =  isimp(nfict);  %this chooses the appropriate value in the file PGSIfile (e.g.), based on sorted simpleness
                 %if PGSSfile(nt,1) >= 10 && PGSSfile(nt,1) <= 16 && PGSIfile(nt,1) >= 2 && PGSIfile(nt,1) <= 8
                    ...
                    box on
                    axis([is ien -yma yma])
                    set(gca,'XTick',[is is+2],'fontsize',6); %changed to is+1 from is+2 for 2s windows
                    if (n==1) 
                        if (m==1) 
                            title(IDENTIF)
                        elseif (m==2) 
                            title([int2str(lo),'-',int2str(hi),'.',int2str(winlen/40),'s.'])
                        elseif (m==3)
                            title(['cc thresh ',num2str(xcmaxAVEnmin)])
                        end
                    end
...
%                     plot(f,Cpgsi,'r')
%                     plot(f,Csiss,'k')
                    plot(f,PGCxx/max(PGCxx),'r+','MarkerSize',2)
                    hold on
                    plot(f,SSIBxx/max(SSIBxx),'b+','MarkerSize',2)
                    plot(f,SILBxx/max(SILBxx),'k+','MarkerSize',2)
                    xlim([0 hi-2])
                    %xlim([0.1 hi+1])
                    %ylim([0.01 1])
...
% The above for running cc
    ...
    orient landscape
    if ifig <= 9
        print('-depsc',['ARMMAP/SIMPLE/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'.',int2str(winlen/40),'s.',int2str(0),int2str(0),int2str(ifig),'.eps'])
    elseif ifig <= 99
        print('-depsc',['ARMMAP/SIMPLE/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'.',int2str(winlen/40),'s.',int2str(0),int2str(ifig),'.eps'])
    else
        print('-depsc',['ARMMAP/SIMPLE/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'.',int2str(winlen/40),'s.',int2str(ifig),'.eps'])
    end
