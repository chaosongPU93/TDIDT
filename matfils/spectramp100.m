...
%         STAZfd = resample(STAZf,2,5);
        ...
        STAfast=imag(STAfastslow);
        len=length(STA);
        STAslow(25:len-25)=STAslow(25+POLROTS(ista,1):len-25+POLROTS(ista,1));
    ...
    %     aaa(i+1,:)=(aa(end,:)-aa(i*64,:))/(itot-i*64);
        ...
        if noise == 1
            noise1=aaa;
            save(['noise',POLSTA(ista,:),'100-',JDAY,'_win',int2str(swin),'.',filt,'.mat'],'noise1')
        else
            ...
            for iday=1:length(noisedays)
                ...
                load (['noise',POLSTA(ista,:),'100-',JNDAY,'_win',int2str(swin),'.',filt,'.mat']);
                loglog(f1,noise1,'color',[0 0 0]+0.75)
                noisekeep(ndivs*(iday-1)+1:ndivs*iday,:)=noise1;
            end
        end
        noiseprct=prctile(noisekeep,[10 50 90]);
%         loglog(f1,noiseprct(1,:),'c--')
%         loglog(f1,noiseprct(3,:),'c--')
        ...
        if noise == 1
            noise1Z=ddd;
            save(['noise',POLSTA(ista,:),'.Z100-',JDAY,'_win',int2str(swin),'.',filt,'.mat'],'noise1Z')
        else
           ...
           for iday=1:length(noisedays)
                ...
                load (['noise',POLSTA(ista,:),'.Z100-',JNDAY,'_win',int2str(swin),'.',filt,'.mat']);
...
%         %end
        ...
        title([POLSTA(ista,:),'  ',YEAR,'.',JDAY,'  win',num2str(swin/sps),'  ',filt,'  noiseZ/noiseH'])
        xlim([0.1 50])
        ylim([2.e-1 5])
        loglog([1.e-5 1.e5],[1 1],'k--')
        ...
        hold on
        loglog(f1,mean(noisekeepZ)./mean(noisekeep),'c')
        title([POLSTA(ista,:),'  ',YEAR,'.',JDAY,'  win',num2str(swin/sps),'  ',filt,'  vert/opt'])
        xlim([0.1 50])
        ylim([2.e-1 5])
        loglog([1.e-5 1.e5],[1 1],'k--')

        set(h,'PaperPosition',[0.25 0.25 8 10])
        print(h,'-depsc',['2018/spectr',POLSTA(ista,:),YEAR,'.',JDAY,'_win',int2str(swin),'.',filt,'100.eps'])
        clear STAoptxx
        clear STAortxx
        clear STAvrtxx
    end
...
%     print(h,'-depsc',['2018/spectr',YEAR,'.',JDAY,'_win',int2str(swin),'.',filt,'100.eps'])
    ...
    if noise 
        noise3=aaatot/nsta;
        save(['noise',int2str(nsta),'100-',JDAY,'_win',int2str(swin),'.',filt,'.mat'],'noise3')
        noise3Z=dddtot/nsta;
        save(['noise',int2str(nsta),'.Z100-',JDAY,'_win',int2str(swin),'.',filt,'.mat'],'noise3Z')
    else
        h=figure('Position',[wid/10 hite/9 1.*wid hite/1.2]);
        subplot(2,1,1,'align')
        loglog(f1,aaatot/nsta,'.')
        ...
        for iday=1:length(noisedays)
            ...
            load (['noise',int2str(nsta),'100-',JNDAY,'_win',int2str(swin),'.',filt,'.mat']);
        ...
        xlim([5e-2 4e1])
        axis square
        title([allsta,'  ',YEAR,'.',JDAY,'  ',num2str(swin/sps),'-s win','  opt'])
        ...
        for iday=1:length(noisedays)
            ...
            load (['noise',int2str(nsta),'.Z100-',JNDAY,'_win',int2str(swin),'.',filt,'.mat']);
...
%       normlz=mean(dddtot(:,f1min:f1max),2);
        ...
        xlim([5e-2 4e1])
        axis square
        title([allsta,'  ',YEAR,'.',JDAY,'  ',num2str(swin/sps),'-s win','  vert'])

        set(h,'PaperPosition',[0.25 0.25 8 10])
        print('-depsc',['2018/spectr',YEAR,'.',JDAY,'_win',int2str(swin),'.',filt,'square100.eps'])
    end
