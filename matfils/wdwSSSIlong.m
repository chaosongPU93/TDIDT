...
%timoffrot= [2004 196]; %002. Could leave out 064 and 257.
% timoffrot= [2003 062
%             2003 063]; %002. 
timoffrot= [2003 062; %002. Could leave out 064 and 257.
            2003 063;
%           2003 064;
            2004 196;
            2004 197;
...
%bostname=['BOSTOCK/NEW/002-246_2004.196']; %002
%bostname=['BOSTOCK/NEW/002-246_2003.062']; %002
bostname=['BOSTOCK/NEW/002-246_2003.062'; %002
...
% bostname=['BOSTOCK/NEW/068_2004.198'; %068
...
fam='002';
PERMROTS=[0 90 32 0;  %PGC , Yajun's "0" changed to 90.  With no splitting I think it doesn't matter.
         0 90 54 9]; %LZB
...
npo=2;
npa=2;
mshift=3; %13; %maximum shift for the x-correlations. 19 for 002 Stanford
loopoffmax=5; %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.
xcmaxAVEnmin=0.2; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
xcmaxSSSImin=0.4; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Get the templates for the stations. Won't be always required.
for ista=1:nsta
    if stas(ista,4) == ' '
        temptemps(ista,:)=load([stas(ista,1:3),'opt_',bostname(1,13:15),'_',num2str(lo),'-',num2str(hi), ...
                         'Hz_',int2str(npa),'pass_',num2str(sps),'sps'],'w');
    else
        temptemps(ista,:)=load([stas(ista,1:4),'opt_',bostname(1,13:15),'_',num2str(lo),'-',num2str(hi), ...
                         'Hz_',int2str(npa),'pass_',num2str(sps),'sps'],'w');
    end
end
tempoffs=[911 997 931]; %these are the zero crossings for 002: PGC,SSIB,SILB.  87 or 86 samples for B's templates, it seems. 
...
%%% SCALEFACT WILL SCALE BOTH TEMPLATES AND SEISMOGRAMS.  Strategy changes with family.
%%% 002
%scalefact=max(max(abs(STAtemps))); 
%whichtoplot=2;
%%% 068
...
scalefact=scalefact/scalefact(1); %This is used to scale seismograms
%figure
%plot(STAtemps(1,:),'r')
%hold on
%plot(STAtemps(2,:),'b')
...
for nd=1:length(timoffrot(:,1))
    ...
    for ista=1:nsta
        ...
        STAopt(ista,:)=opt/scalefact(ista);
        %STAort(ista,:)=ort;
    ...
    for ista=1:nsta
        ...
        STAoptbb(ista,:)=opt/scalefact(ista);
    end
    lolo=0.5;
    hilo=1.5;
    STAoptlo=zeros(nsta,tracelen);
    for ista=1:nsta
        [LIA,idx]=ismember(stas(ista,:),PERMSTA,'rows');
        if LIA
            if strcmp(PERMSTA(idx,1:3),'PGC')
            ...
            [opt,~,~,~]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lolo,hilo,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        [LIA,idx]=ismember(stas(ista,:),POLSTA,'rows');
        if LIA
            if year==2003 && jday<213
            ...
            [opt,~,~]=readpols(prename,POLSTA,POLROTS,idx,sps,lolo,hilo,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        STAoptlo(ista,:)=opt/scalefact(ista);
    end
    lomid=1.5;
    himid=4.5;
    STAoptmid=zeros(nsta,tracelen);
    for ista=1:nsta
        [LIA,idx]=ismember(stas(ista,:),PERMSTA,'rows');
        if LIA
            if strcmp(PERMSTA(idx,1:3),'PGC')
            ...
            [opt,~,~,~]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lomid,himid,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        [LIA,idx]=ismember(stas(ista,:),POLSTA,'rows');
        if LIA
            if year==2003 && jday<213
            ...
            [opt,~,~]=readpols(prename,POLSTA,POLROTS,idx,sps,lomid,himid,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        STAoptmid(ista,:)=opt/scalefact(ista);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     sumsSILB2b=zeros(nwin,2*mshift+1);
    ...
    for n=1:nwin
        if xcmaxAVEn(n)<xcmaxAVEnmin || xcmaxSTA32n(n)<xcmaxSSSImin || abs(xmaxSTA13n(n)-xmaxSTA12n(n)+xmaxSTA32n(n))>loopoffmax ...
...
% I will ignore all this above
                imaxSTA12wr=imaxSTA12cent(n); %Just what it was before 
                imaxSTA13wr=imaxSTA12cent(n)-round(xmaxSTA32n(n));
    % 
                ...
                if abovecut > 0.9*belowcut %-1 %This checks for frequency range; make sure it's not too narrow?
                    ...
                    dummy=STAopt(1,isdiff-2.5*cncntr:isdiff).*STAopt(1,isdiff-2.5*cncntr:isdiff)+ ...
                          STAopt(2,isdiff-2.5*cncntr-imaxSTA12wr:isdiff-imaxSTA12wr).*STAopt(2,isdiff-2.5*cncntr-imaxSTA12wr:isdiff-imaxSTA12wr)+ ...
                          STAopt(3,isdiff-2.5*cncntr-imaxSTA13wr:isdiff-imaxSTA13wr).*STAopt(3,isdiff-2.5*cncntr-imaxSTA13wr:isdiff-imaxSTA13wr);
                    dum2=cumsum(dummy);
                    Prev(nin+1)=dum2(end);
                    clear dummy
                    ...
                    dummy(1,:)=STAopt(1,isdiff-2.5*cncntr:isdiff-1);
                    dummy(2,:)=STAopt(2,isdiff-imaxSTA12wr-2.5*cncntr:isdiff-imaxSTA12wr-1);
                    dummy(3,:)=STAopt(3,isdiff-imaxSTA13wr-2.5*cncntr:isdiff-imaxSTA13wr-1);
                    denoms=dot(dummy,dummy,2);
                    ...
                    if isdiff > winlen+mshift
                        dummy(1,:)=STAopt(1,isdiff-winlen:isdiff-1);
                        dummy(2,:)=STAopt(2,isdiff-imaxSTA12wr-winlen:isdiff-imaxSTA12wr-1);
                        dummy(3,:)=STAopt(3,isdiff-imaxSTA13wr-winlen:isdiff-imaxSTA13wr-1);
                    else
                        dummy(1,:)=STAopt(1,mshift:isdiff-1);
                        dummy(2,:)=STAopt(2,mshift-imaxSTA12wr:isdiff-imaxSTA12wr-1);
                        dummy(3,:)=STAopt(3,mshift-imaxSTA13wr:isdiff-imaxSTA13wr-1);
                    end
                    denoms=dot(dummy,dummy,2);
...
%                     traces(nin*(3*templen)+1:nin*(3*templen)+templen)          =STAopt(1,indx-tempbef:indx+tempaft);
%                     traces(nin*(3*templen)+templen+1:nin*(3*templen)+2*templen)= ...
%                                                         STAopt(2,indx-tempbef-imaxSTA12wr:indx+tempaft-imaxSTA12wr);
%                     traces(nin*(3*templen)+2*templen+1:(nin+1)*(3*templen))    = ...
%                                                         STAopt(3,indx-tempbef-imaxSTA13wr:indx+tempaft-imaxSTA13wr);
                    ...
                    tempxc=xcorr(traces,snips,round(3*cncntr/4),'coeff');
                    [match(nin+1,1) ioff]=max(tempxc);
                    ioff=ioff-(round(3*cncntr/4)+1); %shift "snips" by match(nin+1) (shift right for positive values)
                    timstemp(nin*templen+1:(nin+1)*templen)=timsSTA(indx-tempbef+ioff:indx+tempaft+ioff);
                    tempxcneg=xcorr(traces,-snips,round(3*cncntr/4),'coeff');
                    [match(nin+1,2) ioff]=max(tempxcneg);
                    ioff=ioff-(round(3*cncntr/4)+1); %shift "snips" by match(nin+1) (shift right for positive values)
                    ...
                    STA1lofile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptlo(1,istart:iend)'];
                    STA2lofile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptlo(2,istart-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3lofile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptlo(3,istart-imaxSTA13wr:iend-imaxSTA13wr)'];
                    STA1midfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptmid(1,istart:iend)'];
                    STA2midfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptmid(2,istart-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3midfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptmid(3,istart-imaxSTA13wr:iend-imaxSTA13wr)'];
                    ...
                    mapfile(nin,:)=[timswin(n) xmaxSTA13ntmp(n) xmaxSTA12ntmp(n) ...
                        ...
                        ccprior125(nin) ccprior(nin) ccpost(nin) xcmaxSTA12n(n) xcmaxSTA13n(n) xcmaxSTA32n(n) ];
                else
    ...
    fid = fopen(['ARMMAP/MAPS/SSSI/map',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
    fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %7.2f %10.3e %10.3e %10.3f %7.3f %7.3f %7.3f %10.3e %10.3e %10.3e %10.3e %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f\n',mapfile(1:nin,:)');
    fclose(fid);
    figure 
    subplot(4,1,1,'align'); 
    hold on
    ...
    orient landscape
    print('-depsc',['FIGS/SSSI',IDENTIF,'-',int2str(winlen/sps),'s_',num2str(lo),'-',num2str(hi),'b.eps'])
    ...
    box on
    title([IDENTIF,'_{',num2str(lo),'-',num2str(hi),'}'])
    print('-depsc',['FIGS/SSSI',IDENTIF,'-',int2str(winlen/40),'s_',num2str(lo),'-',num2str(hi),'e.eps'])
...
%% Lines above plot + or - template:
                        ...
                        text(is+0.4, 0.66*yma, int2str(STA13file(nt,1)),'fontsize',6);
                        text(ien-0.8, 0.66*yma, int2str(STA12file(nt,1)),'fontsize',6);
                        if n ==1 && m ==1
                            title([int2str(timoffrot(nd,1)),'.',int2str(timoffrot(nd,2)),'  p.',int2str(ifig)])
                        end
                        box on
                        set(gca,'XTick',(is:5:ien),'fontsize',6);

                        subplot(npanel*nrow,mcol,npanel*(n-1)*mcol+mcol+m,'align');
                        hold on
                        %plot(STA1bbfile(winlen*(nt-1)+1:winlen*nt,1),STA1bbfile(winlen*(nt-1)+1:winlen*nt,2),'r') %Comment out only for low amplitude
...
%                         yvect=[-0.9*yma -0.9*yma];
%                         plot(xvect,yvect,'r','linewidth',3)
                          plot([is+1/hibb is+1/lobb],[-0.8*yma -0.8*yma],'k','linewidth',3) %Comment out only for low amplitude
...
%                     ymi=min(min([cumsumSTA12tr cumsumSTA13tr cumsumSTA23tr]));
                    ...
                    ccdat(nt,5)=STA13file(nt,1); ccdat(nt,6)=STA12file(nt,1); 
                    ccdat(nt,7)=meanalln;
                    ccdat(nt,8)=(ST1sq(end)+ST2sq(end)+ST3sq(end))/3;
                    alln(alln<0)=-1000*yma; %just so they don't plot.
                    ...
                    plot(STA1file(winlen*(nt-1)+cclen/2+1:winlen*nt-cclen/2,1),yma*alln,'mo','markersize',1)
                        text((is+ien)/2, -0.8*yma, num2str(meanalln,3),'fontsize',6);
                        text((is+ien)/2-3, -0.8*yma, num2str(mapfile(nt,4),3),'fontsize',6);
                        box on
                        axis([is ien -yma yma])
                        set(gca,'XTick',(is:5:ien),'fontsize',6);
                    i1=winlen*(nt-1)+1; i2=winlen*nt;
                    datfile(1:8,i1:i2)=[STA1file(i1:i2,1)';
                                        STA1file(i1:i2,2)';
                                        STA2file(i1:i2,2)';
                                        STA3file(i1:i2,2)';
                                        STA1bbfile(i1:i2,2)';
                                        ...
                                        STA3bbfile(i1:i2,2)';
                                        -99*ones(1,winlen)];
                    datfile(8,i1+cclen/2:i2-cclen/2)=(ST12n+ST13n+ST23n)/3;
                    guidefile(nt,:)=[nt STA1file(i1,1) yma];

                    subplot(npanel*nrow,mcol,npanel*(n-1)*mcol+2*mcol+m,'align');
                    hold on
                    plot(STA1lofile(winlen*(nt-1)+1:winlen*nt,1),STA1lofile(winlen*(nt-1)+1:winlen*nt,2),'m')
                    plot(STA2lofile(winlen*(nt-1)+1:winlen*nt,1),STA2lofile(winlen*(nt-1)+1:winlen*nt,2),'b')
                    plot(STA3lofile(winlen*(nt-1)+1:winlen*nt,1),STA3lofile(winlen*(nt-1)+1:winlen*nt,2),'k')
                    is = STA1lofile(winlen*(nt-1)+1,1);
                    ien= STA1lofile(winlen*nt,1);
                    plot([is+1/hilo is+1/lolo],[-0.8*yma -0.8*yma],'k','linewidth',3) %Comment out only for low amplitude
                    axis([is ien -yma yma])
                    box on
                    set(gca,'XTick',(is:5:ien),'fontsize',6);

                    subplot(npanel*nrow,mcol,npanel*(n-1)*mcol+3*mcol+m,'align');
                    hold on
                    plot(STA2midfile(winlen*(nt-1)+1:winlen*nt,1),STA2midfile(winlen*(nt-1)+1:winlen*nt,2),'b')
                    plot(STA3midfile(winlen*(nt-1)+1:winlen*nt,1),STA3midfile(winlen*(nt-1)+1:winlen*nt,2),'k')
                    is = STA1midfile(winlen*(nt-1)+1,1);
                    ien= STA1midfile(winlen*nt,1);
                    plot([is+1/himid is+1/lomid],[-0.8*yma -0.8*yma],'k','linewidth',3) %Comment out only for low amplitude
                    axis([is ien -yma yma])
                    box on
                    set(gca,'XTick',(is:5:ien),'fontsize',6);
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',['ARMMAP/WIGS/SSSI/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',['ARMMAP/WIGS/SSSI/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',['ARMMAP/WIGS/SSSI/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(ifig),'.eps'])
        end
    end
    if nd == 1
        fid = fopen(['ARMMAP/WIGS/SSSI/ccdat_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
        fprintf(fid,'%5i %3i %4i %11.4e %4i %3i %7.2f %7.2f\n',ccdat(1:nin,:)');
        fclose(fid);
    else
        fid = fopen(['ARMMAP/WIGS/SSSI/ccdat_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'a');
        fprintf(fid,'%5i %3i %4i %11.4e %4i %3i %7.2f %7.2f\n',ccdat(1:nin,:)');
        fclose(fid);
    end
    fid = fopen(['ARMMAP/WIGS/SSSI/wigdat',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
    fprintf(fid,'%10.3f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e \n',datfile(:,:));
    fclose(fid);
    bostsecfile(1,:)=bostsec;
    bostsecfile(2,:)=1;    
    fid = fopen(['ARMMAP/WIGS/SSSI/bostsec',IDENTIF(1:8)],'w');
    fprintf(fid,'%10.3f %12.4e \n',bostsecfile(:,:));
    fclose(fid);
    fid = fopen(['ARMMAP/WIGS/SSSI/guide',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
%     fid = fopen(['ARMMAP/MAPS/NEW/pks',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/sps),'s'],'w');
