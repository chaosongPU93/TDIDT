...
%    timoffrot=[261 05 19 48 +085 +020  80 115  55]; % Sept. 18.  Nothing at this spot.
   ...
   timoffrot=[2005 254 09 38 40 +085 +020  80 115  55]; %RESOLVED FRONTS spot.  3s window
   timoffrot=[2005 255 09 38 40 +085 +020  80 115  55]; %Fake
   ...
   %timoffrot=[2003 062 14 15 32 +085 +020  80 115  55]; %faked
...
iup=1;  %WAS 4!!!
nin=0;
fidPGC = fopen(['PGC',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-',int2str(winlen/40),'s'],'w'); 
fidSILB = fopen(['SILB',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-',int2str(winlen/40),'s'],'w'); 
fidSSIB = fopen(['SSIB',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-',int2str(winlen/40),'s'],'w'); 
fidSILBs = fopen(['SILB',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-',int2str(winlen/40),'s.shift'],'w'); 
fidSSIBs = fopen(['SSIB',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-',int2str(winlen/40),'s.shift'],'w'); 
lastend=0;
for n=1:nwin;
    istart=igstart+(n-1)*winoff; %a better way might exist?
    iend=istart+winlen+(2*40)-1; %extend window by 2 seconds
...
%             lastend=iend;
%
            imaxPGSSwr(1:iend+1-istart,1)=round(xmaxPGSSntmp(n));
            imaxPGSIwr(1:iend+1-istart,1)=round(xmaxPGSIntmp(n));

            datfile1=[timsPGC(istart:iend)' realPGC(istart:iend)]';
            fprintf(fidPGC,'%10.3f %9.5e\n',datfile1);
            datfile2=[timsPGC(istart:iend)' realSSIB(istart-imaxPGSSwr(1):iend-imaxPGSSwr(1)) imaxPGSSwr(1:iend+1-istart)]';
            fprintf(fidSSIBs,'%10.3f %9.5e %3i\n',datfile2);
            datfile3=[timsPGC(istart:iend)' realSILB(istart-imaxPGSIwr(1):iend-imaxPGSIwr(1)) imaxPGSIwr(1:iend+1-istart)]';
            fprintf(fidSILBs,'%10.3f %9.5e %3i\n',datfile3);
            datfile4=[timsPGC(istart:iend)' realSSIB(istart:iend) imaxPGSSwr(1:iend+1-istart)]';
            fprintf(fidSSIB,'%10.3f %9.5e %3i\n',datfile4);
            datfile5=[timsPGC(istart:iend)' realSILB(istart:iend) imaxPGSIwr(1:iend+1-istart)]';
            fprintf(fidSILB,'%10.3f %9.5e %3i\n',datfile5);
...
fclose(fidPGC); fclose(fidSILB); fclose(fidSSIB);
fid = fopen(['wd2B',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-',int2str(winlen/40),'s'],'w'); 
fprintf(fid,'%i %9.5f %9.5f %9.4f %9.2f %9.3f\n',mapfile');
fclose(fid);
figure 
subplot(4,1,1,'align'); 
hold on
plot(timswin,xcmaxAVEnmin*8+zeros(nwin,1),'k:');
...
plot(timswin,xmaxPGSIntmp,'ro','MarkerSize',2);
plot(timswin,xmaxSISSntmp,'k*','MarkerSize',2);
axis([3*timbig/2 2*timbig -10 10]);
box on
subplot(4,1,4,'align'); 
hold on
hrf = plotreflinesr(gca,detects,'x','k'); 
