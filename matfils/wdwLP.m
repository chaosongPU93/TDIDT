...
%   displaytim=[79361];
    ...
    rubiname=['ARMMAP/MAPS/2018July/map2003.062.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2003.063.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/NEW/12345map2003.064.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2004.196.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2004.197.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2004.198.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2004.199.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2005.254.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2005.255.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/2018July/map2005.256.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s';
              'ARMMAP/MAPS/NEW/12345map2005.257.002.loff1.5.ccmin0.44.nponpa22_1.25-6.5-ms19-4s'];
...
loopoffmax=3.0; %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.
xcmaxAVEnmin=0.44; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for nd=1:length(timoffrot(:,1))
    ...
    rubins=load(rubiname(nd,:));
    rubinsec=rubins(:,1);
    rub13=round(rubins(:,2))+10; %column 2 is station 1-3 (sign?) offset; +10 circumvents the 86/30 instead of 86/20
    rub12=round(rubins(:,3)); %column 3 is station 1-2 (sign?) offset
    rubinsamp=rubinsec*sps;
    ...
    %Now for higher pass (hp)
    lohp=1.25;
    hihp=6.5;
    STAopthp=zeros(nsta,tracelen);
    for ista=1:nsta
        [LIA,idx]=ismember(stas(ista,:),PERMSTA,'rows');
        if LIA
            if strcmp(PERMSTA(idx,1:3),'PGC')
            ...
            [opt,~,~,~]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lohp,hihp,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        [LIA,idx]=ismember(stas(ista,:),POLSTA,'rows');
        if LIA
            if year==2003 && jday<213
            ...
            [opt,~,~]=readpols(prename,POLSTA,POLROTS,idx,sps,lohp,hihp,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        STAopthp(ista,:)=opt/scaleseisms(ista); 
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     sumsSILB2b=zeros(nwin,2*mshift+1);
    ...
    concentration=1.0; %in seconds; how concentrated is the coherent energy within the window?
...
%                   clear dummy
                    ...
                    STA1hpfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAopthp(1,istart:iend)'];
                    STA2hpfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAopthp(2,istart-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3hpfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAopthp(3,istart-imaxSTA13wr:iend-imaxSTA13wr)'];
                    ...
                    mapfile(nin,:)=[timswin(n) xmaxSTA13ntmp(n) xmaxSTA12ntmp(n) ...
                        ...
                        0 0 ...
                        Prev(nin) Post(nin) Prev15(nin) Prev30(nin) ...
    ...
    fprintf(fid,'%9.1f %6.2f %6.2f %8.3f %7.2f %10.3e %10.3e %10.3f %7.3f %10.3e %10.3e %10.3e %10.3e %7.3f %7.3f %7.3f %5.2f %5.2f %7.3f %7.3f %7.3f\n',mapfile(1:nin,:)');
    fclose(fid);
    figure 
    subplot(4,1,1,'align'); 
    hold on
...
% The following for running cc
                    cclen=80; %running cc window length, in samples
...
%                         plot(xvect,yvect,'r','linewidth',3)
                    ...
                    subplot(3*nrow,mcol,3*(n-1)*mcol+2*mcol+m,'align');
                        plot(STA1hpfile(winlen*(nt-1)+1:winlen*nt,1),STA1hpfile(winlen*(nt-1)+1:winlen*nt,2),'r')
                        hold on
                        plot(STA2hpfile(winlen*(nt-1)+1:winlen*nt,1),STA2hpfile(winlen*(nt-1)+1:winlen*nt,2),'b')
                        plot(STA3hpfile(winlen*(nt-1)+1:winlen*nt,1),STA3hpfile(winlen*(nt-1)+1:winlen*nt,2),'k')
                        is = STA1hpfile(winlen*(nt-1)+1,1);
                        ien= STA1hpfile(winlen*nt,1);
                    % find the peaks etc.
...
%                     end
                        plot([is+1/hihp is+1/lohp],[-0.8*yma -0.8*yma],'k','linewidth',3)
                        text(is+0.2, 0.66*yma, int2str(STA13file(nt,1)),'fontsize',6);
                        text(ien-0.6, 0.66*yma, int2str(STA12file(nt,1)),'fontsize',6);
                        box on
                        axis([is ien -yma yma])
                        set(gca,'XTick',[is (is+ien)/2],'fontsize',6);
                        for ir = 1:length(rubinsec)
                            if rubinsec(ir)>is && rubinsec(ir)<ien
                                plot(rubinsec(ir),-0.93*yma,'go','MarkerSize',4,'MarkerFaceColor','g')
                                text(rubinsec(ir)-0.1, -0.86*yma, int2str(rub13(ir)),'fontsize',6,'color','r','HorizontalAlignment','right'); 
                                text(rubinsec(ir)+0.1, -0.86*yma, int2str(rub12(ir)),'fontsize',6,'color','k','HorizontalAlignment','left');
                            end
                        end
%                     axis([is ien ymi yma])
...
%                     text(ien-1.4, ymi+0.1*(yma-ymi), num2str(STAamp(nt,3),2),'fontsize',6);
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',['ARMMAP/WIGS/2018/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',['ARMMAP/WIGS/2018/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',['ARMMAP/WIGS/2018/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(ifig),'.eps'])
        end
...
%     bostsecfile(2,:)=1;    
%     fid = fopen(['ARMMAP/WIGS/2018July/bostsec',IDENTIF(1:8)],'w');
%     fprintf(fid,'%10.3f %12.4e \n',bostsecfile(:,:));
%     fclose(fid);
%     fid = fopen(['ARMMAP/WIGS/2018July/wigdat',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
%     fprintf(fid,'%10.3f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e \n',datfile(:,:));
%     fclose(fid);
%     fid = fopen(['ARMMAP/WIGS/2018July/guide',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
