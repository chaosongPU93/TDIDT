...
rotSSIBp=rotSSIB+(pi/2);
SSIBrotp=SSIB*exp(1i*rotSSIBp);
    %rtate=cputime-t
    %t=cputime;
...
if SSIBtoff > 0
    ...
    SSIBrotp(1:tracelenPGC-SSIBsoff)=SSIBrotp(SSIBsoff+1:tracelenPGC);
    SSIBrotp(tracelenPGC-SSIBsoff+1:tracelenPGC)=0;
else
    SSIBrot(-SSIBsoff+1:tracelenPGC)=SSIBrot(1:tracelenPGC+SSIBsoff);
    SSIBrot(1:-SSIBsoff)=0;
    SSIBrotp(-SSIBsoff+1:tracelenPGC)=SSIBrotp(1:tracelenPGC+SSIBsoff);
    SSIBrotp(1:-SSIBsoff)=0;
end
if SILBtoff > 0
...
SSIBpauto=realSSIBp.*realSSIBp;
SSIBp2=cumsum(SSIBpauto);
...
for n=-mshift:mshift;
    ...
    SISSx(:,n+mshift+1)=realSILB(1+mshift:tracelenPGC-mshift).* ...
        realSSIB(1+mshift-n:tracelenPGC-mshift-n);
    SSSSx(:,n+mshift+1)=realSSIB(1+mshift:tracelenPGC-mshift).* ...
        realSSIBp(1+mshift-n:tracelenPGC-mshift-n);
end
cumsumPGSS=cumsum(PGSSx);
cumsumPGSI=cumsum(PGSIx);
cumsumSISS=cumsum(SISSx);
cumsumSSSS=cumsum(SSSSx);
xcshifts=cputime-t
t=cputime;
...
sumsSSSS=zeros(nwin,2*mshift+1);
sumsPGC2=zeros(nwin,2*mshift+1);
sumsSSIB2=zeros(nwin,2*mshift+1);
sumsSSIBp2=zeros(nwin,2*mshift+1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
% GOT TO HERE IN SSIBp territory
%%%
for n=1:nwin;
        ...
        if abs(iPGSSbang-imaxinterpPGSS) <= loopoffmax*iup && ...
           ...
           interpPGSSn(iPGSSbang)+interpPGSIn(iPGSIbang)+interpSISSn(iSISSbang) >= 3*xcmaxAVEnmin
            ...
            if abovecut > 0.9*belowcut %-1
                ...
                SSIBpfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsPGC(istart:iend)' realSSIBp(istart-imaxPGSSwr:iend-imaxPGSSwr)];
...
fid = fopen(['ARMMAP/SSIB/map',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['ARMMAP/SSIB/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'b.eps'])
...
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['ARMMAP/SSIB/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'c.eps'])
...
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
orient landscape
print('-depsc',['ARMMAP/SSIB/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'d.eps'])
...
box on
title([IDENTIF,'_{',int2str(lo),'-',int2str(hi),'}'])
print('-depsc',['ARMMAP/SSIB/FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',int2str(lo),'-',int2str(hi),'e.eps'])
...
%                   plot(PGCfile(winlen*(nt-1)+1:winlen*nt,1),PGCfile(winlen*(nt-1)+1:winlen*nt,2),'r')
                    ...
                    plot(SSIBpfile(winlen*(nt-1)+1:winlen*nt,1),2*SSIBpfile(winlen*(nt-1)+1:winlen*nt,2),'r')
                    is = PGCfile(winlen*(nt-1)+1,1);
                    ien= PGCfile(winlen*nt,1);
                    %yma=0.4;
                    yma=max(max([SSIBfile(winlen*(nt-1)+1:winlen*nt,2) ...
                        SILBfile(winlen*(nt-1)+1:winlen*nt,2)]));
                    ymi=min(min([SSIBfile(winlen*(nt-1)+1:winlen*nt,2) ...
                        SILBfile(winlen*(nt-1)+1:winlen*nt,2)]));
...
%                     plot(pks(:,1),pks(:,2),'r+')
    ...
    orient landscape
    if ifig <= 9
        print('-depsc',['ARMMAP/SSIB/WIGS/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'.',int2str(0),int2str(0),int2str(ifig),'.eps'])
    elseif ifig <= 99
        print('-depsc',['ARMMAP/SSIB/WIGS/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'.',int2str(0),int2str(ifig),'.eps'])
    else
        print('-depsc',['ARMMAP/SSIB/WIGS/',IDENTIF,'-',int2str(lo),'-',int2str(hi),'.',int2str(ifig),'.eps'])
    end

end
%fid = fopen(['ARMMAP/MAPS/pks',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
fid = fopen(['ARMMAP/SSIB/pks',IDENTIF,'_',int2str(lo),'-',int2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
