...
mshift=12; %16 %11 %14 %12_for_+085+020 %15_for_+059-073
hi=6.;
lo=1.5;
%%%lo=0.5;
% hi=8.;
...
% lo=0.75;
loopoffmax=1.5
xcmaxAVEnmin=0.4 %0.33 %0.4 for 4s 1.5-6 Hz; 0.36 for 4s2-8 Hz ;  0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
%winlen=4*40;
winlen=128*40;
winlen=4*40;
%winoff=40/1;
...
nrow=4;
mcol=6;
secplot=[59117 59215];  %2005.255 4sec
fid1 = fopen(['ARMMAP/4trace',IDENTIF(1:8),'.',int2str(secplot(1,1)),'_', ...
    int2str(lo),'-',int2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
for ifig=1:floor(nin/(nrow*mcol))+1
    ...
    for n = 1:nrow
        for m = 1:mcol
            ...
            if nt <= nin
                ...
                %secplot=64067;  %EQ 24 sec
                %secplot=32611;  %2003.062
                %secplot=79353;  %2004.197
                %secplot=8127;  %2005.255
                if abs(is-secplot(1,1))<0.45 || abs(is-secplot(1,2))<0.45
                %if abs(is-64063)<0.45  %EQ 32 sec
                %if abs(is-64067)<0.45  %EQ 24 sec
                %if abs(is-32611)<0.45  %2003.062
                %if abs(is-79353)<0.45  %2004.197
    %             if PGSSfile(nt,1) <= 0 && PGSSfile(nt,1) >= -2 && abs(PGSIfile(nt,1))<=2
                    ...
                    tmptrace=[PGCfile(winlen*(nt-1)+1:winlen*nt,1) PGCfile(winlen*(nt-1)+1:winlen*nt,2) ...
                        SSIBfile(winlen*(nt-1)+1:winlen*nt,2) SILBfile(winlen*(nt-1)+1:winlen*nt,2)];
fprintf(fid1,'%16.8f %16.8f %16.8f %16.8f\n',tmptrace');
                    ...
                    ymasv=yma;
                    yma=1.5*max(yma,-ymi);  %was 2.1, before that 2.4
...
%                     plot(PGCfile(winlen*(nt-1)+11:winlen*nt-10,1),SISSn,'k')
                    ...
                    tmpxcorr=[PGCfile(winlen*(nt-1)+11:winlen*nt-10,1) alln];
fid = fopen(['ARMMAP/32xcorr',IDENTIF(1:8),'.',int2str(secplot),'_', ...
   int2str(lo),'-',int2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
fclose(fid);
    ...
    orient landscape
    if ifig <= 9
        print('-depsc',['tmp',int2str(0),int2str(0),int2str(ifig),'.eps'])
    elseif ifig <= 99
        print('-depsc',['tmp',int2str(0),int2str(ifig),'.eps'])
    else
        print('-depsc',['tmp',int2str(ifig),'.eps'])
    end
