...
% wdw3fre3q reads a very bb file, and gets 4 frequency ranges (detection plus 3) from that.
format short e
clear all
close all
...
%hi=6;
%lo=1.5;
hi=6.5;  %In wdw3freq, this is the frequency range for the detections
lo=1.25; %In wdw3freq, this is the frequency range for the detections 
% hi=10;
% lo=4;
npo=2;
npa=2;
mshift=2; %19; %maximum shift for the x-correlations. 19 for 002 Stanford
loopoffmax=1.5; %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.
xcmaxAVEnmin=0.46; %0.44; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for nd=1:length(timoffrot(:,1))
    ...
    for ista=1:nsta
        ...
        if LIA
            ...
            [opt,~,nzeros,timsSTA]=readperms(prename,PERMSTA,PERMROTS,idx,sps,lobb,hibb,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        ...
        if LIA
            ...
            [opt,~,nzeros]=readpols(prename,POLSTA,POLROTS,idx,sps,lobb,hibb,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        found
        STAoptbb(ista,:)=opt/scaleseisms(ista); 
        STAnzeros(ista,:)=nzeros;
    end
    %Now use STAoptbb to get all the windows you want.
    %First the detection bandpass:
    STAopt=zeros(nsta,tracelen);
    STAopt(1,:)=bandpass(STAoptbb(1,:),sps,lo,hi,npo,npa,'butter');
    STAopt(2,:)=bandpass(STAoptbb(2,:),sps,lo,hi,npo,npa,'butter');
    STAopt(3,:)=bandpass(STAoptbb(3,:),sps,lo,hi,npo,npa,'butter');
    %Now for the high pass:
    lohi=4;
    hihi=10;
    STAopthi(1,:)=bandpass(STAoptbb(1,:),sps,lohi,hihi,npo,npa,'butter');
    STAopthi(2,:)=bandpass(STAoptbb(2,:),sps,lohi,hihi,npo,npa,'butter');
    STAopthi(3,:)=bandpass(STAoptbb(3,:),sps,lohi,hihi,npo,npa,'butter');
    %Next for the low pass:
    lolo=0.5;
    hilo=1.25;
    STAoptlo(1,:)=bandpass(STAoptbb(1,:),sps,lolo,hilo,npo,npa,'butter');
    STAoptlo(2,:)=bandpass(STAoptbb(2,:),sps,lolo,hilo,npo,npa,'butter');
    STAoptlo(3,:)=bandpass(STAoptbb(3,:),sps,lolo,hilo,npo,npa,'butter');
    %Now over-write the bandpass:
    lobb=0.5;
    hibb=8;
    STAoptbb(1,:)=bandpass(STAoptbb(1,:),sps,lobb,hibb,npo,npa,'butter');
    STAoptbb(2,:)=bandpass(STAoptbb(2,:),sps,lobb,hibb,npo,npa,'butter');
    STAoptbb(3,:)=bandpass(STAoptbb(3,:),sps,lobb,hibb,npo,npa,'butter');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     sumsSILB2b=zeros(nwin,2*mshift+1);
    ...
    for n=1:nwin
        if xcmaxAVEn(n)<xcmaxAVEnmin || abs(xmaxSTA13n(n)-xmaxSTA12n(n)+xmaxSTA32n(n))>loopoffmax ...
            ...
            interpSTA12n=interp(sumsSTA12n(n,:),iup,2);  %"L" reduced from 3 to 2 because with mshift=2 (meaning +/-2 not accepted) sumsSTA12n has dimensions (n,5). 2L must be <5.
            interpSTA13n=interp(sumsSTA13n(n,:),iup,2);
            interpSTA32n=interp(sumsSTA32n(n,:),iup,2);
...
%                   clear dummy
                    ...
                    STA1hifile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAopthi(1,istart:iend)'];
                    STA2hifile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAopthi(2,istart-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3hifile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAopthi(3,istart-imaxSTA13wr:iend-imaxSTA13wr)'];
...
%                             plot(timstemp((nt-1)*templen+1:nt*templen),STAtemps(whichtoplot,:)*yma/2,'c','linewidth',2)
%                         else %if cc with negative template is larger ...
%                             plot(timstempneg((nt-1)*templen+1:nt*templen),-STAtemps(whichtoplot,:)*yma/2,'m','linewidth',2)
%                         end
% Lines above plot + or - template:
                        hold on
                        tmp1=STA1file(winlen*(nt-1)+1:winlen*nt,:);
                        tmp2=STA2file(winlen*(nt-1)+1:winlen*nt,:);
                        tmp3=STA3file(winlen*(nt-1)+1:winlen*nt,:);
                        is = tmp1(1,1);
                        ien= tmp1(end,1);
                        axis([is ien -yma yma])
...
% The following for running cc
                        STA12tr=tmp1(:,2).*tmp2(:,2);            
                        STA13tr=tmp1(:,2).*tmp3(:,2);
                        STA23tr=tmp2(:,2).*tmp3(:,2);
                        ...
                        ST1auto=tmp1(:,2).*tmp1(:,2);
                        ST1sq=cumsum(ST1auto);
                        ST2auto=tmp2(:,2).*tmp2(:,2);
                        ST2sq=cumsum(ST2auto);
                        ST3auto=tmp3(:,2).*tmp3(:,2);
                        ...
                        plot(tmp1(cclen/2+1:end-cclen/2,1),yma*alln,'co','markersize',1)
                        hold on
% The above for running cc
                        %hrf = plotreflinesr(gca,timstemp((nt-1)*templen+1+tempbef),'x','k');
                        plot([timstemp((nt-1)*templen+1+tempbef) timstemp((nt-1)*templen+1+tempbef)],[-yma yma],'k--','linewidth',0.25)
                        plot(tmp1(:,1),tmp1(:,2),'r')
                        plot(tmp2(:,1),tmp2(:,2),'b')
                        plot(tmp3(:,1),tmp3(:,2),'k')
                        box on
                        set(gca,'XTick',[is (is+ien)/2],'fontsize',6);

                        subplot(npanel*nrow,mcol,npanel*(n-1)*mcol+mcol+m,'align');
                        tmp1hi=STA1hifile(winlen*(nt-1)+1:winlen*nt,2);
                        tmp2hi=STA2hifile(winlen*(nt-1)+1:winlen*nt,2);
                        tmp3hi=STA3hifile(winlen*(nt-1)+1:winlen*nt,2);
                        hold on
                        is = tmp1(1,1);
                        ien= tmp1(end,1);
                        plot([is+1/hihi is+1/lohi],[-0.8*yma -0.8*yma],'k','linewidth',3)
                        text(is+0.2, 0.66*yma, num2str(mapfile(nt,2)),'fontsize',6);
                        text(ien-0.6, 0.66*yma, num2str(mapfile(nt,3)),'fontsize',6);
% The following for running cc
                        STA12tr=tmp1hi.*tmp2hi;            
                        STA13tr=tmp1hi.*tmp3hi;            
                        STA23tr=tmp2hi.*tmp3hi;            
                        cumsumSTA12tr=cumsum(STA12tr);
                        cumsumSTA13tr=cumsum(STA13tr);
                        cumsumSTA23tr=cumsum(STA23tr);
                        cclen=10; %running cc window length, in samples
                        ST1auto=tmp1hi.*tmp1hi;
                        ST1sq=cumsum(ST1auto);
                        ST2auto=tmp2hi.*tmp2hi;
                        ST2sq=cumsum(ST2auto);
                        ST3auto=tmp3hi.*tmp3hi;
                        ST3sq=cumsum(ST3auto);
                        ST12num=cumsumSTA12tr(cclen+1:winlen)-cumsumSTA12tr(1:winlen-cclen);
                        ST13num=cumsumSTA13tr(cclen+1:winlen)-cumsumSTA13tr(1:winlen-cclen);
                        ST23num=cumsumSTA23tr(cclen+1:winlen)-cumsumSTA23tr(1:winlen-cclen);
...
% The above for running cc                        
                        box on
                        plot([timstemp((nt-1)*templen+1+tempbef) timstemp((nt-1)*templen+1+tempbef)],[-yma yma],'k--','linewidth',0.25)
                        plot(tmp1(:,1),tmp1hi,'r')
                        plot(tmp1(:,1),tmp2hi,'b')
                        plot(tmp1(:,1),tmp3hi,'k')
                        axis([is ien -yma yma])
                        set(gca,'XTick',[is (is+ien)/2],'fontsize',6);
                        ...
                        tmp1lo=STA1lofile(winlen*(nt-1)+1:winlen*nt,2);
                        tmp2lo=STA2lofile(winlen*(nt-1)+1:winlen*nt,2);
                        tmp3lo=STA3lofile(winlen*(nt-1)+1:winlen*nt,2);
                        hold on
                        plot(tmp1(:,1),tmp1lo,'r')
                        plot(tmp1(:,1),tmp2lo,'b')
                        plot(tmp1(:,1),tmp3lo,'k')
                        is = tmp1(1,1);
                        ien= tmp1(end,1);
                        plot([is+1/hilo is+1/lolo],[-0.8*yma -0.8*yma],'k','linewidth',3)
                        text(is+0.2, 0.66*yma, num2str(mapfile(nt,2)),'fontsize',6);
                        text(ien-0.6, 0.66*yma, num2str(mapfile(nt,3)),'fontsize',6);
                        box on
                        axis([is ien -yma yma])
