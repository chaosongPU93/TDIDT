...
npo=2;
npa=2;
mshift=19; %19; %maximum shift for the x-correlations. 19 for 002 Stanford
loopoffmax=1.5; %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.
xcmaxAVEnmin=0.34; %0.44; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for nd=1:length(timoffrot(:,1))
    ...
    for ista=1:nsta
        ...
        found=found
        %factr1(ista)=prctile(abs(opt),90); %Not normalized    yp = inteFD(y, dt)
        %factr2(ista)=factr1(ista)/factr1(1); %This is what is used; keeps 1st station unchanged but scales the others
        STAopt(ista,:)=opt/scaleseisms(ista); %Integrate to displacement
        %STAort(ista,:)=ort;
        STAnzeros(ista,:)=nzeros;
    end
    %Now for displacements (U)
    loU=1;
    hiU=8;
    loU=lo;
    hiU=hi;
...
%                   clear dummy
                    ...
                    STA1Ufile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptU(1,istart:iend)'];
                    STA2Ufile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptU(2,istart-imaxSTA12wr:iend-imaxSTA12wr)'];
                    STA3Ufile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsSTA(istart:iend)' STAoptU(3,istart-imaxSTA13wr:iend-imaxSTA13wr)'];
...
% The above for running cc
                        ...
                        plot(STA1Ufile(winlen*(nt-1)+1:winlen*nt,1),STA1Ufile(winlen*(nt-1)+1:winlen*nt,2),'r')
                        hold on
                        plot(STA2Ufile(winlen*(nt-1)+1:winlen*nt,1),STA2Ufile(winlen*(nt-1)+1:winlen*nt,2),'b')
                        plot(STA3Ufile(winlen*(nt-1)+1:winlen*nt,1),STA3Ufile(winlen*(nt-1)+1:winlen*nt,2),'k')
                        is = STA1Ufile(winlen*(nt-1)+1,1);
                        ien= STA1Ufile(winlen*nt,1);
%                         yma=max(max([STA1Ufile(winlen*(nt-1)+1:winlen*nt,2) STA2Ufile(winlen*(nt-1)+1:winlen*nt,2) ...
%                             STA3Ufile(winlen*(nt-1)+1:winlen*nt,2)]));
%                         ymi=min(min([STA1Ufile(winlen*(nt-1)+1:winlen*nt,2) STA2Ufile(winlen*(nt-1)+1:winlen*nt,2) ...
%                             STA3Ufile(winlen*(nt-1)+1:winlen*nt,2)]));
...
%                         plot(xvect,yvect,'r','linewidth',3)
                        ...
                        plot(STA1file(winlen*(nt-1)+1:winlen*nt,1),cumsum(STA1file(winlen*(nt-1)+1:winlen*nt,2)/sps),'r')
                        hold on
                        plot(STA2file(winlen*(nt-1)+1:winlen*nt,1),cumsum(STA2file(winlen*(nt-1)+1:winlen*nt,2)/sps),'b')
                        plot(STA3file(winlen*(nt-1)+1:winlen*nt,1),cumsum(STA3file(winlen*(nt-1)+1:winlen*nt,2)/sps),'k')
                        is = STA1file(winlen*(nt-1)+1,1);
                        ien= STA1file(winlen*nt,1);
                        box on
                        axis([is ien -yma yma])
