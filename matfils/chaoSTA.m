...
%%%%  IMPORTANT NOTES @2019/05/23  %%%%
% the saved offsets in 3-sta trio are xmaxSTA12ntmp/xmaxSTA13ntmp
%
% IF it is positive, means sta 2/3 needs to shift to right to align with
% sta 1, the arrival time then is smaller than 1, arr2= arr1-xmaxSTA12ntmp;
%
% IF it is negative, means sta 2/3 needs to shift to left to align with
% sta 1, the arrival time then is larger than 1, arr2= arr1-xmaxSTA12ntmp; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    hibb=10;
    ...
    % STA12x, 13x, 32x is the dot product of shifted seismogram pair, whole trace 
    ...
    % cumulative sum of the dot product, whole trace
...
%     sumsSILB2b=zeros(nwin,2*mshift+1);
    ...
    for n=1:nwin
        ...
        %%% according to the syntax, sumsSTA12 is the sum of dot product
        %%% sum of the shifted window pair
    ...
    %
    denomSTA12n=realsqrt(sumsSTA1sq.*sumsSTA2sq);    % denominator,Real square root, An error is produced if X is negative
...
%         n=1;
    ...
    % 
                ...
                %%% pwelch is a built-in function, Power Spectral Density estimate via Welch's method,
                %%%% help to see details, [Pxx F] = pwelch(X, WINDOW, NOVERLAP, NFFT, Fs)
...
%     title([IDENTIF,'.',num2str(lo),'-',num2str(hi)])
    box on
