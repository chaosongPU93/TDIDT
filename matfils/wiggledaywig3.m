...
%   timoffrot(1,:)=[2005 255 07 42 12 +086 +020  80 115  50];
   ...
   %timoffrot(9,:)=[2005 262 07 42 12 +086 +020  80 115  50];
   %timoffrot(10,:)=[2005 263 07 42 12 +086 +020  80 115  50];
   %timoffrot(11,:)=[2005 264 07 42 12 +086 +020  80 115  50];
   %timoffrot(12,:)=[2005 265 07 42 12 +086 +020  80 115  50];
   %timoffrot(13,:)=[2005 266 07 42 12 +086 +020  80 115  50];
   %timoffrot(14,:)=[2005 267 07 42 12 +086 +020  80 115  50];
%   
...
% orient landscape
% print('-depsc',['FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',num2str(lo),'-',num2str(hi),'a.eps'])
        ...
        if abs(iPGSSbang-imaxinterpPGSS) <= loopoffmax*iup && ...
           ...
           interpPGSSn(iPGSSbang)+interpPGSIn(iPGSIbang)+interpSISSn(iSISSbang) >= 3*xcmaxAVEnmin
            ...
            if abovecut > 0.9*belowcut %-1
                ...
                % PGCfile, SSIBfile, SILBfile are back-to-back windows for the day (plus sample times)
...
% The above for spectra
                    % This PGSStr etc have different dimensions than previous PGSStr etc.
