...
% 002
    ...
    timoffrot(5,:)=[2005 254 07 42 12 +086 +020  80 115  50];
    timoffrot(6,:)=[2005 255 07 42 12 +086 +020  80 115  50];
    timoffrot(7,:)=[2005 256 07 42 12 +086 +020  80 115  50];
    timoffrot(8,:)=[2005 257 07 42 12 +086 +020  80 115  50];
...
%      %bostname(1,:)='BOSTOCK/NEW/002_2003.063';
%      %bostname(3,:)='BOSTOCK/NEW/002_2003.064';
     bostname(1,:)='BOSTOCK/NEW/002_2004.196';
     bostname(2,:)='BOSTOCK/NEW/002_2004.197';
     bostname(3,:)='BOSTOCK/NEW/002_2004.198';
     bostname(4,:)='BOSTOCK/NEW/002_2004.199';
     bostname(5,:)='BOSTOCK/NEW/002_2005.254';
     bostname(6,:)='BOSTOCK/NEW/002_2005.255';
     bostname(7,:)='BOSTOCK/NEW/002_2005.256';
     bostname(8,:)='BOSTOCK/NEW/002_2005.257';
...
for nd=1:length(timoffrot(:,1))
    ...
    for n=1:length(bostsec)
        ...
        for ista=1:nsta
            ...
            xmin=1800; xmax=3000;
            xmin=1; xmax=6000;
%             if ista==4 %Searching for bad TWKB event...
%                 xmin=1; xmax=6000;
%                 ymin=min(stack(ista,xmin:xmax)); ymax=max(stack(ista,xmin:xmax));
%                 abberent(n)=ymax-ymin;
%                 if abberent(n) > maxabb
%                     maxabb=abberent(n)
%                 end
%             end
            xlim([xmin xmax]) 
...
SILBort=stackort(2,:);
for ista=1:nsta
    fid = fopen([POLSTA(ista,:),'_',num2str(lo),'-',num2str(hi),'_npass',int2str(npa)],'w');
    fprintf(fid,'%12.5e\n',stack(ista,:));
    fclose(fid);
end
xc=xcorr(SSIB,SILB);
...
SSIB(1000:5000)=SSIB(1000+(imax-(winlen+1)):5000+(imax-(winlen+1)));
SSIBort(1000:5000)=SSIBort(1000+(imax-(winlen+1)):5000+(imax-(winlen+1)));
hold on
plot(SSIB,'b')
plot(SILB,'r')
xlim([2000 3000])
box on
is=2200; ilen=500;
xc=xcorr(SSIB(is:is+ilen-1),SILB(is:is+ilen-1));
[~,imax]=max(xc);
SSIB(1000:5000)=SSIB(1000+(imax-(ilen+1)):5000+(imax-(ilen+1)));
SSIBort(1000:5000)=SSIBort(1000+(imax-(ilen+1)):5000+(imax-(ilen+1)));
SSSIx=SSIB.*SILB;
cumSSSI=cumsum(SSSIx);
subplot(3,2,ista+2,'align')
plot(cumSSSI)
xlim([2000 3000])
text(2500,0.4*cumSSSI(end),['cumsum(2200) = ',num2str(cumSSSI(2200))],'fontsize',7)
text(2500,0.3*cumSSSI(end),['cumsum(2400) = ',num2str(cumSSSI(2400))],'fontsize',7)
text(2500,0.2*cumSSSI(end),['cumsum(3000) = ',num2str(cumSSSI(3000))],'fontsize',7)
text(2500,0.1*cumSSSI(end),['cumsum(5000) = ',num2str(cumSSSI(5000))],'fontsize',7)
title('Yajuns parameters for SSIB')
box on
subplot(3,2,ista+3,'align')
hold on
plot(SSIBort,'b')
plot(SILBort,'r')
title('SSIB (blue) SILB (red) orthogonal')
xlim([2000 3000])
box on
SSSIortx=SSIBort.*SILBort;
cumSSSIort=cumsum(SSSIortx);
subplot(3,2,ista+4,'align')
plot(cumSSSIort)
xlim([2000 3000])
box on
set(h,'PaperPosition',[0.25 0.25 8 10.5])
print(h,'-depsc','BFIGS/temp.eps')
...
%     num2str(lo),'-',int2str(hi),'Hz_',int2str(npa),'pass.eps'])
% 
