...
SNBZ(tracelenPGC-39:tracelenPGC)=sin(x).*SNBZ(tracelenPGC-39:tracelenPGC);
...
[SNBZf]=1.6e-4*bandpass(SNBZ,40,lo,hi,npo,npa,'butter');
...
SNBZsoff=-80;
    SNBZ(-SNBZsoff+1:tracelenPGC)=SNBZf(1:tracelenPGC+SNBZsoff);
    SNBZ(1:-SNBZsoff)=0;
        ...
        if abs(iPGSSbang-imaxinterpPGSS) <= loopoffmax*iup && ...
           ...
           interpPGSSn(iPGSSbang)+interpPGSIn(iPGSIbang)+interpSISSn(iSISSbang) >= 3*xcmaxAVEnmin
            ...
            if abovecut > 0.9*belowcut %-1
                ...
                SNBZfile(nin*winlen+1:(nin+1)*winlen,1:2)=[timsPGC(istart:iend)' SNBZ(istart:iend)];
...
for ifig=1:floor(nin/(nrow*mcol))+1
    ...
    %             if PGSSfile(nt,1) <= 0 && PGSSfile(nt,1) >= -2 && abs(PGSIfile(nt,1))<=2
                    ...
                    plot(SNBZfile(winlen*(nt-1)+1:winlen*nt,1),10*SNBZfile(winlen*(nt-1)+1:winlen*nt,2),'c')
