...
for ifig=1:floor(ntot/(nrow*mcol))+1
    ...
    for n = 1:nrow
        for m = 1:mcol
            if nt < ntot 
                nt=nt+1;
                wins=windows(traceplot(nt,321)); %windows is a list of windows when 002 is active, with the day formatted as year.day
                nwins=size(wins,1);
                inwin=0;
                for i=1:nwins
                ...
                text(100,-0.96*maxmax,[num2str(traceplot(nt,325)),' SI'],'fontsize',7)
                text(100,-0.7*maxmax,[num2str(traceplot(nt,326)),' SS'],'fontsize',7)
                text(5,-0.96*maxmax,[num2str(traceplot(nt,327)),'  ',num2str(1000*traceplot(nt,329))],'fontsize',7)
                text(5,-0.7*maxmax,[num2str(traceplot(nt,328)),'  ',num2str(1000*traceplot(nt,330))],'fontsize',7)
...
%                 box on
    ...
    elseif ifig <= 99
        print(h,'-depsc',['BFIGS/',num2str(lo),'-',num2str(hi),'_',int2str(npa),'pass_',int2str(0),int2str(ifig),'.eps'])
    else
        print(h,'-depsc',['BFIGS/',num2str(lo),'-',num2str(hi),'_',int2str(npa),'pass_',int2str(ifig),'.eps'])
    end
end
totSILB
totinSILB
