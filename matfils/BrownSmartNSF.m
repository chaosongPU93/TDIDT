...
timstart=34600; timend=34722; delt=timend-timstart; %day 254
%timstart=34600; timend=34682; delt=timend-timstart; %day 254
...
slope=(ltmax-ltmin)/120;
miss=0;
for ifig=1:floor(keeptot/8)
    figure
    for isub=1:3 %4
        icount=(ifig-1)*4+isub;
...
%                     targkeepSILB(:,mksi(icount))=0.;
        ...
        if isub < 2.5
        subplot(4,5,(isub-1)*5+1,'align'); 
        hold on
        plot(targkeepPGC(:,mksi(icount)),'r');
        plot(tempkeepPGC(:,mksi(icount)),'g');
        plot(crossoff,'k--')
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(maxkeep(mksi(icount))),'fontsize',7);
        %text(90, 0.8*ltmin, num2str(keepPGCtn(mksi(icount))),'fontsize',7);
        text(10, 0.8*ltmin, 'PGC','fontsize',7);
        text(50, 0.6*ltmax, int2str(tempkeept(mksi(icount),floor(templen/2))),'fontsize',7); %This writes the central time of window
        text(90, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        axis([0 120 ltmin ltmax]);
        set(gca,'fontsize',8);
        box on
        subplot(4,5,(isub-1)*5+2,'align'); 
        hold on
        ...
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(maxkeep(mksi(icount))),'fontsize',7);
        %text(90, 0.8*ltmin, num2str(keepSSIBtn(mksi(icount))),'fontsize',7);
        text(10, 0.8*ltmin, 'SSIB','fontsize',7);
        %text(50, 0.8*ltmin, int2str(tempkeepoffSSIB(mksi(icount))),'fontsize',7);
        %text(70, 0.8*ltmin, int2str(targkeepoffSSIB(mksi(icount))),'fontsize',7);
        text(50, 0.6*ltmax, int2str(tempkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        text(90, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        %text(100, 0.6*ltmax, int2str(keepnj(mksi(icount),:)));
        axis([0 120 ltmin ltmax]);
        set(gca,'fontsize',8);
        box on
        subplot(4,5,(isub-1)*5+3,'align'); 
        hold on
        ...
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(maxkeep(mksi(icount))),'fontsize',7);
        %text(90, 0.8*ltmin, num2str(keepSILBtn(mksi(icount))),'fontsize',7);
        text(10, 0.8*ltmin, 'SILB','fontsize',7);
        %text(50, 0.8*ltmin, int2str(tempkeepoffSILB(mksi(icount))),'fontsize',7);
        %text(70, 0.8*ltmin, int2str(targkeepoffSILB(mksi(icount))),'fontsize',7);
        text(50, 0.6*ltmax, int2str(tempkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        text(90, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        %text(100, 0.6*ltmax, int2str(keepnj(mksi(icount),:)));
        axis([0 120 ltmin ltmax]);
        ...
        subplot(4,5,(isub-1)*5+4,'align'); 
        hold on
        plot(tempkeepPGC(:,mksi(icount)),'r');
        plot(tempkeepSSIB(:,mksi(icount)),'b');
        plot(tempkeepSILB(:,mksi(icount)),'k');
        plot(crossoff,'k--')
        PGC2=dot(tempkeepPGC(:,mksi(icount)),tempkeepPGC(:,mksi(icount)));
        SSIB2=dot(tempkeepSSIB(:,mksi(icount)),tempkeepSSIB(:,mksi(icount)));
        SILB2=dot(tempkeepSILB(:,mksi(icount)),tempkeepSILB(:,mksi(icount)));
        PGSIx=dot(tempkeepPGC(:,mksi(icount)),tempkeepSILB(:,mksi(icount)));
        PGSSx=dot(tempkeepPGC(:,mksi(icount)),tempkeepSSIB(:,mksi(icount)));
        SISSx=dot(tempkeepSILB(:,mksi(icount)),tempkeepSSIB(:,mksi(icount)));
        PGSIxn=PGSIx/sqrt(PGC2*SILB2);
        PGSSxn=PGSSx/sqrt(PGC2*SSIB2);
        SISSxn=SISSx/sqrt(SILB2*SSIB2);
        ave=(PGSIxn+PGSSxn+SISSxn)/3.0;
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(ave),'fontsize',7);
...
%         text(110, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        ...
        set(gca,'fontsize',8);
        box on
        subplot(4,5,(isub-1)*5+5,'align'); 
        hold on
        plot(targkeepPGC(:,mksi(icount)),'r');
        plot(targkeepSSIB(:,mksi(icount)),'b');
        plot(targkeepSILB(:,mksi(icount)),'k');
        plot(crossoff,'k--')
        PGC2=dot(targkeepPGC(:,mksi(icount)),targkeepPGC(:,mksi(icount)));
        SSIB2=dot(targkeepSSIB(:,mksi(icount)),targkeepSSIB(:,mksi(icount)));
        SILB2=dot(targkeepSILB(:,mksi(icount)),targkeepSILB(:,mksi(icount)));
        PGSIx=dot(targkeepPGC(:,mksi(icount)),targkeepSILB(:,mksi(icount)));
        PGSSx=dot(targkeepPGC(:,mksi(icount)),targkeepSSIB(:,mksi(icount)));
        SISSx=dot(targkeepSILB(:,mksi(icount)),targkeepSSIB(:,mksi(icount)));
        PGSIxn=PGSIx/sqrt(PGC2*SILB2);
        PGSSxn=PGSSx/sqrt(PGC2*SSIB2);
        SISSxn=SISSx/sqrt(SILB2*SSIB2);
        ave=(PGSIxn+PGSSxn+SISSxn)/3.0;
...
%         text(110, 0.6*ltmax, int2str(targkeept(mksi(icount),floor(templen/2))),'fontsize',7);
        ...
        subplot(4,5,(isub-1)*5+1,'align'); 
        hold on
        plot(targkeepPGC(:,mksi(icount+1)),'r');
        plot(tempkeepPGC(:,mksi(icount+1)),'g');
        plot(crossoff,'k--')
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(maxkeep(mksi(icount+1))),'fontsize',7);
        %text(90, 0.8*ltmin, num2str(keepPGCtn(mksi(icount+1))),'fontsize',7);
        text(10, 0.8*ltmin, 'PGC','fontsize',7);
        text(50, 0.6*ltmax, int2str(tempkeept(mksi(icount+1),floor(templen/2))),'fontsize',7); %This writes the central time of window
        text(90, 0.6*ltmax, int2str(targkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
        axis([0 120 ltmin ltmax]);
        set(gca,'fontsize',8);
        box on
        subplot(4,5,(isub-1)*5+2,'align'); 
        hold on
        plot(targkeepSSIB(:,mksi(icount+1)),'b');
        plot(tempkeepSSIB(:,mksi(icount+1)),'g');
        plot(crossoff,'k--')
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(maxkeep(mksi(icount+1))),'fontsize',7);
        %text(90, 0.8*ltmin, num2str(keepSSIBtn(mksi(icount+1))),'fontsize',7);
        text(10, 0.8*ltmin, 'SSIB','fontsize',7);
        %text(50, 0.8*ltmin, int2str(tempkeepoffSSIB(mksi(icount+1))),'fontsize',7);
        %text(70, 0.8*ltmin, int2str(targkeepoffSSIB(mksi(icount+1))),'fontsize',7);
        text(50, 0.6*ltmax, int2str(tempkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
        text(90, 0.6*ltmax, int2str(targkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
        %text(100, 0.6*ltmax, int2str(keepnj(mksi(icount+1),:)));
        axis([0 120 ltmin ltmax]);
        set(gca,'fontsize',8);
        box on
        subplot(4,5,(isub-1)*5+3,'align'); 
        hold on
        plot(targkeepSILB(:,mksi(icount+1)),'k');
        plot(tempkeepSILB(:,mksi(icount+1)),'g');
        plot(crossoff,'k--')
        text(10, 0.8*ltmax, int2str(icount));
        text(70, 0.8*ltmax, num2str(maxkeep(mksi(icount+1))),'fontsize',7);
        %text(90, 0.8*ltmin, num2str(keepSILBtn(mksi(icount+1))),'fontsize',7);
        text(10, 0.8*ltmin, 'SILB','fontsize',7);
        %text(50, 0.8*ltmin, int2str(tempkeepoffSILB(mksi(icount+1))),'fontsize',7);
        %text(70, 0.8*ltmin, int2str(targkeepoffSILB(mksi(icount+1))),'fontsize',7);
        text(50, 0.6*ltmax, int2str(tempkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
        text(90, 0.6*ltmax, int2str(targkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
        %text(100, 0.6*ltmax, int2str(keepnj(mksi(icount+1),:)));
        axis([0 120 ltmin ltmax]);
        ...
        subplot(4,5,(isub-1)*5+4,'align'); 
        hold on
        plot(tempkeepPGC(:,mksi(icount+1)),'r');
        plot(tempkeepSSIB(:,mksi(icount+1)),'b');
        plot(tempkeepSILB(:,mksi(icount+1)),'k');
        plot(crossoff,'k--')
        PGC2=dot(tempkeepPGC(:,mksi(icount+1)),tempkeepPGC(:,mksi(icount+1)));
        SSIB2=dot(tempkeepSSIB(:,mksi(icount+1)),tempkeepSSIB(:,mksi(icount+1)));
        SILB2=dot(tempkeepSILB(:,mksi(icount+1)),tempkeepSILB(:,mksi(icount+1)));
        PGSIx=dot(tempkeepPGC(:,mksi(icount+1)),tempkeepSILB(:,mksi(icount+1)));
        PGSSx=dot(tempkeepPGC(:,mksi(icount+1)),tempkeepSSIB(:,mksi(icount+1)));
        SISSx=dot(tempkeepSILB(:,mksi(icount+1)),tempkeepSSIB(:,mksi(icount+1)));
        PGSIxn=PGSIx/sqrt(PGC2*SILB2);
        PGSSxn=PGSSx/sqrt(PGC2*SSIB2);
        SISSxn=SISSx/sqrt(SILB2*SSIB2);
        ave=(PGSIxn+PGSSxn+SISSxn)/3.0;
...
%         text(100, 0.8*ltmax, num2str(maxkeep(mksi(icount+1))),'fontsize',7);
%         text(110, 0.8*ltmin, num2str(keepPGCtn(mksi(icount+1))),'fontsize',7);
%         text(10, 0.8*ltmin, 'PGC','fontsize',7);
%         text(60, 0.6*ltmax, int2str(tempkeept(mksi(icount+1),floor(templen/2))),'fontsize',7); %This writes the central time of window
%         text(110, 0.6*ltmax, int2str(targkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
        axis([0 120 ltmin ltmax]);
        set(gca,'fontsize',8);
        box on
        subplot(4,5,(isub-1)*5+5,'align'); 
        ...
        plot(targkeepSILB(:,mksi(icount+1)),'k');
        plot(crossoff,'k--')
        PGC2=dot(targkeepPGC(:,mksi(icount+1)),targkeepPGC(:,mksi(icount+1)));
        SSIB2=dot(targkeepSSIB(:,mksi(icount+1)),targkeepSSIB(:,mksi(icount+1)));
        SILB2=dot(targkeepSILB(:,mksi(icount+1)),targkeepSILB(:,mksi(icount+1)));
        PGSIx=dot(targkeepPGC(:,mksi(icount+1)),targkeepSILB(:,mksi(icount+1)));
        PGSSx=dot(targkeepPGC(:,mksi(icount+1)),targkeepSSIB(:,mksi(icount+1)));
        SISSx=dot(targkeepSILB(:,mksi(icount+1)),targkeepSSIB(:,mksi(icount+1)));
        PGSIxn=PGSIx/sqrt(PGC2*SILB2);
        PGSSxn=PGSSx/sqrt(PGC2*SSIB2);
        SISSxn=SISSx/sqrt(SILB2*SSIB2);
        ave=(PGSIxn+PGSSxn+SISSxn)/3.0;
...
%         text(110, 0.6*ltmax, int2str(targkeept(mksi(icount+1),floor(templen/2))),'fontsize',7);
    ...
    %orient landscape
    print('-depsc',['254_',int2str(timstart),'_BrownS',int2str(ifig),'.eps'])
end
