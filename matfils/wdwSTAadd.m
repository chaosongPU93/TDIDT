%From wiggledaywig3(?).  Makes trhe stations generic.  Adds additional ones at the end.  Only shifts to nearest sample.
format short e
clear all
close all
...
%UPGRADING SINVE MODIFYOING READPOLS & READPERMS STOPED HERE
hi=6.5;
lo=1.25;
npo=2;
npa=2;
mshift=19; %maximum shift for the x-correlations.
loopoffmax=1.5; %1.5 for standard 1.5-6Hz; 4 for 0.5-1.5Hz.  2 for non-interpolated.
xcmaxAVEnmin=0.44; %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
...
%     sumsSILB2b=zeros(nwin,2*mshift+1);
    ...
    orient landscape
    % print('-depsc',['FIGS/',IDENTIF,'-',int2str(winlen/40),'s_',num2str(lo),'-',num2str(hi),'b.eps'])
...
%     stasnew=['LZB '
    ...
    for istanew=1:nstanew
        ...
        if LIA
            ...
            [opt,ort,nzeros,timsSTA]=readperms(prename,PERMSTA,PERMROTS,idx,lo,hi,npo,npa,fact,nwin,winlen,winoff,igstart);
        end
        ...
        for n=1:nin
            istart=istartkeep(n); 
            iend=istart+winlen-1;
            STAoptnew(istanew,(n-1)*winlen+1:n*winlen)=opt(istart:iend)';
        end
    end
    ...
    for istanew=1:nstanew
        for n=1:nin
            istart=(n-1)*winlen+1;
            iend=n*winlen;
            cc1=xcorr(STA1file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
            cc2=xcorr(STA2file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
            cc3=xcorr(STA3file(istart:iend,2),STAoptnew(istanew,istart:iend),mshiftnew,'coeff');
            [cc1max,icc1]=max(cc1);
            [cc2max,icc2]=max(cc2);
            [cc3max,icc3]=max(cc3);
            ccmax(istanew,n)=(cc1max+cc2max+cc3max)/3;
            loff(istanew,n)=(abs(icc1-icc2)+abs(icc1-icc3)+abs(icc2-icc3))/3;
            if abs(icc1-icc2)<=2 && abs(icc1-icc3)<=2 && abs(icc2-icc3)<=2 && ...
                    (cc1max+cc2max+cc3max)/3>1*xcmaxAVEnmin %0.8
                in(istanew,n)=1;
