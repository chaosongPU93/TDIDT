...
%set(0,'DefaultFigureVisible','off');
  ...
  %timoffrot(1,:)=[2005 254 07 42 12 +086 +020  80 55  50];
   timoffrot(1,:)=[2005 255 07 42 12 +086 +020  80 55  50];
...
%    timoffrot(1,:)=[2005 259 07 42 12 +086 +020  80 115  50];
%    timoffrot(2,:)=[2005 260 07 42 12 +086 +020  80 115  50];

   %timoffrot(1,:)=[2004 196 07 42 12 +086 +020  80 55  50];
...
mshift=19; %16 %11 %14 %12_for_+085+020 %15_for_+059-073  %19 for 128-s?
hi=6.;
%lo=0.75;
lo=1.5;
% hi=8.;
...
lo=0.02;
loopoffmax=12 %1.5
xcmaxAVEnmin=0.1 %36 %0.4 for 4s 1.5-6 Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
winlen=4*40;
winlen=128*40;
winoff=40/1;
winoff=40*8;
...
fltr=cputime-t
t=cputime;
%clear PGCE PGCN SSIBE SSIBN SILBE SILBN
...
%clear SSIBEf SSIBNf SILBEf SILBNf
dmate=cputime-t
t=cputime;

PGC=PGCEf+1i*PGCNf;
SSIBsplit=SSIBEfd+1i*SSIBNfd;
SILB=SILBEfd+1i*SILBNfd;

% Fix SSIB.  Rotate 10 degrees, shift N' by -6:
rotfast=10; shiftslow=-6;
SSIBrot=SSIBsplit*exp(1i*rotfast*pi/180);
SSIBeprime=real(SSIBrot); SSIBnprime=imag(SSIBrot);
SSIBnprime(1:tracelenPGC+shiftslow)=SSIBnprime(-shiftslow+1:tracelenPGC);
SSIBnprime(tracelenPGC+shiftslow+1:tracelenPGC)=0;
SSIB=SSIBeprime+1i*SSIBnprime;
...
dummy=imagPGC.*imagPGC;
orthPGC2=cumsum(dummy);
dummy=imagSSIB.*imagSSIB;
orthSSIB2=cumsum(dummy);
dummy=imagSILB.*imagSILB;
orthSILB2=cumsum(dummy);
        ...
        if abs(iPGSSbang-imaxinterpPGSS) <= loopoffmax*iup && ...
           ...
           interpPGSSn(iPGSSbang)+interpPGSIn(iPGSIbang)+interpSISSn(iSISSbang) >= 3*xcmaxAVEnmin
            ...
            if abovecut > 0.9*belowcut %-1
                ...
                optPGC=PGC2(istart+idiff+40)-PGC2(istart+idiff);
                optSSIB=SSIB2(istart-imaxPGSSwr+idiff+40)-SSIB2(istart-imaxPGSSwr+idiff);
                optSILB=SILB2(istart-imaxPGSIwr+idiff+40)-SILB2(istart-imaxPGSIwr+idiff);
                optamp=sqrt(optPGC+optSSIB+optSILB);
                orthPGC=orthPGC2(istart+idiff+40)-orthPGC2(istart+idiff);
                orthSSIB=orthSSIB2(istart-imaxPGSSwr+idiff+40)-orthSSIB2(istart-imaxPGSSwr+idiff);
                orthSILB=orthSILB2(istart-imaxPGSIwr+idiff+40)-orthSILB2(istart-imaxPGSIwr+idiff);
                orthamp=sqrt(orthPGC+orthSSIB+orthSILB);
...
% opt/orth used to be Ampcomp(n)
                mapfile(nin,:)=[timswin(n) xmaxPGSIntmp(n) xmaxPGSSntmp(n) ...
                    xcmaxAVEn(n) loopoff(n) optamp/orthamp cumsumtrdiff timswin(n)-2+idiff/40. cumsumtrdiff/cumsumtr(winlen)];
%                 if timswin(n) > 35204
...
figure
colormap(jet)
scatter(mapfile(:,5),mapfile(:,4),3,log(mapfile(:,6))/2.303)
hold on 
plot(-5:5,xcmaxAVEnmin+zeros(11,1),'k:');
axis([-2 2 0.3 1.0])
hrf = plotreflinesr(gca,-1.5,'x','k');colorbar
