...
xcmaxAVEnmin=0.42; %0.44 for 002 Stanford %0.45; %0.36 for 4s 1-12 Hz; %0.4 for 4s 1.5-6 Hz and 6s 0.5-1.5Hz; 0.36 for 4s2-8 Hz ; 0.38 for 4s0.75-6 Hz; 0.094 for 128s 2-8Hz;  0.1 for 128s 1.5-6Hz; 0.44 for 3-s window?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                                                         STAopt(3,indx-tempbef-imaxSTA13wr:indx+tempaft-imaxSTA13wr);
    ...
    fid = fopen(['ARMMAP/MAPS/NEW/map',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
...
%% Lines above plot + or - template:
                        ...
                        box on
                        set(gca,'XTick',(is:4:ien),'fontsize',6);
...
%                     ymi=min(min([cumsumSTA12tr cumsumSTA13tr cumsumSTA23tr]));
                    ...
                    ccdat(nt,5)=meanalln;
                    ccdat(nt,6)=(ST1sq(end)+ST2sq(end)+ST3sq(end))/3;
...
%                     text(ien-0.8, ymi+0.1*(yma-ymi), num2str(STA32file(nt,1),2),'fontsize',6);
...
%                     pkfile(nt,:)=[STA1file(winlen*(nt-1)+1+(winlen/2)) pks(imaxpk,1) maxpk pksid12 pksid13 pkwid rat14];
        ...
        orient landscape
        if ifig <= 9
            print('-depsc',['ARMMAP/WIGS/NEW/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(0),int2str(ifig),'.eps'])
        elseif ifig <= 99
            print('-depsc',['ARMMAP/WIGS/NEW/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(0),int2str(ifig),'.eps'])
        else
            print('-depsc',['ARMMAP/WIGS/NEW/',IDENTIF,'-',num2str(lo),'-',num2str(hi),'ms',int2str(mshift),'-',int2str(winlen/sps),'s','.',int2str(ifig),'.eps'])
        end
    end
    fid = fopen(['ARMMAP/MAPS/NEW/ccdat_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'a');
    fprintf(fid,'%5i %4i %4i %11.4e %7.2f %7.2f\n',ccdat(1:nin,:)');
    fclose(fid);
    fid = fopen(['ARMMAP/WIGS/NEW/wigdat',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
    fprintf(fid,'%10.3f %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e %12.4e \n',datfile(:,:));
    fclose(fid);
    bostsecfile(1,:)=bostsec;
    bostsecfile(2,:)=1;    
    fid = fopen(['ARMMAP/WIGS/NEW/bostsec',IDENTIF(1:8)],'w');
    fprintf(fid,'%10.3f %12.4e \n',bostsecfile(:,:));
    fclose(fid);
    fid = fopen(['ARMMAP/WIGS/NEW/guide',IDENTIF,'_',num2str(lo),'-',num2str(hi),'-','ms',int2str(mshift),'-',int2str(winlen/40),'s'],'w');
