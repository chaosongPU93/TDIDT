...
%     SIfreq(end+1)=SIfreq(end);
    ...
    SIfreq2=((40/2)/(2*pi))*(realSILB(plotstart:plotend).*yprime'-xprime'.*imag(hSI))./abs(hSI).^2; %divided by 2 b.c. of centered f.d.
    ...
    for j=1:npages
       ...
       hold on
       %semilogy(timsPGC(istart:iend),SIfreq2(iscmplx:iecmplx),'b'); %DON'T KNOW WHY THIS DOESN'T WORK! 
       plot(timsPGC(istart:iend),SIfreq2(iscmplx:iecmplx),'b');
    ...
    cmplxseisms(ndata+1:ndata+longlen,2)=SIfreq2;
...
title(['2003.063  ',num2str(lo),'-',num2str(hi),' Hz'])
print('-depsc','Amp_v_Freq_2003.063.eps')
