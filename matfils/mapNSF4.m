...
orig4=0.33*locs4(:,2)+0.6i*locs4(:,3);
orig128=0.33*locs128(:,2)+0.6i*locs128(:,3);
if OLDER==1
    origOLDER=0.33*olderlocs(:,2)+0.6i*olderlocs(:,3);
    origOLDER=origOLDER*exp(1i*rotang);
else
    origYOUNGER=0.33*youngerlocs(:,2)+0.6i*youngerlocs(:,3);
    origYOUNGER=origYOUNGER*exp(1i*rotang);
end
orig4=orig4*exp(1i*rotang);
orig128=orig128*exp(1i*rotang);
%figure
for k=1:ntees
    for i=2:nin4-1
   ...
   elseif k==3
        ...
        axis equal
        axis([-4 3.5 -4 3.5])
        text(2.8, -3.6, 'c','FontSize',8);
    elseif k==4
        axes('position',[0.26 0.35 0.21 0.21],'FontSize',6,'DataAspectRatio',[1 1 1]) %,'XLim',[-4 4],'YLim',[-4 4])
        axis equal
        axis([-4 3.5 -4 3.5])
        text(2.8, -3.6, 'd','FontSize',8);
%     elseif k==5
%         subplot('position',[0.25 0.55 0.2 0.2])
    end    
    hold on
    if OLDER==0
        plot(origYOUNGER,'ro','MarkerSize',1,'linewidth',0.2)
    end
    hold on 
    plot(orig128(iend128:length(locs128)),'ro','MarkerSize',1,'linewidth',0.2)
    hold on 
    plot(orig128(1:istart128),'ko','MarkerSize',1,'linewidth',0.2)
    hold on 
    if OLDER==1
        plot(origOLDER,'ko','MarkerSize',1,'linewidth',0.2)
    end
    if k==3
        scatter(real(orig128(istart128:iend128)),imag(orig128(istart128:iend128)),60,locs128(istart128:iend128,1)...
        ,'linewidth',0.6)
    else
        scatter(real(orig128(istart128:iend128)),imag(orig128(istart128:iend128)),80,locs128(istart128:iend128,1)...
        ,'linewidth',0.6)
    end
%     if k==1
%         xline=[-3 -2];
%         yline=[-1.5 -0.77];
%         plot(xline,yline,'k','linewidth',2.5)
%     end
    timestamp=[int2str(tees(k,1)),'-',int2str(tees(k,2)),' s'];
    if k==3 || k==4
        scatter(real(orig4(istart4:iend4)),imag(orig4(istart4:iend4)),10,locs4(istart4:iend4,1),'filled')
        text(-3.7, 3, timestamp,'FontSize',6);
        text(-3.7, 2.5, 'Sept 11 2005','FontSize',6);
    else
        scatter(real(orig4(istart4:iend4)),imag(orig4(istart4:iend4)),15,locs4(istart4:iend4,1),'filled')
...
%         axes('position',[0.05 0.1 0.21 0.21],'FontSize',6,'DataAspectRatio',[1 1 1],'XLim',[-4 4],'YLim',[-4 4])
    ...
    hold on 
    h0=plot(orig128(iend128:length(locs128)),'ro','MarkerSize',1,'linewidth',0.2)
    hold on 
    h1=plot(orig128(1:istart128),'ko','MarkerSize',1,'linewidth',0.2)
    hold on 
    if OLDER==1
        h2=plot(origOLDER,'ko','MarkerSize',1,'linewidth',0.2)
    end
    scatter(real(orig128(istart128:iend128)),imag(orig128(istart128:iend128)),60,locs128(istart128:iend128,1)...
        ,'linewidth',0.6)
    scatter(real(orig4(istart4:iend4)),imag(orig4(istart4:iend4)),10,locs4(istart4:iend4,1),'filled')
...
% % %     scatter(0.33*locs4(istart4:iend4,2),0.6*locs4(istart4:iend4,3),10,locs4(istart4:iend4,1),'filled')
    ...
    timestamp=[int2str(tees(k,1)),'-',int2str(tees(k,2)),' s'];
    text(-3.7, 3, timestamp,'FontSize',6);
    text(-3.7, 2.5, 'Sept 12 2005','FontSize',6);
%     if k==1
%         %%xlabel('PGC-SILB: KM ESE','FontSize',7)
%         %ylabel('PGC-SSIB: KM NNE','FontSize',7)
%     end
...
%     axis equal
    ...
    timestamp=[int2str(tees(k,1)),'-',int2str(tees(k,2)),' s'];
    text(-3.7, 3, timestamp,'FontSize',6);
    text(-3.7, 2.5, 'July 14 2004','FontSize',6);
    if k==1
        %xlabel('PGC-SILB: KM ESE','FontSize',7)
        %ylabel('PGC-SSIB: KM NNE','FontSize',7)
    end
...
%     hold on
    if k==1
        ...
        text(2.8, -3.6, 'g','FontSize',8);
        k
    elseif k==2
        subplot('position',[0.47 0.08 0.21 0.21],'FontSize',6,'DataAspectRatio',[1 1 1],'XLim',[-3.4 3.4],'YLim',[-3.4 3.4])
        text(2.8, -3.6, 'h','FontSize',8);
        k
%     elseif k==3
%         subplot('position',[0.45 0.15 0.21 0.21],'FontSize',6,'DataAspectRatio',[1 1 1],'XLim',[-4 4],'YLim',[-4 4])
    end    
    if OLDER==0
        plot(origYOUNGER,'ro','MarkerSize',1,'linewidth',0.2)
    end
...
% % %     scatter(0.33*locs4(istart4:iend4,2),0.6*locs4(istart4:iend4,3),10,locs4(istart4:iend4,1),'filled')
    ...
    timestamp=[int2str(tees(k,1)),'-',int2str(tees(k,2)),' s'];
    text(-3.7, 3, timestamp,'FontSize',6);
    text(-3.7, 2.5, 'Mar 03 2003','FontSize',6);
    %if k==1
        %xlabel('PGC-SILB: KM ESE','FontSize',7)
