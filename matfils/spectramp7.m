...
PERMROTS=[0 90 32 00;  %PGC , Yajun's "0" changed to 90.
 ...
 POLROTS=[6 85 33 86;  %SSIB
          0 90 39 20;  %SILB
          0 90  7 00;  %KLNB
          4 75 38 00;  %TWKB Not sure about TWKB/MGCB
          4 70 48 00]; %MGCB
%           4 70 48 00;  %MGCB
%           4 75 38 00]; %TWKB
...
for ista=1:size(PERMSTA,1)
    ...
    swin=512; %for small window; in samples
    itot=0;
    for n=1:size(wins,1)
        ...
        for iswin=1:nswins
            ...
            STAoptxx(itot,1:swin/2+1)=sqrt(STAoptxx(itot,1:swin/2+1));
            [STAortxx(itot,1:swin/2+1) f1] = periodogram(STAort(istart:iend),[],[],sps);
            STAortxx(itot,1:swin/2+1)=sqrt(STAortxx(itot,1:swin/2+1));
            ...
            STAoptxx(itot,swin/2+3) = istart/sps;  %VERY last row gets estimate of envelope amplitude (optimal orientation)
        end
    end
    a=sortrows(STAoptxx,258); %Each row is a periodogram; first row smallest amplitude; last row largest.
    aa=cumsum(a(:,1:257));  %cumsum cumsums the columns
    b=sortrows(STAortxx,258);
    bb=cumsum(b(:,1:257));
    ...
    title([PERMSTA(ista,:),'  ',YEAR,'.',JDAY,'  opt'])
    xlim([0.1 20])
    ylim([2.5e-4 0.4])
    subplot(3,1,2,'align')
    ...
    title([PERMSTA(ista,:),'  ',YEAR,'.',JDAY,'  ort'])
    xlim([0.1 20])
    ylim([2.5e-4 0.4])
    subplot(3,1,3,'align')
    ccc=aaa./bbb;
    ...
    loglog(f1,ccc)
    hold on
    title([PERMSTA(ista,:),'  ',YEAR,'.',JDAY,'  opt/ort'])
    xlim([0.1 20])
    ylim([2.e-1 5])
    loglog([1.e-5 1.e5],[1 1],'k--')
    set(h,'PaperPosition',[0.25 0.25 8 10])
    print(h,'-depsc',['spectr',PERMSTA(ista,:),YEAR,'.',JDAY,'.eps'])
    clear STAoptxx
    clear STAortxx
end
...
axis square
title([allsta,'  ',YEAR,'.',JDAY,'  opt'])
print('-depsc',['spectr',YEAR,'.',JDAY,'square.eps'])
